...
- **Second-of-everything** ([`docs/conventions/09-SECOND-OF-EVERYTHING.md`](docs/conventions/09-SECOND-OF-EVERYTHING.md)): any lane touching a catalogue entity (code/SKU/number/name-keyed, operator-edited) ships a second-company, a second-location and a re-run/idempotency test. New `unique(['tenant_id', …])` on such a table without `company_id` fails `TenantOnlyUniqueOnCatalogueTablesRatchetTest` (live PG schema ratchet, PG lane) unless the baseline entry carries a written `waiver` reason.
