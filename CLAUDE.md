...
### 22. Journey Hardening — second-of-everything, one surface per concept, benchmark-first, campaign gate (post-2026-08-29 onboarding-bug class)
Manual onboarding testing found in two days a class of journey bugs that depth-first gates missed (single-company/location fixtures, tests asserting code not data meaning, duplicate surfaces invisible to diff review, no industry baseline in specs). Standing rules — details in `docs/conventions/09..11` and `docs/qa/`:
- **Second-of-everything** ([`docs/conventions/09-SECOND-OF-EVERYTHING.md`](docs/conventions/09-SECOND-OF-EVERYTHING.md)): any lane touching a catalogue entity (code/SKU/number/name-keyed, operator-edited) ships a second-company, a second-location and a re-run/idempotency test. Any unique key on such a table that lacks `company_id` fails `TenantOnlyUniqueOnCatalogueTablesRatchetTest` (live PG schema ratchet, PG lane); the baseline is legacy debt (shrink-only, ceilinged in the test) and a `waiver` is only for keys tenant-global by nature.
- **One surface per concept** ([`docs/conventions/11-ONE-SURFACE-PER-CONCEPT.md`](docs/conventions/11-ONE-SURFACE-PER-CONCEPT.md)): every noun is in [`docs/glossary.md`](docs/glossary.md) with one table, one primary write path, one operator surface; synonyms are declared there, never discovered. No hand-rolled FE type beside a generated DTO.
- **Benchmark-first specs** ([`docs/conventions/10-BENCHMARK-FIRST-SPECS.md`](docs/conventions/10-BENCHMARK-FIRST-SPECS.md)): a spec/brief for a user-facing flow opens with the Odoo/ERPNext/Dolibarr baseline table (guarantee · AutoERP today `path:line` · decision) — round-0 check 6 blocks the gate without it.
- **Promotion gate**: a green run of the automated onboarding campaign (`scripts/campaign-onboarding.sh`, [`docs/qa/ONBOARDING-CAMPAIGN.md`](docs/qa/ONBOARDING-CAMPAIGN.md)) and a clean `tenant:census-day-one` are promotion preconditions (`docs/handoff/PROMOTION-CHECKLIST-*`). Tester findings flow through [`docs/qa/MANUAL-TESTING-LOOP.md`](docs/qa/MANUAL-TESTING-LOOP.md).
