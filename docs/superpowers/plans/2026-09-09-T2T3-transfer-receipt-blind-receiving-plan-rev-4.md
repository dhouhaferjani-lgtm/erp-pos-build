...
### 0A.2 Round-2 closure table — every NOT CLOSED / REGRESSED row, and every qualified closure
...
### 0A.3 Slice executability — every non-OK row of gate r3
...
### 0A.4 Source-ledger verification — the one issue raised
...
### 0A.5 Spec-fidelity table — every non-`PRESENT` row of both sub-tables
...
### 0A.7 Consistency sweeps rev 4 performed while closing the findings above
...
### 0A.8 B1 — every line rev 4 changed to make the `/complete` call graph single-valued
...
### 0B.2 Slice executability — every non-OK row of gate r2
...
### 0B.3 Spec fidelity matrix — every non-`PRESENT` row of all four sub-tables
...
### 0C.2 Slice executability assessment — every non-`Present` row
...
### 0C.3 Spec fidelity matrix — every non-`PRESENT` row
...
- **Dirty-tree census at `e734ee872` — exactly TWO unrelated untracked files** (gate r1 m1: rev 1's prose said "three" in two places while listing two; the true count is two; gate r2 confirmed the rev-2 statement accurate, gate r3 confirmed it again at its own audit HEAD, and the census is unchanged at the rev-4 pin). Quoted:
...
git diff --name-only e734ee8723a48c1b0346c46b879cdadacb7c0a7e.."$DISPATCH_SHA" | grep -v '^docs/' || echo 'NO PRODUCTION DRIFT'
```

Record `DISPATCH_SHA` in the handback. Rules:

1. If `DISPATCH_SHA` equals the rev-4 planning HEAD `e734ee8723a48c1b0346c46b879cdadacb7c0a7e`, the citations are pinned; start.
...
Plan: docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-4.md
...
**Every string above is a literal repo-relative path, with no alias and no ellipsis (closes gate r3 MAJOR M1).** Rev 3 abbreviated five French locale files as `.../fr/<ns>.json` inside the English bullets, so a literal dispatch inventory, an ownership checker or a handback comparison could not resolve them and the ledger was only exact under human inference. The ten web locale files and the two POS locale files are now one bullet each; all twelve were opened at the rev-4 pin `e734ee872` and exist. The counts are **unchanged**, and now bullet count equals path count so a parser needs no normalisation: the "Modify (production)" block holds **40 bullets = 40 paths**. Three of them are the register's lane-infrastructure and test-class entries, explicitly marked as such in the bullets themselves (`apps/api/tests/Feature/Inventory/TransferBlindLeakOracleTest.php`, `apps/api/tests/feature-lane-manifest.json`, `.github/workflows/ci.yml`), leaving the **37** production-source modify paths rev 3 named; removing the fourth register member, `apps/api/app/Modules/Inventory/Providers/InventoryServiceProvider.php`, leaves the **36** single-push production modifies §11.4 counts as `16 + 7 + 12 + 1 = 36` — 16 `apps/web` source files, 7 `apps/pos` source files, **12** locale files (ten under `apps/web/src/locales`, two under `apps/pos/src/locales`) and `docs/03-ERP-INTEGRATION/REALIGNMENT-LOG.md`. The lane's ledger totals — 159 single-push files, 13 multi-push files, 30 touches, 189 push rows, 172 distinct ERP files — are therefore untouched, which is what gate r3's "Source-ledger verification" asked to preserve.
...
Stage 1 is executable against the untouched tree because every symbol it needs is a vendor class or a fixture the test itself creates. That is the difference from rev 3, whose stage 1 needed a production class that does not exist. The exact commands are in the row table below and in §13; both use a self-contained `phpunit-pgsql.xml` invocation rather than the `docs/sessions/t1/pg-test.sh` helper, which is a **gitignored session artifact** (CLAUDE.md rule 15) absent from a fresh worktree at the rev-4 pin — wherever else this plan writes that helper, the same substitution applies and is the memory rule `feedback_pg_test_db_per_session` written out.
