...
## 0A. Round-6 change log — DISPATCH-READY
...
### 0B.2 Round-4 closure table — rows gate r5 marked REGRESSED at dispatch
...
### 0C.2 Round-3 closure table — rows gate r4 left NOT CLOSED
...
### 0D.2 Round-2 closure table — every NOT CLOSED / REGRESSED row, and every qualified closure
...
### 0D.3 Slice executability — every non-OK row of gate r3
...
### 0D.4 Source-ledger verification — the one issue raised
...
### 0D.5 Spec-fidelity table — every non-`PRESENT` row of both sub-tables
...
### 0D.7 Consistency sweeps rev 4 performed while closing the findings above
...
### 0D.8 B1 — every line rev 4 changed to make the `/complete` call graph single-valued
...
### 0E.2 Slice executability — every non-OK row of gate r2
...
### 0E.3 Spec fidelity matrix — every non-`PRESENT` row of all four sub-tables
...
### 0F.2 Slice executability assessment — every non-`Present` row
...
### 0F.3 Spec fidelity matrix — every non-`PRESENT` row
...
Plan: docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-7.md
