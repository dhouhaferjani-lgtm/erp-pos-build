...
## Deployment record shape (observed 2026-09-09 via `application.one`, all four apps)

Each entry of `deployments[]` has: `deploymentId`, `title` = the full git commit message of the deployed commit, `description` = the literal string `Commit: <40-hex sha>` (e.g. `Commit: ad1d6ceb1df9428003f7e78ef0a53d6f38fb64db`), `status` (`done` when finished; other values while running or on error), `createdAt`/`startedAt`/`finishedAt` ISO timestamps, `errorMessage`, `logPath` = `/etc/dokploy/logs/<appName>/<appName>-<timestamp>.log` on the host, `applicationId`. The array is not guaranteed to be ordered or to have the newest entry at index 0 — correlate a push by `description == "Commit: <sha>"` and `createdAt` after the push time, then poll that `deploymentId` until `status == done`. Automatic deployments and manual `application.redeploy` produce the same record shape. The API redeploy takes ~3 min, worker/scheduler ~15 s, web ~5–10 min (observed durations).
