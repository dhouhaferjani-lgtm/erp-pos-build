...
| | |
|---|---|
| **Version** | **v6 — 2026-08-05** (revised after the round-5 scoped re-gate; v1–v5 verdicts were all **REJECT**) |
| **Date** | 2026-08-05 |
| **Status** | **REVISED — pending round-6 re-review, then owner decisions D-1…D-21** |
...
| **Review input** | Round 1: `docs/superpowers/reviews/2026-08-05-production-env-design-review.md` — 38 findings, 8 BLOCKERs. Round 2: `docs/superpowers/reviews/2026-08-05-production-env-design-review-round2.md` — re-gate of all 38 plus **N1–N6**, verdict **REJECT** on 4 blocker clusters. Round 3: `docs/superpowers/reviews/2026-08-05-production-env-design-review-round3.md` — **scoped** re-gate of the round-2 dispositions only; 6 BLOCKERs collapsing to **three root causes R3-N1/N2/N3**, plus ~10 MAJORs (**R3-N4…R3-N7**) and four confirmations. Round 4: `docs/superpowers/reviews/2026-08-05-production-env-design-review-round4.md` — **scoped** re-gate of R3-N1/N2/N3 and the Phase-0 code blockers; verdict **REJECT** on **3 BLOCKERs, 2 MAJORs and 1 MINOR** (Horizon master-timeout drain defeat; restic pairing/cycle-retention; bootstrap concurrency; offsite-lock ordering; `permissions:verify` ordering; stale workflow inventory). Round 5: `docs/superpowers/reviews/2026-08-05-production-env-design-review-round5.md` — **scoped** re-gate of the v5 dispositions; verdict **REJECT** on **2 BLOCKERs** (Mode A terminate-then-stop drain race; restic content-verification + cycle retention), **1 MAJOR** (two stale universal Phase-0 claims) and **2 v5-introduced MINORs** (bootstrap attempt-counter ordering; GitHub single-pending concurrency). **Every finding from all five rounds has an explicit disposition in §11 (round 1), §11.3 (round 2), §11.4 (round 3), §11.5 (round 4) and §11.6 (round 5).** |
...
| **v6** | **2026-08-05** | **This revision.** Round-5 scoped re-gate closure — dispositions in **§11.6**. Four residuals + two v5-introduced MINORs, no scope expansion beyond them: (1) **BLOCKER 1 — Mode A (cluster-wide restore) still ran `horizon:terminate`-then-stop**, leaving the replacement-worker race during the MOST destructive path (Swarm can start a replacement while the restore is fencing every queue consumer). v6 converts Mode A to the **IDENTICAL atomic `docker service scale=0` + full-task-set drain** used by Mode B (§4.6a), reusing Mode B's sub-procedure (changes 1–3, steps 2/3/5) so the two paths cannot drift; and it strengthens **every `stop_grace_period` requirement** (D-21, §3.3 worker row, §4.6a Mode B step 3, §7.7b, the Phase compose-completeness row, R-31) from `> longest job timeout (3,600 s)` to **`> HORIZON_SUPERVISOR_TIMEOUT (3900 s) + operational margin`**, matching the runtime invariant `stop_grace_period > supervisor.timeout` (a grace period between 3,600 and 3900 s would SIGKILL a still-draining worker). (2) **BLOCKER 2 — restic verification + retention.** (a) B4a bound only a `restic ls --json` inventory hash — **METADATA**, which a same-size mutation passes; v6 adds **CONTENT verification** (`restic restore` to a scratch dir + `sha256sum -c SHA256SUMS`) **before** the attestation and **before** the dead-man ping, binds `content_verified` into the attestation, and O-3b / restore selection rule 2 / §4.4c re-run it. (b) The retention loop **orphaned `role=media-mirror`**; v6 folds it into the per-cycle role set — **FOUR roles**. (c) v6 makes the forget **CYCLE-ATOMIC**: resolve **exactly one** snapshot per role (fail on MISSING **or** DUPLICATE — no `.[0].id`), pass the whole four-ID set to **ONE `restic forget`** + one `prune`, strengthen the postcondition to assert **BOTH** exactly-one-bound-role-set per RETAINED cycle **AND ZERO** snapshots per EXPIRED cycle, and require `cycles_to_expire` to be **tested against calendar-boundary fixtures**. (3) **MAJOR 4 — two stale universal claims.** §7.0.2's audit row 5 and DR-3 both implied Phase-0 provenance for offsite-leg artefacts created only in Phase 4; v6 qualifies row 5 (**"workflow artefact in Phase 0; both offsite legs from Phase 4.3b"**) and separates DR-3's **Phase-0 panel-independence** from its **Phase-4.3b GitHub-independence**. Wording only. (4) **Two v5-introduced MINORs** (design notes, not new mechanisms): the bootstrap dispatcher's `open`/SHA validation is moved **BEFORE** the attempt-counter increment so an invalid/unauthorized dispatch does not burn an attempt (the count check stays increment-before-read, the only race-sensitive gate); and GitHub concurrency keeping only **one pending** run is accepted as **serial-operator semantics** (the three attempts are sequential, separately-approved operator actions bounded by `attempts_used`/`attempt_cap`, not simultaneous dispatches). **No new owner decision.** Code facts re-verified against `apps/api` this session: `config/horizon.php:217` timeout=60, `config/queue.php:71` retry_after=90, `ProcessImportJob.php:56` timeout=3600. |
...
| **D-5** | **Email provider** | 🔧 **AMENDED 2026-08-06 (owner-confirmed, see `docs/handoff/DECISION-HANDOVER-prod-env-choices-2026-08-06.md` Decision 1): RESEND** (was Brevo). **Required — not optional.** `MAIL_MAILER=log` (`apps/api/.env.example:120`) silently swallows every email, and the app really sends: `EmailVerificationService`, `DocumentEmailService` (invoices to customers), `FraudAlertNotificationService`. Free tier expected to cover launch volume — **confirm current caps at signup**. The owner must verify the sending domain and add **SPF + DKIM + DMARC** records (Appendix A). App uses standard Laravel mail — provider-agnostic. | 0 (Resend free) | Email verification at onboarding; invoice delivery; `MAIL_PASSWORD` row in E-1 | ✅ |
| **D-6** | **Alert destination** | 🔧 **AMENDED 2026-08-06 (owner-confirmed, Decision 3): SENTRY (Crons + Uptime) + email backstop — Telegram DROPPED.** Owner ensures a paid Sentry plan including **Crons** (backup dead-man §6 O-3, scheduler O-4) and **Uptime** (O-1/O-2). **Every "Telegram" mention in §6's alert tables now reads as "the D-6 channel" = Sentry alert + email.** Dokploy notification event types must still include **database backups** and **volume backups** (route to email/Sentry webhook) — a silent backup outage is the realistic failure mode. If the plan turns out to lack Crons/Uptime: fallback = healthchecks.io free dead-man + email (§6 already names it). | Sentry plan | §6 entirely | ✅ |
...
| **D-21** *(NEW in v4)* | 🚨 **Accept the worker `stop_grace_period` that the Mode B / deploy drain requires** (§4.6a) | **ACCEPT — and note it costs money in the worst case, which is why it is a decision.** v4 drops `horizon:pause` as a holding state (a paused Horizon fails the image's own healthcheck, so an unattended pause invites the orchestrator to replace the draining task mid-job — round-3 finding 1.2). The replacement is a **single atomic `docker service scale <stack>_worker=0`** (which sets desired replicas to 0 and SIGTERMs in one operation — no separate `horizon:terminate`, so no replacement window; 🚨 v6/round-5 BLOCKER 1 applies this to Mode A as well as Mode B, §4.6a), letting Docker's stop grace period cover the in-flight job. **For that to be a drain rather than a kill, `stop_grace_period` on the worker service must exceed `HORIZON_SUPERVISOR_TIMEOUT` (3900 s, Phase 0.8k) + an operational margin — NOT merely the longest job timeout (3,600 s, `ProcessImportJob.php:56`). 🚨 v6 (round-5 BLOCKER 1) strengthens the bound to match the runtime invariant: the Horizon master holds up to its own supervisor `timeout` (3900 s) before force-exiting (`MasterSupervisor.php:167-203`), so a grace period between 3,600 s and 3900 s would SIGKILL a still-draining worker — the manifest requirement now reads `stop_grace_period > supervisor.timeout` (§4.6a change 3), not `> max job timeout`.** The owner accepts that (a) a worker stop can therefore block for up to an hour in the pathological case, (b) the deploy/restore runbooks state that bound up front, and (c) the alternative — capping job timeouts at, say, 900 s — is a product change to the import pipeline, ticketed but not taken now. 🎫 **Named exit:** move long imports to their own queue and their own worker service with its own grace period, so the general drain is bounded by seconds | 0 | **§4.6a step 3, §7.7b, §3.3 worker row, Phase 3.3** | ✅ the ticket bounds it |
...
| 5 | **worker** | same repo, target **`worker`**. 🚨 **v4/v6: declares `stop_grace_period` GREATER THAN `HORIZON_SUPERVISOR_TIMEOUT` (3900 s, Phase 0.8k) + operational margin — NOT merely the longest job timeout (3,600 s, `ProcessImportJob.php:56`); v6 (round-5 BLOCKER 1) strengthened the bound to match the runtime invariant `stop_grace_period > supervisor.timeout` (§4.6a)** — this is **D-21**, and it is what makes §4.6a's atomic `docker service scale=0` a *drain* rather than a mid-job SIGKILL | **`main`** | none | 4 CPU / **2 G** ⚠️ | shared `appstorage` → `/var/www/html/storage` | `php artisan horizon:status \| grep -q running` — ⚠️ **note this healthcheck is why `horizon:pause` is never held (R-31, §4.6a)** | api, redis healthy |
...
| **L4a″** | 🚨 **NEW in v3 — the deletable media mirror lives on the restic leg.** `restic backup` of the MinIO data directory, so deletions and replacements *do* propagate somewhere and the mirror can be pruned | hourly, with the cycle | Storage Box (restic) | §4.5 tiers — 🚨 **v6: forgotten as the `role=media-mirror` snapshot of its cycle in the cycle-atomic four-role `restic forget` (§4.4b2 rule 4)**, never orphaned |
...
| **B4a** | 🚨 **v6 (round-5 BLOCKER 2, part a) — CONTENT-VERIFY `$COMPLETE_SNAP` against `SHA256SUMS`, not just its inventory, because IT is the restore candidate.** v5 bound only a `restic ls --json` inventory hash — that is **METADATA** (paths/sizes/timestamps), which a **same-size mutation passes**; a corrupted-but-same-length dump could then be attested and dead-man-pinged, caught only later at restore (too late — the marker already advertised a bad candidate) | **(1) CONTENT:** `restic restore "$COMPLETE_SNAP" --target "$SCRATCH"`, then `( cd "$SCRATCH" && sha256sum -c "$STAGE/SHA256SUMS" )` — reads the **actual file bytes** of every data artefact and compares to the local authoritative digest. **(2) SET:** `restic ls --json "$COMPLETE_SNAP"` → `INVENTORY_SHA256`; assert it lists **exactly** the `SHA256SUMS` names **plus `MANIFEST.json`** (`ATTESTATION.json` not yet written, correctly absent) — this catches extra/missing files that `sha256sum -c` alone would not, and `MANIFEST.json` must hash to `manifest_sha256`. **Bind `content_verified=true` (both passed) AND `inventory_sha256` into the attestation** | v5 verified metadata only, but **B4 is the snapshot a restore actually reads**, and a same-size mutation is precisely the class an inventory hash cannot see. **The content verify runs BEFORE B5 writes the attestation and BEFORE the dead-man ping** — a complete/attestation pair can never exist unless the *complete* snapshot's file content was itself checked byte-for-byte against `SHA256SUMS` |
| **B5** | **Write the completion attestation** as its own small snapshot | `restic backup --tag "cycle=$TS" --tag "role=attestation" "$STAGE/ATTESTATION.json"` | `ATTESTATION.json` = `{cycle, data_snapshot, complete_snapshot, manifest_sha256, inventory_sha256, content_verified, os_check_rc, restic_check_rc, promoted_at}`. 🚨 **v6 (round-5 BLOCKER 2, part a): `content_verified` records that B4a's restore-to-scratch `sha256sum -c SHA256SUMS` passed against the *complete* snapshot** — O-3b and the restore path re-run that content check, not merely the inventory hash. 🚨 **v5: `inventory_sha256` is B4a's hash of the `$COMPLETE_SNAP` inventory** (the restore candidate), not the data snapshot's. It is the **externally-addressable index** O-3b and the restore path read first (a snapshot cannot contain its own ID; naming is always one direction) |

**Selection rules that make this unambiguous — these are the restore contract:**

1. **Restore selects by TAG, never by "latest".** `restic snapshots --tag "cycle=$TS" --tag "role=complete" --json` returns the restore candidate. `restic restore latest` is **FORBIDDEN in every runbook** — with 🚨 **v6 four roles per cycle (`data`, `complete`, `attestation`, `media-mirror`)**, "latest" is normally the attestation snapshot (created last), i.e. one JSON file.
2. **The pairing is verified before any data is written.** Read the attestation, then assert the named `complete` snapshot exists, carries `role=complete` and the same `cycle` tag, that the `MANIFEST.json` inside it hashes to `manifest_sha256`, that a fresh `restic ls --json "$COMPLETE_SNAP"` inventory hashes to the attestation's `inventory_sha256` (v5, the same binding B4a wrote), **and 🚨 v6 (round-5 BLOCKER 2, part a) — that a restore of `$COMPLETE_SNAP` to a scratch dir passes `sha256sum -c SHA256SUMS`** (the same CONTENT verify B4a performed, re-run at read time so a same-size mutation of the at-rest snapshot is caught before any data is written; `SHA256SUMS`'s own integrity is already bound through the manifest chain — §4.4c row 3). Any mismatch ⇒ **REFUSE**, fall back to the previous cycle, and alert. A `complete` snapshot with no attestation, an attestation naming a snapshot that is absent, an inventory that no longer matches `inventory_sha256`, or restored content that fails `sha256sum -c`, is a **failed/tampered promote**, not a restore candidate.
...
4. 🚨 **`restic forget` applies retention at the CYCLE level and forgets all FOUR role snapshots of an expired cycle as ONE atomic unit (v5, extended in v6 for round-5 BLOCKER 2 parts b/c).** `--group-by tags` is **WRONG here and is struck**: every cycle carries a **unique** `cycle=$TS` tag, so grouping on tags puts each cycle (and each role within it) in its **own singleton group** — restic then applies `--keep-hourly/daily/weekly/monthly` *within* each group, i.e. keeps everything, and the intended **cross-cycle** ageing never runs. The correct algorithm: (i) enumerate the distinct `cycle=<TS>` tags with their promotion timestamps; (ii) apply the hourly/daily/weekly/monthly policy **over the set of cycles** to compute which cycles to EXPIRE — this is `cycles_to_expire`, and 🚨 **v6 (round-5 part c) requires it to be DEFINED and TESTED against explicit calendar-boundary fixtures** (hour/day/week/month rollovers, DST transitions, and month-length edges) so a cycle can never be mis-aged; (iii) for each expired cycle, resolve **EXACTLY ONE** snapshot per role for **all FOUR roles — `role=data`, `role=complete`, `role=attestation`, and 🚨 v6 `role=media-mirror`** (created as a cycle snapshot at §4.4d step 5 / L4a″, retention-governed per §4.3 L4a″ and §4.5a1) — **failing loudly on a MISSING or a DUPLICATE role** (never `.[0].id`, which lets duplicates survive; never a silent skip of a missing role), then pass the **whole four-ID set to a SINGLE `restic forget` invocation** so the cycle is deleted as a unit or not at all (separate per-role `forget` calls can leave a partial cycle after a failure), followed by ONE `restic prune`. 🚨 **v6 (parts b/c) — the post-prune assertion (`assert_cycle_retention_invariant`) now requires BOTH (i) EXACTLY ONE valid `data`, `complete`, `attestation`, AND `media-mirror` snapshot per RETAINED cycle, all mutually bound** (the attestation names data+complete, and the `manifest_sha256`, `inventory_sha256`, and `content_verified` bindings match) **AND (ii) ZERO snapshots for every EXPIRED cycle.** v5's assertion checked only retained cycles and omitted `media-mirror`, so an orphaned expired media snapshot or an expired-cycle remnant passed silently. A pruning policy that split a cycle would manufacture the exact defect this protocol removes — the assertion is part of the script, not a convention.
...
| 🚨 **restic leg only (v4, §4.4b2; extended v5):** `ATTESTATION.json` for the cycle exists; the `complete_snapshot` it names **exists**, carries `role=complete` + the same `cycle` tag, its embedded `MANIFEST.json` hashes to `manifest_sha256`, a fresh `restic ls --json` of that complete snapshot hashes to the attestation's `inventory_sha256` (v5), **AND 🚨 v6 (round-5 BLOCKER 2, part a) a restore of that complete snapshot to a scratch dir passes `sha256sum -c SHA256SUMS`** (content, not metadata — matching the attestation's `content_verified`) | **REFUSE and fall back to the previous cycle.** A `complete` snapshot with no attestation — or an attestation naming an absent snapshot, or one whose inventory no longer matches `inventory_sha256`, or whose restored content fails `sha256sum -c` — is a **failed/tampered promote**, not a restore candidate. **Never restore `role=data`** except as a recorded forensic override; it carries a `staged` marker by construction |
...
COMPLETE_SNAP=$(restic backup --json --tag "cycle=$TS" --tag "role=complete" "$STAGE" \
                | jq -r 'select(.message_type=="summary").snapshot_id')
# B4a — 🚨 v6 (round-5 BLOCKER 2, part a): CONTENT-verify the COMPLETE snapshot (the restore candidate),
#        BEFORE the attestation and BEFORE the ping. `restic ls` inventory is METADATA — a same-size
#        mutation passes it — so RESTORE to scratch and read the actual bytes against SHA256SUMS.
SCRATCH=$(mktemp -d)
CONTENT_RC=$(run_rc sh -c "restic restore '$COMPLETE_SNAP' --target '$SCRATCH' && cd '$SCRATCH' && sha256sum -c '$STAGE/SHA256SUMS'")
INVENTORY_SHA256=$(inventory_hash "$COMPLETE_SNAP" "$STAGE/SHA256SUMS")   # restic ls --json | canonical-sort | sha256; asserts the exact file SET (extras/missing)
rm -rf "$SCRATCH"
{ [ "$CONTENT_RC" = "0" ] && [ -n "$INVENTORY_SHA256" ]; } \
    || { alert "COMPLETE snapshot content/inventory verify FAILED (content=$CONTENT_RC) — cycle stays STAGED"; exit 6; }
...
write_attestation "$STAGE/ATTESTATION.json" "$TS" "$DATA_SNAP" "$COMPLETE_SNAP" \
                  "$MANIFEST_SHA256" "$INVENTORY_SHA256" "content_verified=true" "$OS_RC" "$RESTIC_RC"
...
# 9b. 🚨 v6 (round-5 BLOCKER 2, parts b/c): CYCLE-LEVEL, CYCLE-ATOMIC retention over the FOUR role
#     snapshots of each cycle. `--group-by tags` remains WRONG (every cycle's unique cycle=$TS tag
#     self-groups, so cross-cycle ageing never runs). `cycles_to_expire` is tested against explicit
#     calendar-boundary fixtures (rule 4). For each EXPIRED cycle, resolve EXACTLY ONE snapshot per
#     role — data, complete, attestation, media-mirror — failing on MISSING or DUPLICATE, then pass
#     the whole four-ID set to ONE `restic forget` so a cycle dies as a unit or not at all.
for CYC in $(cycles_to_expire --keep-hourly 48 --keep-daily 14 --keep-weekly 8 --keep-monthly 12); do
  CYCLE_IDS=""
  for ROLE in data complete attestation media-mirror; do
    IDS=$(restic snapshots --tag "cycle=$CYC" --tag "role=$ROLE" --json | jq -r '.[].id')
    N=$(printf '%s\n' "$IDS" | grep -c . || true)   # `|| true`: grep exits 1 on 0 matches — do not trip set -e
    [ "$N" -eq 1 ] || { alert "retention ABORT cycle=$CYC role=$ROLE resolved $N snapshots (want exactly 1)"; exit 7; }
    CYCLE_IDS="$CYCLE_IDS $IDS"
  done
  restic forget $CYCLE_IDS            # ONE invocation → the four-role cycle is atomic
done
restic prune
assert_cycle_retention_invariant   # (i) EXACTLY one data+complete+attestation+media-mirror per RETAINED
                                   #     cycle, all mutually bound; AND (ii) ZERO snapshots per EXPIRED cycle
...
17. 🚨 **NEW in v4, corrected in v5/v6 — the restic promote is `role=complete` + `role=attestation`, never `restic backup MANIFEST.json`** (§4.4b2). 🚨 **v5/v6 (round-4/5 BLOCKER 2): retention is applied at the CYCLE level — NOT `--group-by tags`, which self-groups on the unique `cycle=$TS` tag and never ages cycles against each other** — and forgets all **FOUR role snapshots** (`data`+`complete`+`attestation`+`media-mirror`) of an expired cycle as ONE atomic `restic forget` (resolve exactly one per role; fail on missing OR duplicate), followed by `assert_cycle_retention_invariant` requiring **both** exactly one bound `data`+`complete`+`attestation`+`media-mirror` per RETAINED cycle **and ZERO snapshots per EXPIRED cycle**, with `cycles_to_expire` tested against calendar-boundary fixtures. 🚨 **v6: `$COMPLETE_SNAP` (the restore candidate) is CONTENT-verified at B4a (`restic restore` to scratch + `sha256sum -c SHA256SUMS`, not just an inventory hash) before the attestation and before the ping; `content_verified` + `inventory_sha256` are bound into the attestation and re-checked at restore and by O-3b.**
...
**Mode A — cluster-wide restore (DR-2, DR-3): stop the application. 🚨 v6 (round-5 BLOCKER 1): the worker is drained with the SAME atomic scale-to-zero + full-task-set procedure as Mode B — never `horizon:terminate`-then-stop.**

🚨 **v5 fixed Mode B's drain but left Mode A running `horizon:terminate` first (step 1) and stopping the service afterwards (step 2) — the identical two-command replacement window, during the MOST destructive restore path: while a cluster-wide restore is fencing every queue consumer, Swarm can start a replacement worker in the interval between the terminate and the stop, because the worker's desired replica count is still 1 until the service is stopped. v6 reuses Mode B's worker-drain sub-procedure verbatim so the two paths cannot drift.**

| # | Action | Verification |
|---|---|---|
| 1 | 🚨 **v6 — drain the worker with Mode B's atomic sub-procedure (§4.6a Mode B changes 1–3 and steps 2/3/5), NOT a separate `horizon:terminate` then stop.** Record the full pre-stop task-ID set (`WORKER_TASK`, `WORKER_TASK_BASELINE`, `WORKER_CID` — Mode B step 2), then issue a **SINGLE atomic `docker service scale <stack>_worker=0`** (or the panel stop — the same Swarm primitive; Mode B step 3), which sets desired replicas to 0 **and** SIGTERMs in one operation so no replacement can be scheduled. The drain COMPLETES only because Phase 0.8k raised the supervisor `timeout` to `env('HORIZON_SUPERVISOR_TIMEOUT', 3900)` and D-21 sets `stop_grace_period > HORIZON_SUPERVISOR_TIMEOUT` — otherwise the master exits 0 after 60 s (`MasterSupervisor.php:167-203`) and this "stop" is a silent kill | Mode B step 5, verbatim: recorded `$WORKER_TASK` is `Shutdown`/`Complete`, **running-task count is 0**, **every task id present was already in `$WORKER_TASK_BASELINE`** (`comm -13` empty — no Swarm replacement), container exit code **0, not 137**. **ANY task id outside the baseline, or a `137`, ⇒ ABORT the restore** |
| 2 | Dokploy **stop** the `api` and `scheduler` Applications (websocket may stay — it has no DB writes). The worker is already at desired replicas 0 from step 1 | `api` and `scheduler` show stopped; worker desired replicas = 0; `/health` returns connection-refused via Traefik |
...
| 6 | Start `api` → wait healthy → **restore the worker's desired replicas: `docker service scale <stack>_worker=<N>`** (or panel start) → start `scheduler` | §7.4 V-suite |

Stopping the applications is preferred over connection surgery because it is **observable** — a stopped Application is a fact in the panel and a 502 at the edge, not a race you hope you won. **The worker specifically is scaled to zero atomically rather than terminated-then-stopped, so the destructive cluster-restore path carries the identical no-replacement guarantee Mode B does — the single most dangerous divergence round 5 found is closed by making the two procedures one.**
...
**v2's DR-3 procedure.** 🚨 **v6 (round-5 MAJOR 4): DR-3's PANEL-independence rests on Phase-0 deliverables (registry, secret-free compose, break-glass package); its FULLY GitHub-independent leg — fetching `release-set.lock.json` from an offsite backup leg (step 4a) — is a Phase-4.3b guarantee, because the offsite legs do not exist until Phase 4.1/4.2 and Phase 0 is artefact-store-only (§7.0.2b). This is consistent: no total-host DR scenario exists before backups do.** Per-step prerequisites (Phase-0 unless the step notes otherwise):
...
| **O-3** | **Backup dead-man** 🚨 **v3: keyed on the AGE OF THE LAST `complete` MANIFEST**, not on cron exit status and not on the existence of a cycle directory (round-2 F7/N2) | **healthchecks.io free** — §4.4d step 11 pings **only after the PROMOTE** (§4.4b1 phase 3), i.e. only when both offsite legs have verified. 🚨 **v5 (round-4 BLOCKER 2 fix a): the ping is downstream of B4a, so it also requires the `$COMPLETE_SNAP` inventory-verify to have passed** — a cycle that staged, failed to upload, OR whose complete snapshot failed 🚨 **v6 content/inventory verification (`exit 6` — `restic restore` + `sha256sum -c SHA256SUMS`, not just an inventory hash)** produces **no ping**, which is the correct signal | **off-provider** | Telegram + email → escalation ladder (§4.4d item 14) | no ping in 90 min (hourly job + 30 min grace) |
| **O-3b** | 🚨 **NEW in v3 — offsite completeness audit. 🚨 v4 (round-3 finding 4.3): it now audits BOTH LEGS, because v3's version could not detect the failure it was named after.** v3 read only the newest Object Storage `MANIFEST.json`. If Object Storage received `complete` while restic's marker snapshot was missing, staged-only, or disconnected from its data snapshot, O-3b still read a document **asserting both legs verified** — it audited the assertion, not the property. v4 checks: **(a) leg A** — read the newest remote `MANIFEST.json`, assert `result == "complete"` and both `offsite.*.state == "verified"`; **(b) leg B** — `restic snapshots --tag "cycle=<TS>" --json`, assert the `role=complete` snapshot **and** the `role=attestation` snapshot exist, that the attestation names that complete snapshot **and** the data snapshot, that the manifest hash inside the complete snapshot matches the attestation's `manifest_sha256`, that a fresh `restic ls --json` inventory of the complete snapshot matches the attestation's `inventory_sha256` (v5), **and 🚨 v6 (round-5 BLOCKER 2, part a) — that a restore of the newest cycle's complete snapshot to a scratch dir passes `sha256sum -c SHA256SUMS`** (CONTENT, matching the attestation's `content_verified`, not merely the inventory hash — one restore per hour, bounded to the newest cycle) (§4.4b2 selection rule 2 — O-3b performs the SAME content+binding check the restore path does) | host cron, **reads both remotes** | on-box check, **off-box alert** | Telegram | **any leg** whose newest cycle is `staged`, unreadable, missing its pair, internally inconsistent (manifest OR inventory hash mismatch, OR failed content `sha256sum -c`), or older than 90 min — **including "leg A says complete but leg B has no bound complete snapshot"**, which is exactly the state v3 was blind to |
...
> 2. **The job's FIRST step is a guard, and it happens BEFORE any build credential (`docker login`, GHCR token) is read.** It reads `BOOTSTRAP_STATE` and 🚨 **v6 (round-5 MINOR) validates the SIDE-EFFECT-FREE preconditions FIRST — fail when `open == false` or when the dispatched SHA ≠ `designated_sha` — BEFORE touching the counter, so an invalid or unauthorized dispatch does NOT consume an attempt.** Only then does it — atomically — **INCREMENT `attempts_used` and RE-READ it**, gating on the **re-read** value: fail when `attempts_used > attempt_cap`. `attempt_cap = 3`. The count check is the one gate that must increment-before-read, because it is the only race-sensitive one: a racer that somehow slipped past the concurrency group still sees its own committed count and stops. The `open`/SHA checks are deterministic on the dispatch input and current state (not count races), so validating them first is strictly safer — it refuses a bad dispatch without spending a scarce attempt. Because the whole guard precedes every credential read, **no run can build without first having passed validation and claimed its own attempt number**.
>
...
>
> 🚨 **v6 design note (round-5 MINOR — GitHub keeps only one PENDING run).** GitHub's concurrency queue holds **one running + one pending** member; a third simultaneous dispatch evicts the older *pending* one. **This is accepted as serial-operator semantics, and no queue/admission mechanism is added:** the three attempts are NOT three simultaneous dispatches — each is a deliberate, separately-approved operator action taken *after* the previous attempt's outcome (an expected red Phase-0.9 CI, R-5) is known. A second *simultaneous* dispatch is therefore never a legitimate second attempt; the concurrency group exists only to serialise the rare accidental double-dispatch, not to preserve a backlog of pending attempts. The attempt budget is bounded by `BOOTSTRAP_STATE.attempts_used`/`attempt_cap`, not by pending-queue occupancy, so an evicted duplicate pending dispatch loses nothing.
...
**Audit trail the exception must leave** — 🚨 **all seven in v4** (rows 5 and 7 changed/added), recorded in the Phase 0 evidence set. 🚨 **v6 (round-5 MAJOR 4): the Phase-0 evidence for row 5 is the WORKFLOW-ARTEFACT copy of the lock plus the compose commit; row 5's OFFSITE-leg copies are a Phase-4.3b artefact, because the offsite legs do not exist until Phase 4.1/4.2 (§7.0.2b). Phase 0 is artefact-store-only:**
...
| 5 | 🚨 **v4 — the attested `release-set.lock.json` for the bootstrap set** (not a digest commit) + the commit that lands the **variable-referencing, digest-free** `compose.production.yml` | workflow artefact **in Phase 0**; **both offsite legs from Phase 4.3b** (once the legs exist — §7.0.2b); the compose commit in git |
...
| Complete | all eight persistent services **plus** the one-shot bucket job, the four named volumes, the network, resource limits, and healthchecks — i.e. it is the §3.3 table in executable form. 🚨 **v4/v6: the worker service carries an explicit `stop_grace_period` exceeding `HORIZON_SUPERVISOR_TIMEOUT` (3900 s, Phase 0.8k) + operational margin (D-21, §4.6a — v6 round-5 BLOCKER 1: matches the runtime invariant `stop_grace_period > supervisor.timeout`, not merely `> max job timeout`)** |
...
| **R-36** | 🚨 **NEW in v4, sharpened in v5/v6 — a retention policy that is not CYCLE-aware could split a cycle's snapshot set**, leaving data without its completion marker (or the reverse) and silently recreating the false-completion class. 🚨 **v5: `--group-by tags` is worse than that — it self-groups on the unique `cycle=$TS` tag and never ages cycles at all, so the repository grows without bound. 🚨 v6 (round-5): the cycle has FOUR role snapshots — `media-mirror` was being orphaned — and a per-role `forget` is not cycle-atomic** | Medium | §4.4b2 selection rule 4 | Cycle-level enumeration + resolve exactly one snapshot per role (fail on missing OR duplicate) + a SINGLE `restic forget` of all **four** role snapshots (`data`+`complete`+`attestation`+`media-mirror`) per expired cycle as a unit, then `restic prune`, then `assert_cycle_retention_invariant` (**both** exactly one bound four-role set per RETAINED cycle **and ZERO** snapshots per EXPIRED cycle) after every prune (§4.4d step 9b); `cycles_to_expire` tested against calendar-boundary fixtures. **The assertion is in the script, not in the convention** — a retention policy is exactly the kind of thing that gets tuned later by someone who has not read §4.4b2 |
...
## 11.6 Round-5 scoped re-gate disposition (v6)

Source: `docs/superpowers/reviews/2026-08-05-production-env-design-review-round5.md` — verdict **REJECT**. A **scoped** re-gate of the v5 dispositions only (BLOCKER 3, MAJOR 5 and MINOR 6 were CLOSED in round 5 and are NOT re-opened here). Every finding is closed **in the design** with the review's own prescribed fix. **No fix demanded a new owner decision** — the `stop_grace_period` strengthening is a correction to D-21's already-accepted bound (not a new acceptance), and the restic/retention and wording fixes are mechanism/text. Code facts re-verified against `apps/api` this session: `config/horizon.php:217` `timeout=60`, `config/queue.php:71` `retry_after=90`, `ProcessImportJob.php:56` `$timeout=3600`.

| ID | Round-5 finding | v6 closure | Where |
|---|---|---|---|
| **BLOCKER 1** | "Mode A still runs `horizon:terminate` first then stops the service after — the replacement-worker race persists on the MOST destructive path"; "D-21 and the service table still say `stop_grace_period > 3600`, weaker than the `> supervisor.timeout` invariant" | ✅ **CLOSED HERE.** **(a)** Mode A is converted to the **IDENTICAL atomic `docker service scale <stack>_worker=0` + full-task-set drain** as Mode B — it now records `WORKER_TASK`/`WORKER_TASK_BASELINE`/`WORKER_CID`, issues a single atomic scale-to-zero (no separate `horizon:terminate`), and aborts on any task id outside the baseline or a `137`; the Mode A table **references Mode B changes 1–3 / steps 2/3/5 verbatim** so the two cannot drift, and step 6 scales the worker back up. **(b)** Every `stop_grace_period` requirement is strengthened from `> longest job timeout (3,600 s)` to **`> HORIZON_SUPERVISOR_TIMEOUT (3900 s) + operational margin`** (matching the runtime invariant `stop_grace_period > supervisor.timeout`), across all six live occurrences (grep-verified: D-21, §3.3 worker row, §4.6a Mode B step 3, §7.7b step 2, Phase compose-completeness row, R-31). Line 1182's invariant already read `> supervisor.timeout` and is unchanged | **§4.6a Mode A**, D-21, §3.3 row 5, §4.6a Mode B step 3, §7.7b step 2, Phase table, R-31 |
| **BLOCKER 2** | "(a) B4a binds an `restic ls` inventory hash = METADATA; a same-size mutation is attested + pinged, caught only at restore; (b) the retention loop orphans `role=media-mirror`; (c) per-role `forget` is non-atomic, `.[0].id` keeps duplicates, missing roles skipped silently, postcondition checks only retained cycles" | ✅ **CLOSED HERE.** **(a)** B4a now **content-verifies** `$COMPLETE_SNAP` — `restic restore` to a scratch dir + `sha256sum -c SHA256SUMS` (actual bytes) — **before** the attestation and **before** the dead-man ping; binds `content_verified` into `ATTESTATION.json`; restore selection rule 2, §4.4c restic row, and **O-3b** all re-run the content check, not just the inventory hash. **(b)** `role=media-mirror` is added to the per-cycle role set — **FOUR roles**. **(c)** For each expired cycle, resolve **exactly one** snapshot per role (**fail on missing OR duplicate**; `.[].id` + count, never `.[0].id`), pass the **whole four-ID set to ONE `restic forget`** (cycle-atomic) + one `prune`; `assert_cycle_retention_invariant` now asserts **BOTH** exactly-one-bound `data`+`complete`+`attestation`+`media-mirror` per RETAINED cycle **AND ZERO** snapshots per EXPIRED cycle; `cycles_to_expire` is required to be **tested against calendar-boundary fixtures** | **§4.4b2** B4a/B5 + rules 2/4, §4.4c restic row, §4.3 L4a″, §4.4d step B4a + step 9b, **§6 O-3b** |
| **MAJOR 4** | "Two stale universal claims: §7.0.2 says all seven audit records are Phase-0 evidence but row 5 lists offsite legs created in Phase 4; DR-3 says every prerequisite is a Phase-0 deliverable while the offsite/GitHub-independent part is Phase 4.3b" | ✅ **CLOSED HERE — wording only.** The audit-trail header and **row 5** are qualified: **"workflow artefact in Phase 0; both offsite legs from Phase 4.3b"**. DR-3's intro now **separates Phase-0 panel-independence** (registry, secret-free compose, break-glass) **from the Phase-4.3b GitHub-independence** (offsite-leg lock fetch, step 4a). No mechanism change | **§7.0.2a** audit header + row 5, **DR-3** intro |
| **MINOR (attempt counter)** | "The attempt counter increments before dispatch-input validation — an invalid dispatch consumes an attempt" | ✅ **RESOLVED — validation moved before increment.** The guard now validates the side-effect-free preconditions (`open == true`, dispatched SHA == `designated_sha`) **first**, before touching the counter, so an invalid/unauthorized dispatch does not burn an attempt; only the **count** check keeps its increment-before-read (it is the sole race-sensitive gate). Design note, small ordering tweak — no new owner decision | **§7.0.2a** state-record item 2 |
| **MINOR (single pending)** | "GitHub concurrency keeps only one pending member; a 3rd dispatch evicts the older pending one — state whether all 3 attempts must be preserved" | ✅ **RESOLVED — serial-operator semantics explicitly accepted.** A v6 design note states the three attempts are **sequential, separately-approved operator actions** (each after the prior attempt's red-CI outcome, R-5), never simultaneous dispatches; the attempt budget is bounded by `attempts_used`/`attempt_cap`, not by pending-queue occupancy, so an evicted duplicate pending dispatch loses nothing. **No queue/admission mechanism is added** | **§7.0.2a** state-record design note |

### 11.6a Round-5 items recorded as already-closed (not re-opened)

| Finding | Round-5 status | Note |
|---|---|---|
| **BLOCKER 3 — bootstrap concurrency** | **CLOSED in round 5** | Fixed non-cancelling concurrency group + atomic increment-and-re-read before credentials; only the two v5-introduced MINORs above touch this area, and both are refinements, not re-opens |
| **MAJOR 5 — 0.8h1 before 0.8g** | **CLOSED in round 5** | Ordering fix confirmed; untouched |
| **MINOR 6 — workflow inventory + Probe A** | **CLOSED in round 5** | `origin/main` = `ci.yml`/`smoke-test.yml`/`sonarcloud.yml`; Probe A "stays reserved" wording; untouched |
