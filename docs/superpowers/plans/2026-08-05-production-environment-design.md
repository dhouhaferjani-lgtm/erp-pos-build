...
| | |
|---|---|
| **Version** | **v4 — 2026-08-05** (revised after the round-3 scoped re-gate; v1, v2 and v3 verdicts were all **REJECT**) |
| **Date** | 2026-08-05 |
| **Status** | **REVISED — pending round-4 re-review, then owner decisions D-1…D-21** |
...
COMPLETE_SNAP=$(restic backup --json --tag "cycle=$TS" --tag "role=complete" "$STAGE" \
                | jq -r 'select(.message_type=="summary").snapshot_id')
write_attestation "$STAGE/ATTESTATION.json" "$TS" "$DATA_SNAP" "$COMPLETE_SNAP" "$OS_RC" "$RESTIC_RC"
...
# 9b. retention: cycles age as a UNIT, and the pairing invariant is asserted after every prune
restic forget --group-by tags \
       --keep-hourly 48 --keep-daily 14 --keep-weekly 8 --keep-monthly 12 --prune
assert_no_orphaned_cycle_snapshots          # no cycle may keep `data` without `complete`, or vice versa
...
#### 🚨 Mode B — single-tenant restore (DR-1) with a FAIL-CLOSED drain (v3 round-2 F9; **re-ordered in v4 for round-3 R3-N1**)
...
🚨 **v4 re-orders this fence (round-3 R3-N1). Two changes, and the first is a deletion:**
...
2. **The worker application STOP is issued INSIDE `horizon:terminate`'s drain window, not after it closes.** `horizon:terminate` begins the graceful drain (SIGTERM, `fast_termination => false`, each worker finishes its current job); the Application stop is issued immediately behind it, so the container is already in *stopping* state for the whole drain. A service that is being stopped is not a service the orchestrator restarts, so **the healthcheck-replacement race is structurally absent rather than merely bounded**. The in-flight job finishes inside Docker's stop grace period — which is why **`stop_grace_period` on the worker service must exceed the longest job timeout (D-21)**, and why that is an owner decision rather than a footnote.
...
| **2** | 🚨 **v4 — RECORD THE WORKER TASK IDENTITY** *before* touching it | `WORKER_TASK=$(docker service ps <stack>_worker --filter desired-state=running --format '{{.ID}}')` and `WORKER_CID=$(docker ps -q -f name=<stack>_worker)` — recorded in the incident log | Step 5 asserts that **this same task** exited and that **no replacement appeared**. Without an identity recorded up front, "the worker is gone" and "the worker was replaced and is happily consuming" look identical from the outside |
| **3** | 🚨 **Drain the running jobs and stop the service in one movement** — no pause, and no gap between them | `php artisan horizon:terminate` **immediately followed by** the Application stop (panel stop, or `docker service scale <stack>_worker=0`) | `horizon:terminate` sends SIGTERM and, because `fast_termination => false` (`config/horizon.php:175`), each worker **finishes its current job** before dying. Issuing the stop inside that window means the task is *stopping*, not *unhealthy* — so there is no state in which the orchestrator schedules a replacement. 🚨 **`stop_grace_period` on the worker service must exceed the longest job timeout (3,600 s today — `ProcessImportJob.php:50-65`), or Docker's SIGKILL turns the drain back into a mid-job abort. That is D-21** |
...
| **5** | 🚨 **v4 — ASSERT THE SAME TASK EXITED AND NOTHING REPLACED IT** | `docker service ps <stack>_worker --format '{{.ID}} {{.CurrentState}}'` → the recorded `$WORKER_TASK` is `Shutdown`/`Complete`, **and the running-task count is 0**; `docker inspect --format '{{.State.ExitCode}}' $WORKER_CID` → `0` (not `137`, which is SIGKILL, i.e. the grace period was too short) | This is the assertion that makes the reorder *provable* rather than merely *plausible*. **A non-zero running-task count means a replacement was scheduled — ABORT the fence, do not proceed to the drop.** An exit code of `137` means the job was killed, not drained: proceed only after establishing what job was interrupted and what it left behind |
...
sleep 95                                   # > queue retry_after (90 s, config/queue.php:71)
RESERVED2=$(docker exec "$API" php artisan queue:monitor "$QUEUES" --json --max=999999 \
            | jq '[.[] | .reserved] | add')
[ "$RESERVED2" = "0" ] || { echo "A crashed worker's job reappeared — do NOT proceed"; exit 1; }
...
| **14. 🚨 Fence + drain test** (v2 Finding 9; rewritten in v3 for round-2 F9; **rewritten again in v4 for round-3 R3-N1 — the v3 rehearsal could not have detected the defect it was meant to prove absent**): (a) start a job for the target tenant whose runtime **exceeds 90 s** — deliberately longer than three healthcheck intervals, which is the whole point — then execute Mode B steps 0–6 **while it is running**; (b) during steps 7–10, issue an API request for the target tenant and dispatch a fresh queued job for it; (c) after the drain, `systemctl start erp-backup.timer` early **on purpose** and confirm the fence detects the reconnect; (d) kill a worker mid-job (`docker kill` one worker process) and re-run probe A; (e) run the whole fence once with `stop_grace_period` deliberately set **below** the job's runtime | (a) the job's own log shows **completion, not a mid-transaction abort**; `docker service ps` shows the **recorded task ID** in `Shutdown/Complete`, **zero running tasks, and no replacement task ID at any point** (this is the assertion v3 lacked); container exit code is **0, not 137**; probe A reaches `reserved == 0` only afterwards; (b) the API request gets **403 `ORGANIZATION_UNAVAILABLE`** (not 503 — §4.6a) and the job is not consumed; `pg_stat_activity` for that datname stays at **zero** throughout the drop/restore window; (c) probe B **sees the backup backend and exits non-zero** — the test fails if the fence proceeds; (d) probe A stays non-zero because the orphan **remains reserved** (it does not migrate with no worker popping — §4.6a residual table) and the fence **refuses to proceed**; (e) exit code **137** appears and the runbook's abort path fires — this is how the team learns what a too-short grace period looks like *before* it happens under pressure |
...
**Change 1 — the dispatcher must exist on the DEFAULT BRANCH first (round-3 finding 3.1).** GitHub's [manual-workflow documentation](https://docs.github.com/en/actions/how-tos/manage-workflow-runs/manually-run-a-workflow) is explicit: a `workflow_dispatch` workflow must exist **on the default branch** to be dispatchable at all. The default remote branch here is `origin/main`, and `.github/workflows/` on it contains only `ci.yml`, `react-doctor.yml`, `smoke-test.yml` and `sonarcloud.yml`. v3 landed the new build workflow on `dev` at Phase 0.8i and dispatched it *before* promoting it to `main` at Phase 0.9 — **there is no dispatchable definition at that moment.** This is a hard execution failure, not a governance nuance.

> **Ruled: option (a) — a minimal BOOTSTRAP DISPATCHER lands on `main` first, in its own separately reviewed control-plane commit (Phase 0.8b1).** It is deliberately small: `workflow_dispatch` inputs (`ref`, `expected_sha`), `environment: production`, the `bootstrap_open` guard of Change 3, and a single job that **checks out the designated `dev` ref and calls the reviewed build logic from that ref** (a reusable workflow / composite action living on `dev`). The dispatcher on `main` is the *entry point*; the build definition being executed is still the reviewed `dev` one, and the run log records both SHAs.
...
> **Scope.** This exception applies to the initial stand-up of exactly one production environment. It is **bounded by a state record, not by prose** (below).
...
> **The state record is the enforcement.** A repository-variable/environment record `BOOTSTRAP_STATE` holds `{open: bool, designated_sha, attempts_used, attempt_cap, closed_at}`. **Every bootstrap job's FIRST step reads it and fails before any credential is read when `open == false`, when the dispatched SHA ≠ `designated_sha`, or when `attempts_used >= attempt_cap`.** `attempt_cap = 3`. Each dispatch increments `attempts_used`.
...
> **Where the digests go.** 🚨 **Not into git.** The build emits the attested `release-set.lock.json` of §7.0.2b; `deploy/production/compose.production.yml` is committed **variable-referencing and digest-free**, and is part of the `dev`→`main` promotion (Phase 0.9). **There is no digest commit, therefore no fixed point.**
...
| `release-set.lock.json` | **workflow artefact** (retained per §7.0.5's N) **+ both offsite backup legs, written with the backup cycle** | `{release_set_id, source_sha, ci_run_url, built_at, images: {api: "sha256:…", …}, bootstrap: bool}` | one per release; **never edited, only superseded** |
...
| # | Step | Command |
|---|---|---|
| 1 | 🚨 **v4 — record the worker task identity** | `docker service ps <stack>_worker --filter desired-state=running --format '{{.ID}}'`, recorded in the deploy record |
| 2 | **Drain what is running AND stop the service, in one movement** — no `horizon:pause`, no gap | `php artisan horizon:terminate`, **immediately followed by** the worker Application stop. `fast_termination => false` (`config/horizon.php:175`) means each worker finishes its current job; the stop means the orchestrator cannot schedule a replacement while it does. 🚨 **`stop_grace_period` must exceed the longest job timeout (D-21)** |
| 3 | 🚨 **v4 — assert the same task exited and nothing replaced it** | the recorded task ID is `Shutdown`/`Complete`, **running-task count is 0**, container exit code is `0` (a `137` means SIGKILL — the job was killed, not drained; investigate what it left behind before deploying) |
...
| **0.8b** | 🚨 **Land the image build+push workflow (§7.0.2)** — was Phase 6.1/6.2. 🚨 **v4: it also emits and attests `release-set.lock.json` (§7.0.2b) and writes it to both offsite legs** | **[A]** | Five digests pushed to GHCR from a green run; **a deliberate red-CI run pushes NOTHING**; a lock artefact whose **signature verifies** and whose digests match the pushed manifests |
| **0.8b1** | 🚨 **NEW in v4 (round-3 R3-N3) — land the BOOTSTRAP DISPATCHER on `main`**, in its own separately reviewed control-plane commit, **before** 0.8i can run (§7.0.2a Change 1). Includes the `BOOTSTRAP_STATE` guard step | **[O]** approves the control-plane commit, **[A]** authors | The workflow file present on `origin/main`; **a rehearsal dispatch against a scratch registry namespace proving `ref=dev` executes the DEV build definition**; a guard-step transcript showing failure when `open == false` |
| **0.8c** | 🚨 **Land `deploy/production/compose.production.yml` + `BOOTSTRAP.md` (§7.0.3)** — secret-free, **and 🚨 v4: VARIABLE-REFERENCING (`${IMAGE_*}`, no defaults), NOT digest-pinned in git** (§7.0.2b, D-20). `BOOTSTRAP.md` gains the fetch-and-verify-the-lock step. Worker service carries `stop_grace_period` per D-21 | **[A]** | Files checked in; `docker compose config` **fails loudly with the image variables unset**; **secret scan clean**; a cold-start dry run that materialises the lock from an offsite leg and comes up |
...
| **0.8g** | 🚨 **v3 — land `scripts/verify-production-release.sh`** (the V-0…V-12 suite as one script, §7.0.5), so the Phase 3.6 manual run and the Phase 6.6 workflow invoke **identical** checks; call `permissions:verify` (§7.4a) and **generate** the verifier invocations from `php artisan help` (§7.4b). 🚨 **v4: implement V-10 to the §7.4c input/pass table — both modes — and V-12 against the ATTESTED LOCK, not a checked-in file** | **[A]** | Script; a dry run against staging; a deliberately-failing V-2 and V-11 each **exit non-zero**; **V-10 exercised in both bootstrap modes**, including a denylisted digest after closure → FAIL, and an unverifiable lock → FAIL; the §7.4b option-set regression test red on a mutated signature |
...
| **0.8h1** | 🚨 **NEW in v4 — land the `permissions:verify {--tenant=} {--expect=}` command (§7.4a)** with its six required tests. **This is now the whole of V-11** — v3's inline SQL is withdrawn as non-executable (round-3 R3-N4) | **[A]** | Command + tests green, **including hostile-quoting, empty-expect, unknown-tenant, and per-tenant physical-DB-name resolution**; a deliberate missing permission on one tenant of several ⇒ fleet continues, aggregate exits non-zero |
...
| **R-36** | 🚨 **NEW in v4 — a `restic forget` policy that is not tag-aware could split a cycle's snapshot pair**, leaving data without its completion marker (or the reverse) and silently recreating the false-completion class | Medium | §4.4b2 selection rule 4 | `restic forget --group-by tags` + `assert_no_orphaned_cycle_snapshots` after every prune (§4.4d step 9b). **The assertion is in the script, not in the convention** — a retention policy is exactly the kind of thing that gets tuned later by someone who has not read §4.4b2 |
...
| The four workflow files are `ci.yml`, `react-doctor.yml`, `smoke-test.yml`, `sonarcloud.yml` — **no registry or deploy workflow** | `.github/workflows/` |
