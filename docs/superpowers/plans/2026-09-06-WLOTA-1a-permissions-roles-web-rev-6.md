<!-- W-LOT-A-1a rev 6, Codex CLI fix round 5 (gpt-5.6-sol, workspace-write, in-place edits on a byte copy of rev 5) 2026-09-09. Rev 5 = 80507fc0c is the verbatim base. Status: awaiting gate r6. -->
<!-- W-LOT-A-1a rev 6; Tasks 1/2/6 only. Audited at b05506eef4f477d86d4745b219a8651b11068801 on 2026-09-09. -->
# Slice plan W-LOT-A-1a — lot action permissions, general-manager role delta, web gating (rev 6)

## 0. Round-5 change log
...
| M-E — Push 5 collides with web auto-deploy | CLOSED — §11.8 removes the normal-path manual redeploy, polls `application.one` until `deployments[0].status=done` and `deployments[0].description=PUSH5_SHA`, and only then runs asset/fingerprint checks. A separately gated manual redeploy remains only for a recorded stale-edge incident after automatic completion. Web `autoDeploy=true` is resolved at [U-1 resolution:17](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/plans/2026-09-09-parapharmacy-staging-topology-U1-resolution.md:17). |
...
- Planning HEAD read in full: `b05506eef4f477d86d4745b219a8651b11068801`.
- Branch state: local `dev`.
- Commit: `docs(parapharmacy): Codex gate r5 on slice plan W-LOT-A-1a rev 5 — CHANGES-REQUIRED (M-C/N1 closed, 147/147 citations semantic-valid; open: §11 executability B-B2 OFF/rollback blocks, B-B3a prose transitions, B-B3b Artisan::call buffered registration marker, M-E redundant web redeploy, N-B/N-C minors)`.
- Plan preparation at the planning pin changed no production file, ran no test, and ran no git command; rev 6 is an in-place documentation edit over the rev-5 byte copy.
...
- Rev 5 is the verbatim base for this round-5 correction.
...
run_censuses() {
  ...
  while IFS= read -r tenant_id; do
    ...
    {
      php artisan tenants:run tenant:census-day-one --tenants="$tenant_id" --option='fail-on-drift=1' --no-ansi
      printf 'WLOTA1A-CENSUS-DAY tenant=%s outcome=PASS\n' "$tenant_id"
    } 2>&1 | tee "$day"
    {
      php artisan inventory:lot-drift-census --tenant="$tenant_id" --fail-on-drift --no-ansi
      printf 'WLOTA1A-CENSUS-LOT tenant=%s outcome=PASS\n' "$tenant_id"
    } 2>&1 | tee "$lot"
    {
      php artisan inventory:repair-phantom-default-batches --tenant="$tenant_id" --dry-run --no-ansi
...
`run_censuses` captures each per-tenant gate's `WLOTA1A-CENSUS-*` stdout marker in the corresponding unfiltered command log, aggregates every such raw line across all day/lot/phantom logs before running verdict validation, and never manufactures the aggregate from the expected tenant file. `assert_census_cmp` then expands the committed three-line heredoc template once per immutable-manifest UUID and compares the complete raw aggregate with `cmp`.
...
At the matching rollout boundary, invoke the reusable phase helper with one of these exact commands. The helper requires, validates, and consumes `PHASE`; it copies the helper into the current API, captures the manifest there, copies the whole manifest directory back, and requires a non-empty immutable tenant file:
...
capture_permission_snapshot "$MANIFEST" 'initial-off.before'
INITIAL_OFF_BEFORE
...
ROOT='/root/wlota1a'; source "$ROOT/helpers/host-helpers.sh"
MANIFEST="$ROOT/manifests/p1/expected-tenants.txt"; test -s "$MANIFEST"
capture_permission_snapshot "$MANIFEST" 'initial-off.after'
...
prepare_source_push 'p2' "$MANIFEST"
```
...
ROOT='/root/wlota1a'; source "$ROOT/helpers/host-helpers.sh"; source "$ROOT/shas/push-shas.env"
MANIFEST="$ROOT/manifests/p2/expected-tenants.txt"; test -s "$MANIFEST"
verify_source_push_inert 'p2' "$MANIFEST"
API="$(resolve_container 'erp-staging-api-kghqex')"
docker cp "$ROOT/helpers/container-helpers.sh" "$API:/tmp/wlota1a-container-helpers.sh"
...
ROOT='/root/wlota1a'; source "$ROOT/helpers/host-helpers.sh"; source "$ROOT/shas/push-shas.env"
MANIFEST="$ROOT/manifests/p2/expected-tenants.txt"; test -s "$MANIFEST"
prepare_source_push 'p3' "$MANIFEST"
```
...
ROOT='/root/wlota1a'; source "$ROOT/helpers/host-helpers.sh"; source "$ROOT/shas/push-shas.env"
MANIFEST="$ROOT/manifests/p2/expected-tenants.txt"; test -s "$MANIFEST"
verify_source_push_inert 'p3' "$MANIFEST"
```
...
prepare_source_push 'p5' "$MANIFEST"
```
...
AUTO_DEPLOYMENT="$WORK/web-auto-deployment.json"
for attempt in $(seq 1 120); do
  curl --fail-with-body --silent --show-error -H "x-api-key: ${DOKPLOY_API_KEY}" \
    "${DOKPLOY_URL}/api/application.one?applicationId=mY6P_PHb4pw-2LdG1Y7Ml" > "$AUTO_DEPLOYMENT"
  status="$(jq -er '.deployments[0].status' "$AUTO_DEPLOYMENT")"
  description="$(jq -er '.deployments[0].description' "$AUTO_DEPLOYMENT")"
  case "$status" in error|failed|cancelled|canceled) exit 1 ;; esac
  if test "$status" = 'done' && test "$description" = "$PUSH5_SHA"; then
    break
  fi
  sleep 5
done
test "$(jq -er '.deployments[0].status' "$AUTO_DEPLOYMENT")" = 'done'
test "$(jq -er '.deployments[0].description' "$AUTO_DEPLOYMENT")" = "$PUSH5_SHA"
scp "$AUTO_DEPLOYMENT" "$STAGING_HOST:/root/wlota1a/evidence/web-auto-deployment.json"
ssh "$STAGING_HOST" 'bash -s' <<'PUSH5_INERTNESS'
set -Eeuo pipefail
ROOT='/root/wlota1a'; source "$ROOT/helpers/host-helpers.sh"; source "$ROOT/shas/push-shas.env"
MANIFEST="$ROOT/manifests/p4/expected-tenants.txt"; test -s "$MANIFEST"
verify_source_push_inert 'p5' "$MANIFEST"
PUSH5_INERTNESS
for attempt in $(seq 1 90); do
...
The normal path contains no manual web redeploy: web `autoDeploy=true` is the resolved staging fact at [U-1 resolution:17](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/plans/2026-09-09-parapharmacy-staging-topology-U1-resolution.md:17). A manual redeploy is permitted only as an incident fallback when the preceding automatic-deployment file already proves `status=done` and `description=PUSH5_SHA`, but the served asset remains stale. Record the incident ID and run this separate block; it refuses to start before automatic completion and polls the fallback deployment to the same exact commit before returning:
...
test "$(jq -er '.deployments[0].status' "$WORK/automatic.json")" = 'done'
test "$(jq -er '.deployments[0].description' "$WORK/automatic.json")" = "$PUSH5_SHA"
...
curl --fail-with-body --silent --show-error -X POST "${DOKPLOY_URL}/api/application.redeploy" \
  ...
  --data "$(jq -nc --arg applicationId 'mY6P_PHb4pw-2LdG1Y7Ml' \
    --arg title "$FALLBACK_TITLE" --arg description "$PUSH5_SHA" \
    '{applicationId:$applicationId,title:$title,description:$description}')" >/dev/null
for attempt in $(seq 1 120); do
  curl --fail-with-body --silent --show-error -H "x-api-key: ${DOKPLOY_API_KEY}" \
    "${DOKPLOY_URL}/api/application.one?applicationId=mY6P_PHb4pw-2LdG1Y7Ml" > "$WORK/fallback.json"
  status="$(jq -er '.deployments[0].status' "$WORK/fallback.json")"
  description="$(jq -er '.deployments[0].description' "$WORK/fallback.json")"
  case "$status" in error|failed|cancelled|canceled) exit 1 ;; esac
  if test "$status" = 'done' && test "$description" = "$PUSH5_SHA" \
    && test "$(jq -er '.deployments[0].title' "$WORK/fallback.json")" = "$FALLBACK_TITLE"; then break; fi
  sleep 5
done
test "$(jq -er '.deployments[0].status' "$WORK/fallback.json")" = 'done'
test "$(jq -er '.deployments[0].description' "$WORK/fallback.json")" = "$PUSH5_SHA"
test "$(jq -er '.deployments[0].title' "$WORK/fallback.json")" = "$FALLBACK_TITLE"
for attempt in $(seq 1 90); do
  curl --fail --silent --show-error https://erp.otospex.dev/ \
...
scp "$WORK/fallback.json" "$BEFORE" "$AFTER" "$WORK/fallback-fingerprint.json" \
...
After registration is paused by the deployment runbook, capture the host-persisted `preactivation` manifest with the literal phase invocation, then compare it with `p4` on the host. No container-local `p4` file is referenced.
...
Only after that block exits zero may registration resume; then run API smoke, post-activation Playwright, and §12.
...
capture_permission_snapshot "$MANIFEST" 'rollback.before'
ROLLBACK_BEFORE
...
ROOT='/root/wlota1a'; source "$ROOT/helpers/host-helpers.sh"
MANIFEST="$ROOT/manifests/postactivation/expected-tenants.txt"; test -s "$MANIFEST"
capture_permission_snapshot "$MANIFEST" 'rollback.after'
...
Preserve the additive schema, provisioning marker, role ID, protected-role API behavior, generated DTO type, and read-only Roles page. Never restore manager recall or edit Spatie pivots directly. Because `SYNC_PERMISSIONS_ON_BOOT=true` remains set, the rollback API boot also runs the seeder and the exact pre/post snapshot `cmp` above must pass before rollback is accepted.
...
Emergency rollback follows only the self-contained separate-application procedure in §11.12: it defines its own workstation dependencies and updater, captures exact pre/post API permission snapshots, updates the full current env and performs `application.one` readback for worker → API → scheduler, verifies every replacement container with `printenv`, and requires the Dokploy, container, configuration-cache, permission-cache, and Horizon heredoc `cmp` gates. Never delete, rename, unmark, or mutate the protected role through direct SQL; never roll back the marker migration over a marked row; never hand-edit Spatie pivots or generated types.
...
14. Add Task-2 red tests, including marked-role mutation, wrapper string-option, roles-index response-contract, and fresh-registration stderr-marker tests.
...
39. Set `CANDIDATE_SHA=PUSH5_SHA`; poll the automatic web deployment until `deployments[0].status=done` and `deployments[0].description=PUSH5_SHA`, then require changed served asset, served feature string, exact build SHA, and feature fingerprint in one self-contained shell. Do not manually redeploy on the normal path.
...
47. Capture every structured stderr `WLOTA1A-RESEED` marker from the registration log window and `cmp` the disposable tenant's initial activated `APPLIED` record against the committed manifest template.
...
51. Run the exact configuration-cache, permission-cache, and Horizon command/exit/heredoc-`cmp` gates; resume registration only after all pass.
52. Run post-activation API and Playwright duplicate/B2/location gates.
53. Run the manifest §4 checklist.
54. Archive application records, Push 1–5 SHAs, manifests, backups, snapshots, literal migration comparisons, raw markers, expected-marker templates, censuses, automatic web-deployment evidence, structured registration stderr, generated hashes, asset evidence, OFF/ON/rollback comparisons, cache/Horizon comparisons, and reviewer verdicts under `/root/wlota1a/`.
55. Dispatch W-LOT-A-1b separately.
56. Dispatch W-LOT-A-1c only after W-LOT-A-1b acceptance.

## 17. Final verification checklist

- [ ] Planning SHA `b05506eef4f477d86d4745b219a8651b11068801` and its commit subject are recorded.
- [ ] Actual `DISPATCH_SHA` is recorded and named seams are re-censused after drift.
- [ ] Only Tasks 1, 2, and 6 are implemented.
- [ ] The round-5 gate and resolved U-1 fact sheet are read in full and every item is CLOSED.
...
- [ ] Every raw prefixed marker is aggregated before validation; every raw `WLOTA1A-CENSUS-*` line is collected into one census aggregate and no expected-tenant grep can hide an unexpected UUID.
- [ ] Every backup, migration, delta, verify, census, rollout, registration, reseed, and activation aggregate is compared with a committed literal-template expected record through `cmp`.
...
- [ ] Push 5 waits for web `deployments[0].status=done` and `deployments[0].description=PUSH5_SHA` before asset/fingerprint checks; the normal path performs no manual web redeploy.
