...
| **O-2** | **Runner-flip checklist, corrected at plan gate r3 M-2 — it is a REMOVAL plus a RECOMPUTE, never a decrement.** When `vars.SELF_HOSTED_RUNNER_READY` flips and `feature-lane-tenancy` starts running the `Identity` group, do exactly three things, in this order. **(1) Remove the ONE temporary allow-list entry** `\|PermissionCacheTenantScopingTest` from the `t6-phase0b-pgsql` `--filter` alternation at `dev:.github/workflows/ci.yml:1271` — otherwise `PermissionCacheTenantScopingTest` runs twice. **(2) Leave every group's `classes` alone.** `Identity.classes` is the **actual on-disk class count**, 33 (`dev:apps/api/tests/feature-lane-manifest.json:817-821`), not a budget: when a lane goes live the checker stops *enforcing* that number and it becomes informational — it does not become smaller (`apps/api/tools/feature-lane-manifest-check.php:774-781`, and the enforcement branch at `:782-799`). Lowering it would falsify the manifest. **(3) Recompute `gated_ceiling` from the groups that are STILL behind an inactive lane**, never by subtracting one. The checker sums `count($members)` over every group whose lane is still gated and compares that sum to the single top-level `gated_ceiling` (`feature-lane-manifest-check.php:765-772,802-819`), so the flip changes the ceiling by however many classes leave the gated set — **and the tenancy flip moves eleven groups, not one**: `Admin` 9, `Authorization` 2, `Billing` 2, `Company` 34, `CompanyConfig` 1, `Identity` 33, `Modules` 53, `Permissions` 3, `Subscription` 1, `SupportAccess` 13, `Tenant` 31 = **182 classes** out of the 1 254 the ceiling currently covers. A lane leaves the gated set when its `execution_gate` (and the owning job's matching `if:`) is removed, which is what the flip's edit actually is — the checker reads the declaration, not the variable's runtime value (`:704-721`). **All 70 gated lanes today share the one variable** `vars.SELF_HOSTED_RUNNER_READY`, so a single flip empties the gated set entirely and the `gated_ceiling` block at `:802-819` stops being consulted at all; only if the flip is ever staged lane by lane does a smaller recomputed ceiling remain. Recorded in the manifest note at `dev:apps/api/tests/feature-lane-manifest.json:817-821` (the note itself is `:820`) and repeated here so it survives the note. **Both `dev:` citations are local-`dev` facts** — the S-1 allow-list and the `Identity.classes` 33 raise both arrived with the 0a-1 merge and exist at neither the audit worktree's HEAD nor `origin/dev` (plan gate r2 citation audit). | a repository variable only the owner sets |
...
| 0a | `docs/superpowers/plans/2026-09-10-rbac-wave-0a.md` (**rev 4**) | `docs/handoff/HANDBACK-rbac-w0a-2026-09-10.md` | plan gates: `docs/superpowers/reviews/2026-09-10-rbac-plan-0a-codex-gate-r*.md` · lane gates: `docs/superpowers/reviews/2026-09-10-rbac-w0a-gate-r*.md` |
