...
**Audit gates are AUTONOMOUS (owner order 2026-07-11):** Codex runs each gate review itself via `claude -p --model claude-opus-4-8` (standard reviewer), escalating the same gate to `--model claude-fable-5` only on money-path BLOCKER/HIGH findings, money-path uncertainty, or recorded money-path deviations (brief §3 protocol: tag → review → fix → re-run until APPROVE; stop only on a 3-strike BLOCKER or an unmet Wave-G precondition). **The final whole-branch review happens in the Claude session after Codex reports done** — orchestrator-only Fable session dispatching Opus (verification) + Sonnet (discovery) subagents; the merge decision lives there, not in the gates.
