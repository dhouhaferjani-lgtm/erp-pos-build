# Execution plan T-2 / T-3 — transfer destination receipt (partial + discrepancy), close dispositions, blind receiving, minimum notifications (rev 3)

Date 2026-09-09 · Status: REV 3, for Codex plan gate round 3 · Lanes T-2, T-3, T-4-min · Four independently dispatchable ERP slices S1–S4 **plus one sibling-repository packet S4-mobile**.

**Supersedes** `docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-2.md` (REV 2), which itself superseded `docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-1.md` (REV 1). Rev 2 is the verbatim base of this document and is retained unchanged for diff review; **rev 3 is the only authority for implementation**. Nothing in this plan is stated "as in rev 1" or "as in rev 2": every contract an implementer needs is reproduced here.

**Gate closed by this revision:** `docs/superpowers/reviews/2026-09-09-t2-t3-plan-codex-gate-r2.md` — **VERDICT: CHANGES-REQUIRED**, 1 BLOCKER (B1), 6 MAJOR (M1–M6), 1 MINOR (m1), plus the non-OK rows of its "Slice executability" table and of all four sub-tables of its "Spec fidelity matrix". Every one of those is dispositioned in §0A. The gate's "Preserve" list and its "Round-1 closure register" are carried into rev 3 untouched.

**Gate closed by the previous revision:** `docs/superpowers/reviews/2026-09-09-t2-t3-plan-codex-gate-r1.md` — **VERDICT: CHANGES-REQUIRED**, 1 BLOCKER (B1), 8 MAJOR (M1–M8), 1 MINOR (m1), plus the non-OK rows of its "Slice executability assessment" and "Spec fidelity matrix". Every one of those is dispositioned in §0B, and gate r2's "Round-1 closure register" confirms which of them are CLOSED. The gate-r1 "Rejected false positives" and "Preserve" lists are carried forward untouched.
...
4. **Format exemplar:** `docs/superpowers/plans/2026-09-06-WLOTA-1a-permissions-roles-web-rev-9.md` (the current revision of the A-1a slice plan; its round change-log shape is reproduced in §0A/§0B here, newest round first).
...
### 0A.2 Slice executability — every non-OK row of gate r2
...
### 0A.3 Spec fidelity matrix — every non-`PRESENT` row of all four sub-tables
...
### 0B.2 Slice executability assessment — every non-`Present` row
...
### 0B.3 Spec fidelity matrix — every non-`PRESENT` row
...
- **Planning HEAD for rev 3, read in full and used for every `path:line` in this plan: `364b1668be62a252240e9459e720bf20299a2a61`** — re-derived as the FIRST step of this revision with `git rev-parse HEAD`. Rev 2 pinned `ab4b321f0417e536d676d8c94b97e09491f18b3c` and rev 1 pinned `73f2040c6d997b26a2a55dfb7d3c8742d361792e`; the ancestry from the rev-2 pin to this one is **documentation-only**, verified and quoted:

```text
$ git rev-parse HEAD
364b1668be62a252240e9459e720bf20299a2a61

$ git diff --name-only ab4b321f0..HEAD | grep -v '^docs/'
(no output — exit status 1 from grep, meaning zero non-docs paths)
```

  Every `path:line` in this plan is therefore still valid at `364b1668b`, and the rev-2 statement that the citations survive the T-1 merge stands. The rev-2 ancestry note is preserved for completeness: `git diff --name-only 73f2040c6..HEAD | grep -v '^docs/'` was likewise empty at the rev-2 pin. For completeness, rev 1's own ancestry note is preserved: planning opened at `7dcc77afcf0365d4a3ab9ca605d3dd2e1e647395`; while the plan was being written, the **T-1 lane merged into local `dev`** (`73f2040c6`, "Merge lane/t1-transfers-edge into dev"). `git diff --name-only 7dcc77afc..73f2040c6 | grep -v '^docs/'` lists six production paths: `.github/workflows/ci.yml`, `apps/api/app/Modules/Replenishment/Presentation/Requests/CreateTransferFromRequestsRequest.php`, `apps/api/tests/Feature/Inventory/StockTransferCompleteConcurrencyPostgresTest.php`, `apps/api/tests/Feature/Inventory/StockTransferEdgeCasesTest.php`, `apps/api/tests/Feature/Replenishment/ReplenishmentEdgeCasesTest.php` and `apps/api/tests/feature-lane-manifest.json`. **Every citation in this plan was re-verified against `364b1668b`**: the two CI drift gates are at `.github/workflows/ci.yml:2683-2700` and `:2705-2714`; the `backend-test-pgsql` allowlist this lane extends is the single `--filter` at `.github/workflows/ci.yml:1141-1142`, whose T-1 comment block opens at `:1132`; the Inventory feature lane is `"feature-lane-inventory/Inventory"` in `apps/api/tests/feature-lane-manifest.json`. The T-1 tickets of §15 are on `dev` at `docs/superpowers/tickets/2026-09-09-t1-*.md`.
...
- **Dirty-tree census at `364b1668b` — exactly TWO unrelated untracked files** (gate r1 m1: rev 1's prose said "three" in two places while listing two; the true count is two; gate r2 confirmed the rev-2 statement accurate, and the census is unchanged at the rev-3 pin). Quoted:
...
git diff --name-only 364b1668be62a252240e9459e720bf20299a2a61.."$DISPATCH_SHA" | grep -v '^docs/' || echo 'NO PRODUCTION DRIFT'
```

Record `DISPATCH_SHA` in the handback. Rules:

1. If `DISPATCH_SHA` equals the rev-3 planning HEAD `364b1668be62a252240e9459e720bf20299a2a61`, the citations are pinned; start.
...
Read from `apps/api/tests/feature-lane-manifest.json` at `364b1668b`, after the T-1 merge's own raise (`groups.Inventory.raise_note_2026_09_09_t1`, `groups.Replenishment.raise_note_2026_09_09_t1`, `gated_ceiling_raise_note_2026_09_09_t1`). These are the numbers gate r1's B1 table quoted, verified line by line:

| Manifest group | Lane (all PARKED) | `classes` at `364b1668b` | Anchor |
|---|---|---:|---|
...
final class StockTransferReceiptService
{
    ...
    public function receiveAllRemaining(StockTransfer $transfer, string $userId, string $idempotencyKey): TransferReceiptResult { /* §7.5 rule 9 */ }
}
```
...
The first three are today's list verbatim (`apps/api/app/Modules/Inventory/Presentation/Controllers/StockTransferController.php:28-32`).
...
- `complete` (`:231`) is **unchanged in the controller**: it keeps calling `$this->service->complete($existing->id, $user->id)` (`:226`) and **keeps returning 200** (the handler calls `response()->json(...)` with no status argument); the delegation to `StockTransferReceiptService::receiveAllRemaining(...)` happens one layer down, inside `StockTransferService::complete()` (§6.7b). Only the new `receive` and `close` return 201, and only they call `$this->receiptService` directly.
...
Plan: docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-3.md
...
The new file `docs/handoff/PROMOTION-CHECKLIST-2026-09-09-t2t3.md` is in S1's exact file list (§7.2), in push 1 of the ledger (§11.1), in §12's deployment variables, in §16 step 9, and is a named deliverable and handback item of dispatch packet §17.1.
 ...
 * the unsigned integer column migration 1d creates.
 */
#[TypeScript]
final class TransferReconciliationData extends Data
{
...
| `TransferNotificationRaceTest::test_two_workers_racing_the_same_notification_id_write_exactly_one_row` | RED-FIRST (the child exits non-zero on `SQLSTATE[23505]`) | RED |
| `TransferNotificationRaceTest::test_two_workers_racing_two_distinct_ids_write_two_rows` | **CONTROL** (case S4-C1 — the harness-liveness half) | RED at capture, then PASS *(see the note below)* |
...
**Note on S4-C1.** `test_two_workers_racing_two_distinct_ids_write_two_rows` is a control in **purpose** — its job is to keep passing forever, proving the harness really runs two processes and that `insertOrIgnore` does not swallow a legitimate second row — but it is **also red at capture**, because before the notification classes exist neither process can write anything and the count is `0`, not `2`. It is listed as a CONTROL so that a future change which makes it fail is read as "the idempotency key is too coarse", never as "the feature regressed"; its capture-time expectation is `RED`, and its post-implementation expectation is `PASS` forever after.

**Falsification procedure for S4's two controls — mandatory, recorded in the handback.**

| Case | Control | Mutation to apply | Expected result under the mutation | Revert |
|---|---|---|---|---|
| **S4-C1** | `test_two_workers_racing_two_distinct_ids_write_two_rows` | In `TransferInitiatedNotification`, temporarily drop `$userId` from the `Uuid::uuid5(...)` identity so two recipients collapse onto one id | `self::assertSame(2, DatabaseNotification::query()->count());` fails with `1`, proving the control watches the identity granularity and not merely "some row exists" | `git checkout -- ` that file |
...
Production modify, single-push (36): the thirty-seven paths listed in §10.2's "Modify (production)" block **minus** `InventoryServiceProvider.php`, which is in the register — so **16** `apps/web` source files (six under `features/stock-transfers`, five under `features/inventory`, plus `routes/index.tsx`, `ReplenishmentQueuePage.tsx`, `NotificationPanel.tsx`, `FraudSettingsPage.tsx` and `lib/i18n.ts`), **7** `apps/pos` source files, **12** locale files (ten in `apps/web/src/locales`, two in `apps/pos/src/locales`) and `docs/03-ERP-INTEGRATION/REALIGNMENT-LOG.md`. `16 + 7 + 12 + 1 = 36`.
...
- [ ] **(r2 M1)** The final constructors of `StockTransferService` (six arguments), `StockTransferReceiptService` (nine) and `StockTransferController` (six) are on the tree as §6.7b reproduces them; `complete()` is a three-line forwarder that opens no transaction and takes no lock; `receiveAllRemaining` takes a transfer **id**; every row of §6.7b's call-graph table resolves to a real method with the stated signature; `canAccessLocationOrFail()` performs the `return_to_source` source check before the service is entered; and the response is built by the controller on every receipt route.
...
**Red-first classification — stated for all four ERP slices (gate r2 M5).** §7.14b classifies every S1 method (two controls, S1-C1 and S1-C2, each with a falsification mutation); §8.11 states that S2 ships no control and why; §9.9b classifies every S3 method (four controls, C1–C4); §10.10b classifies every S4 method and test name (two controls, S4-C1 and S4-C2). The four rows gate r2 named as passing at HEAD — T13, T14, the second-company masking case and the notification payload scan — now open on an assertion that is red at HEAD, and the web "none of the three read permissions" case is reclassified as a CONTROL rather than being presented as red-first evidence.
