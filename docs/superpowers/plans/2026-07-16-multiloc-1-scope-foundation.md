...
- Create: `apps/api/app/Modules/Company/Presentation/Console/BackfillMembershipsCommand.php` (signature `users:backfill-memberships {--company=}`)
...
- Command `BackfillMembershipsCommand`: `--company=<uuid>` (required) — validates the id is a company in the CURRENT tenant DB, then backfills every memberless active user into it (same NULL-membership row shape). Idempotent; reports the inserted count. Operates on the current tenant connection (deploy invokes it via `php artisan tenants:run --tenants=<uuid> "users:backfill-memberships --company=<id>"` per multi-company tenant).
...
- [ ] Write the companion command's failing test. `apps/api/tests/Feature/Company/Console/BackfillMembershipsCommandTest.php`: in a MULTI-company tenant, a memberless active user is skipped by the migration; running `$this->artisan('users:backfill-memberships', ['--company' => $companyA->id])` (with the tenant DB active) then creates exactly one NULL-membership row for that user in `$companyA`, exit 0; asserts a second run is a no-op (still one row); asserts `--company=<unknown-uuid>` exits non-zero and writes no rows.
- [ ] Run it (red): `cd apps/api && ./vendor/bin/phpunit tests/Feature/Company/Console/BackfillMembershipsCommandTest.php`
- [ ] Implement the command. `apps/api/app/Modules/Company/Presentation/Console/BackfillMembershipsCommand.php` — signature `users:backfill-memberships {--company= : Company UUID to map memberless active users into}`. In `handle()`: require `--company`; verify it is a `companies.id` row in the current tenant connection (else `error()` + `return self::FAILURE`); select memberless active users (same `whereNotExists` predicate as the migration) and insert one NULL-membership row per user for that company (VIEWER, is_primary false, active); `info()` the inserted count; `return self::SUCCESS`. Constructor injection only; no `app()`.
...
- [ ] Implement — permission-registration migration (Finding 4). `apps/api/database/migrations/tenant/2026_07_16_100100_register_manage_location_access_permission.php`: idempotently `Permission::findOrCreate('users.manage_location_access', 'api')` (or a `WHERE NOT EXISTS` insert), grant it to the `admin` Spatie role (`Role::findByName('admin', 'api')->givePermissionTo(...)` guarded so re-runs no-op), then `app(PermissionRegistrar::class)->forgetCachedPermissions()` so the tenant-blind cache is flushed in the same push. This migration is the MECHANISM by which the permission ships; the seeder edit below is belt-and-suspenders for fresh installs.
...
2. **Multi-company tenants only:** grep the deploy logs for `multiloc.backfill.skipped_ambiguous_users`; for each such tenant, decide the correct company per user and run `php artisan tenants:run --tenants=<uuid> "users:backfill-memberships --company=<companyId>"`. Until run, those users stay deny-all (no regression). Single-company tenants need nothing here.
