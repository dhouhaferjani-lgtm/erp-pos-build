...
| Data fetching | paginated `{data,meta}` → `api.get` + return `response.data`; single-resource → `apiGet` (already unwraps). ALL query keys via `tenantScopedKey([...])`. **Invalidation uses BARE namespace prefixes** — `invalidateQueries({queryKey: ['replenishment']})` — NEVER `tenantScopedKey([ns])`: tenant/company are SUFFIXES, so a suffixed filter prefix-matches zero list keys (no-op; empirically proven at Gate C — `stock-transfers` ships this same latent no-op, masked by navigation). `audit-tanstack-keys.mjs` recognizes an invalidation-only exception: a bare array-literal `queryKey` is approved for `invalidateQueries` (prefix filter), while `useQuery`/`useQueries`/`fetchQuery`/`prefetchQuery` keys still require `tenantScopedKey([...])`. | double-unwrapping; raw query keys; suffixed invalidation filters |
...
- [ ] **Step 3:** Run the laptop-safe scoped preflight below → all green (PHPStan L8 on all feature-new backend seams, Pint, PHPUnit targeted, tsc, ESLint, targeted web Vitest). The two path overrides are additive; unset preserves preflight's full VPS/CI behavior. Then run the three targeted POS Vitest files and POS typecheck separately because preflight's Vitest cwd is `apps/web`.
  ```bash
  PREFLIGHT_SCOPE=paths \
  PREFLIGHT_TEST_PATHS='tests/Feature/Replenishment tests/Feature/POS/PosReplenishmentControllerTest.php tests/Feature/Inventory/TransferLineQueryServiceTest.php' \
  PREFLIGHT_PINT_PATHS='app/Modules/Replenishment app/Modules/Inventory/Application/Services/StockTransferService.php app/Modules/Inventory/Application/Services/TransferLineQueryService.php app/Modules/Inventory/Providers/InventoryServiceProvider.php app/Modules/Document/Application/Services/DraftPurchaseOrderService.php app/Modules/Document/Application/DTOs/DraftPurchaseOrderData.php app/Modules/Document/Application/DTOs/DraftPurchaseOrderLineData.php app/Modules/POS/Presentation/Controllers/PosReplenishmentController.php app/Modules/POS/routes.php app/Shared/Contracts/TransferLineReader.php app/Shared/DTOs/TransferLineDTO.php bootstrap/app.php bootstrap/providers.php database/migrations/tenant/2026_07_10_100000_create_replenishment_requests_table.php database/seeders/RolesAndPermissionsSeeder.php tests/Feature/Replenishment tests/Feature/POS/PosReplenishmentControllerTest.php tests/Feature/Inventory/TransferLineQueryServiceTest.php' \
  PREFLIGHT_PHPSTAN_PATHS='app/Modules/Replenishment app/Modules/Inventory/Application/Services/StockTransferService.php app/Modules/Inventory/Application/Services/TransferLineQueryService.php app/Modules/Document/Application/Services/DraftPurchaseOrderService.php app/Modules/Document/Application/DTOs/DraftPurchaseOrderData.php app/Modules/Document/Application/DTOs/DraftPurchaseOrderLineData.php app/Shared/Contracts/TransferLineReader.php app/Shared/DTOs/TransferLineDTO.php app/Modules/POS/Presentation/Controllers/PosReplenishmentController.php' \
  PREFLIGHT_VITEST_PATHS='src/features/replenishment' \
