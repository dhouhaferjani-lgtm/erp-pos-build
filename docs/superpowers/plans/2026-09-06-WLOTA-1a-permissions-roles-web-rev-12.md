<!-- W-LOT-A-1a rev 12, orchestrator addendum on a byte copy of rev 11 (gate r3 tenancy rulings 2026-09-10: M-1(r3) Push-3 Vitest split; §6.1 fourth declared flag-off exception = product.quantity_decimals) — earlier header: W-LOT-A-1a rev 11, orchestrator addendum on a byte copy of rev 10 (implementation gate r2 rulings 2026-09-10: B-1(r2) membership-derived response scope before the write, B-2(r2) membership requirement gated behind activation, §10 ledger split — web string-tolerance files move to Push 3, M-1(r2) list display precision fixed now, M-3(r2) create-response key declared) — earlier header: W-LOT-A-1a rev 10, orchestrator addendum on a byte copy of rev 9 (implementation gate r1 rulings 2026-09-10: B-2 string totals adopted at Push 3 with riders, §6.1 non-inert exceptions declared, I-1 scoped mutation responses, technician ruling, manifest raise + promotion order) — earlier header: rev 9, orchestrator addendum on a byte copy of rev 8 (gate r8 DISPATCH-READY, c5dddaddb) 2026-09-09. Rev 8 = 3305e6f46 is the verbatim base; the ONLY change is §0 + the §8.4 addendum below (owner/orchestrator ruling on legacy NULL-team roles). Status: DISPATCH-READY with addendum; the addendum is reviewed by tenancy-authz-reviewer at the implementation gate (§14), not by a new plan gate. -->
<!-- W-LOT-A-1a rev 12; Tasks 1/2/6 only. Audited at c76435df4a98193dfacb80ad9c25839169179509 on 2026-09-09. -->
# Slice plan W-LOT-A-1a — lot action permissions, general-manager role delta, web gating (rev 12)

## 000. Round-11 addendum — implementation gate r3 tenancy rulings (orchestrator, 2026-09-10)

Gate r3 tenancy on code `52f5ad796` / docs `a7010fe4d` ([register](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-w-lot-a-1a-impl-gate-r3-tenancy.md:1)): CHANGES-REQUIRED 0B/1M/4m — every r2 blocker and major VERIFIED closed on PostgreSQL; round 2b known-red verified both ways. Rulings: **M-1(r3)** the Push-3 Vitest must not depend on `BatchDetailPage.tsx` (Push 5) → split `BatchPermissions.test.tsx` into a Push-3 list Vitest and a Push-5 detail Vitest; ledger 76 rows. **N-1(r3)** §6.1 gains the fourth declared exception (`product.quantity_decimals`) — applied in this revision. **N-2(r3)** `Str::isUuid()` guard on the flag-off `expiring` location filter. **N-3(r3)** `pgsql` driver skip on the 2b pin test. **N-4(r3)** handback cites. Fix round 3 prompt: `docs/handoff/CODEX-PROMPT-WLOTA-1a-fix-round-3-2026-09-10.md`. Inventory and frontend r3 run on the round-3 HEAD.

## 00. Round-10 addendum — implementation gate r2 rulings (orchestrator, 2026-09-10)

Implementation gate r2 on `lane/w-lot-a-1a` HEAD `2fa724c1d` (fix round 1). Registers: [tenancy r2](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-w-lot-a-1a-impl-gate-r2-tenancy.md:1) CHANGES-REQUIRED B-1(r2)/B-2(r2)/M-1..M-3(r2)/N-1..N-5(r2); [inventory r2](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-w-lot-a-1a-impl-gate-r2-inventory.md:1) CHANGES-REQUIRED B-1(r2) executed double write-off / M-1(r2) / I-1..I-4(r2); [frontend r2](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-w-lot-a-1a-impl-gate-r2-frontend.md:1) CHANGES-REQUIRED B-1(r2) push-ordering crash / M-1(r2) / F-1..F-6(r2). Every gate-r1 item is VERIFIED closed by all three reviewers; gate-r1 frontend B-1 and B-7 are RETRACTED (Node-25 source-map artefact; FormRequests already enforce create/update). All rulings below are edits applied to this byte copy of rev 10; nothing else changed.

| Item | Ruling |
|---|---|
| **Tenancy/inventory B-1(r2) — post-write read-scope resolve throws 403 after the mutation committed** (`BatchController::loadScopedResourceRelations()` re-reads request input and calls `LocationScopeResolver::resolve()`, which throws; reproduced on PG for `PATCH` and executed on the write-off path: two calls, both 403, lot 10→6→2, two `stock_movements`) | **FIX.** The response scope of every mutation response derives from membership only (`LocationContext::getAllowedLocationIds()`; `null` = unrestricted), resolved ONCE before the write and passed into `loadScopedResourceRelations()`. The response-shaping step must never throw. Request-input `location_id` is a WRITE target, never a read scope (this closes tenancy M-2(r2): unrestricted actors get company-wide totals). Tests: `PATCH` and `POST …/write-off` with an out-of-scope body `location_id` from a restricted actor → 403 BEFORE the write with the seven-table snapshot byte-identical; unrestricted actor → 200 with company-wide totals; one write-off = exactly one `stock_movements` row (retry case). |
| **Tenancy B-2(r2) — undeclared flag-off change on `POST /users/{userId}/roles`** (422 without an active membership in the current company, every tenant at Push 3) | **GATE behind `activation->enforced()`.** §6.1 "While false" is the Push-3 dormancy contract and takes precedence over §8.8's wording; the membership requirement activates with the flag. Flag-off HTTP test pins the legacy behaviour (assignment succeeds without a current-company membership); flag-on test pins the 422. Owner may overturn to "declare as a third Push-3 exception" (see `OWNER-QUESTIONS-consolidated-2026-09-06.md`, 2026-09-10 entry) — then this row, §6.1, the release note and REALIGNMENT-LOG get the declaration instead of the gate. |
| **Frontend B-1(r2) — Push 3 before Push 5 crashes the Batches list** (pre-lane `BatchListPage.tsx:212` calls `.toFixed()` on `available_quantity`, which Push 3 turns into a string; flag-independent) | **§10 LEDGER SPLIT.** `apps/web/src/features/batches/types.ts` and `apps/web/src/features/batches/pages/BatchListPage.tsx` (with its Vitest) move from Push 5 to **Push 3** as the "web string-tolerance slice"; it is bidirectionally safe because `formatQuantity` accepts `string \| number`, and `BatchListPage`'s only gating change (`canCreate &&`) is inert while `batches.create` is already held by every role that saw the button. The permission-gating web changes stay in Push 5. Handback `:116` and the release note name the window. |
| **Tenancy M-1(r2) / inventory M-1(r2) — Push-3 declared contracts and three mutation responses unpinned in any CI lane** | **FIX.** One HTTP test per remaining path (recall, transfer, write-off) asserting the restricted actor's membership-scoped totals; the flag-off `/batches/expired?location_ids[]=`, `/batches/expiring?location_id=` and create-response assertions are added to (or duplicated into) `BatchReadLocationScopeTest`, which is in the `ci.yml` PG allowlist. `BatchActionPermissionsTest` stays outside the allowlist per the owner's ruling. |
| **Tenancy M-3(r2) — `POST /batches` gains `batch_stock: []`** | **DECLARE** in the Push-3 release note and REALIGNMENT-LOG (no code change). |
| **Frontend M-1(r2) / inventory I-1(r2) / frontend F-2(r2) — list renders a lot quantity at currency precision; `quantity_decimals` differs by endpoint** | **FIX NOW** (the lane touched the line). `BatchListPage` renders through `getQuantityDecimals(batch.product)`; prerequisites: `Batch.product` gains `quantity_decimals` in `types.ts`; the API eager-loads `product.unitOfMeasure` in `loadScopedResourceRelations()` and `getByCompany()`; the fixture gets a unit with `decimal_places != 4`; the Vitest case asserts `'3.1234'` and is renamed truthfully. The parseFloat deferral ticket keeps `BatchDetailPage` and adds the three `ExpiryWriteOffPage` deprecated-`formatQuantity` sites (F-3(r2)). |
| **Minors** | Tenancy N-1..N-5(r2) (cite drift; assertion counts; dead `whenLoaded` branch; `types.ts` `batch_stock[]` → string and `product_variant_id` → `variant_id`; seeder `$this->command?->info`), inventory I-2/I-3(r2) (ticket pointer on the Vitest case; cites), I-4(r2) → ticket (`BatchStock` dead float accessors, file untouched), frontend F-1(r2) (`ExpiredBatchStock.location_id: string`, UUID fixtures), F-4(r2) (evidence log header: `git rev-parse HEAD`, command, `node -v`), F-5(r2) (drop `?? 0`), F-6(r2) (`!== '1'` skip guard). |
...
- While false:
  ...
  - **(rev 11, gate r2 B-2(r2))** Role assignment (`POST /users/{userId}/roles`) does NOT require an active membership in the current company; the §8.8 membership requirement is gated behind the flag and activates with it.
  - **Declared flag-independent exceptions (rev 10 §0 Option B, rev 11 §00, rev 12 §000):** 4-dp string `total_quantity`/`available_quantity` on every batch response (and the `batch_stock: []` key on create); the location-filtered `/batches/expiring?location_id=` / `/batches/expired?location_ids[]=` totals; and `product.quantity_decimals` now emitted at the unit's real precision on `/batches` and `/batches/{uuid}` and every mutation response (was a hardcoded 4; expiring/expired already did this) — release note + REALIGNMENT-LOG carry it. Nothing else changes while false.
...
- `apps/api/tests/feature-lane-manifest.json` and `.github/workflows/ci.yml` (rev 10 §0 manifest raise).
- **(rev 11, frontend B-1(r2)) Web string-tolerance slice — ships WITH Push 3, before or together with the API string totals:** `apps/web/src/features/batches/types.ts`, `apps/web/src/features/batches/pages/BatchListPage.tsx`, `apps/web/src/features/batches/pages/__tests__/BatchPermissions.test.tsx` (its list-render cases). Bidirectionally safe (`formatQuantity` accepts `string | number`); the served Batches list must never call `.toFixed()` on a Push-3 string.
...
All Task-6 modified and added files EXCEPT the web string-tolerance slice already promoted with Push 3 (rev 11), including:
