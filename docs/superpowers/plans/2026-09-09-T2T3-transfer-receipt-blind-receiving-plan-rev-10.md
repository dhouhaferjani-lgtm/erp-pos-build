# Execution plan T-2 / T-3 — transfer destination receipt (partial + discrepancy), close dispositions, blind receiving, minimum notifications (rev 10)

Date 2026-09-10 · Status: REV 10 — **DISPATCH-READY** (rev 10 = rev 9 + §0AB: S1 implementation gate r2 rulings 2026-09-10 — tenancy CHANGES-REQUIRED 1B/2M/5m, stock-gl ACCEPT, inventory CHANGES-REQUIRED 0B/1M/3m, frontend CHANGES-REQUIRED 0B/3M/9m; fix round 2 dispatched). Earlier status line: REV 9 — **DISPATCH-READY** (rev 9 = rev 8 + §0AA: S1 implementation gate r1 rulings 2026-09-10 — three registers CHANGES-REQUIRED, fix round 1 dispatched; §7.15 tenancy citation 3 corrected; §14 S1 gate r2 adds frontend-conventions-reviewer under the M-2 default). Earlier status line: REV 8 — **DISPATCH-READY** per gate r6 (rev 8 = rev 7 + the §6.7b/§7.2 movement-creation amendment ruled 2026-09-10 on the S1 handback's blocking finding; the amendment is reviewed by stock-gl-interaction-reviewer at the S1 implementation gate, not by a new plan gate); rev 7 was (`docs/superpowers/reviews/2026-09-10-t2-t3-plan-codex-gate-r6.md`, 0 BLOCKER / 0 MAJOR / 1 MINOR); rev 7 differs from rev 6 ONLY by the four provenance sentences of gate r6 m1 and by the authority pointers (rev 6 → rev 7), so it needs no further plan gate · Lanes T-2, T-3, T-4-min · Four independently dispatchable ERP slices S1–S4 **plus one sibling-repository packet S4-mobile**.

**Supersedes** `docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-9.md` (REV 9, the S1 fix-round-1 authority; its own "Supersedes" line mis-pointed at itself — corrected here), which itself superseded `docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-8.md` (REV 8, the S1-dispatched text), which itself superseded `docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-7.md` (REV 7), which itself superseded `docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-6.md` (REV 6, the gated text), `docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-5.md` (REV 5), `docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-4.md` (REV 4), `docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-2.md` (REV 2) and `docs/superpowers/plans/2026-09-09-T2T3-transfer-receipt-blind-receiving-plan-rev-1.md` (REV 1). Rev 7 is the verbatim base of this document and is retained unchanged for diff review; **rev 8 is the only authority for implementation**. Nothing in this plan is stated "as in rev 1", "as in rev 2" or "as in rev 3": every contract an implementer needs is reproduced here.
...
## 0AB. Round-9 addendum — S1 implementation gate r2 rulings (orchestrator, 2026-09-10)

S1 fix round 1 (`lane/t2-receipt-spine` @ `d7123fa30`, source tip `ede6a990a`) was gated by the four §14 reviewers (frontend-conventions added by rev 9). Registers: [tenancy](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-t2-s1-impl-gate-r2-tenancy.md:1) CHANGES-REQUIRED 1B/2M/5m · [stock-gl](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-t2-s1-impl-gate-r2-stock-gl.md:1) **ACCEPT** 0B/0M/4m · [inventory](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-t2-s1-impl-gate-r2-inventory.md:1) CHANGES-REQUIRED 0B/1M/3m · [frontend](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-t2-s1-impl-gate-r2-frontend.md:1) CHANGES-REQUIRED 0B/3M/9m. **Every §0AA item (T-1..T-8, M-1, M-2, I-1..I-10, G-1..G-8) is CLOSED and re-verified**; the stock↔GL seam is accepted. Fix round 2 prompt: `docs/handoff/CODEX-PROMPT-T2T3-S1-fix-round-2-2026-09-10.md`. All rulings are edits to this byte copy of rev 9.

| Item | Ruling |
|---|---|
| **Tenancy T-9(r2) BLOCKER — i18n completeness ratchet red** (the option-(a) `lib/i18n.ts` spread reclassified `stock-transfers` from aliased to wired → 118 `ar` gaps; `frontend-lint` ∈ `all-checks-pass`) | **REVERT** `lib/i18n.ts` to the whole-namespace `enStockTransfers` alias (byte-identical to `93b106461`), delete `locales/ar/stock-transfers.json`, delete the badge test's `ar` case. The three new labels fall back to English in `ar` like the namespace's other 118 keys. Arabic coverage of the namespace (121 keys) → ticket, not S1. Never the pin/baseline/mirror. §0AA's "en/fr/ar" wording is corrected to "en/fr (ar stays aliased)". |
| **Tenancy T-10(r2)** no exit-0 preflight | **FIX** — preflight to exit 0 after the revert; `pnpm lint:ratchet` included. |
| **Tenancy T-11(r2)** implementer committed an edited copy of the orchestrator-owned plan | **RESOLVED on dev** (the two-line §7.12 I-2 note was adopted into dev's copy; blobs identical). Standing rule: implementers never edit plans/reviews; plan-level decisions go in the handback. |
| **Tenancy T-5(r2)** unscoped identity lookups on the three compatibility callers | **FIX** (`StockTransferService.php:312,328,397` predicate tenant+company, or take the controller's scoped model). |
| **Tenancy T-12(r2) / stock-gl nest-guard note** | **KEEP the guard**; recover runtime on the test side (once-per-class migrate or truncation isolation in `StockTransferEdgeCasesTest` / `InventoryTransferServiceTest`); disclose; `--parallel` never on the PG step (comment in `ci.yml`). |
| **Tenancy T-13/T-15(r2)** | FIX (PostgreSQL **15.15** in the handback; T-6 line in the checklist). |
| **Inventory I-11(r2) MAJOR — web Complete on `partially_received` books the whole remainder** | **PACKAGE**: (a) checklist disclosure (short shipments are closed via `POST /close` until S4); (b) confirm copy becomes **line-count** bearing per frontend F-4 (supersedes the "remaining total" wording), rendered only for `partially_received`, string comparison against `'0.0000'`, en/fr; (c) backend data-meaning test (receive 5 of 12 → `POST /complete` → destination `12.0000`, `completed`, `freight_uncapitalized '0.0000'`, identity holds). Behaviour unchanged in S1; **S4 must render per-line received/remaining behind `canSeeExpected` before any terminal affordance on a partial** (added to the S4 packet §17.4). |
| **Inventory I-12/I-13/I-14(r2)** | I-12 FIX — drop the cross-unit quantity totals from `TransferReconciliationSummaryData` (keep `lines`, `lines_with_discrepancy`, `freight_uncapitalized`), regenerate; I-13 FIX (one line); I-14 FIX (message). |
| **Stock-gl G-9..G-12(r2)** | FIX (test/comment adds; G-12 optional). |
| **Frontend F-1 MAJOR — hand-rolled `StockTransfer`/`StockTransferLine`/`StockTransferLineBatchAllocation` shadow the slice's own generated DTOs** | **REPLACE NOW** with aliases to `App.Modules.Inventory.Application.DTOs.*`; ruled file set widened by `__tests__/StockTransferListPage.test.tsx`, `__tests__/StockTransferDetailPage.batchAllocations.test.tsx` and the dead `?? '—'` at `StockTransferDetailPage.tsx:248` (F-10). `StockTransferType` / `TransferCostDistribution` duplicates → S4. |
| **Frontend F-2 MAJOR / tenancy T-14(r2) — `Badge` atom replaced by a span** | **RESTORE THE ATOM** with exactly three rows (`partially_received: 'warning'`, `closed_with_writeoff: 'danger'`, `closed_returned: 'default'`); pixel-identical; `StatusBadge` migration → S4. |
| **Frontend F-3 MAJOR — the detail page is now a web receipt surface under R5** | **RECORD** the masking obligation in the checklist's S1 disclosures and in §17.4 (S4 masks sent/remaining behind `canSeeExpected`/`blind` on this page; `StockTransferData` needs a receiver-shape discriminant in S2). No remaining column/total in S1. |
| **Frontend F-5..F-8, F-10..F-12 minors** | FIX in the round (F-5 exhaustive `satisfies Record<StockTransferStatus, true>`; F-6 enum order; F-7 delete the `ar` case, move to `__tests__/`; F-8 typed per-test fixture copy; F-11 one sharpening word — `hasModule('Inventory')` is the FE counterpart; F-12 pin untouched, auditor note optional). **F-9 → S4.** |

## 0AA. Round-8 addendum — S1 implementation gate r1 rulings (orchestrator, 2026-09-10)

S1 (`lane/t2-receipt-spine` @ `c30dcdf9c`, handback commit `93b106461`) was gated by the three §14 reviewers; all three returned CHANGES-REQUIRED, no BLOCKER on the stock/GL seam itself. Registers: [inventory-costing](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-t2-s1-impl-gate-r1-inventory.md:1) 0B/2M/10m · [stock-gl-interaction](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-t2-s1-impl-gate-r1-stock-gl.md:1) 0B/1M/7m · [tenancy-authz](/Users/houssamr/Projects/syneriva/apps/erp/docs/superpowers/reviews/2026-09-10-t2-s1-impl-gate-r1-tenancy.md:1) 2B/1M/5m. Fix round 1 prompt: `docs/handoff/CODEX-PROMPT-T2T3-S1-fix-round-1-2026-09-10.md`. Every §7.15 citation was VERIFIED except tenancy citation 3 (corrected below) and inventory citation 5 (freight identity on `cancelled`). All rulings are edits to this byte copy of rev 8.

| Item | Ruling |
|---|---|
| **Tenancy T-1 — `ci.yml` comment block between `php artisan test … \\` and `--filter`** (the PG job runs the whole suite unfiltered, then fails on `--filter: command not found`) | **FIX**: the comment block goes ABOVE the `php artisan test` line (§6.7.5 already says so); verify with a stubbed `php` echoing argv; record in the handback. |
| **Tenancy T-2 — three receipt tables carry parent-scoped uniques classified nowhere; `TenantOnlyUniqueOnCatalogueTablesRatchetTest` RED on PG** | **FIX**: three `EXCLUDED_TABLES` entries with the parent-scoped reasons; no waiver, no ceiling raise. §7.15 tenancy citation 3 corrected (rev 9). |
| **Tenancy T-3 — closed direction of the widened any-of read gate and the `reconciliation` gate unpinned** | **FIX** (test-only) in `TransferReceiptAuthorityGateTest`. |
| **Tenancy T-5 — service trusts callers for company scope** | **FIX**: `tenant_id`/`company_id` predicates inside `transact()` and `lockTransfer()` (or accept a scoped model). |
| **Tenancy T-4 / T-6 / T-7 / T-8** | T-4 DISCLOSE (promotion checklist: widened backend read gate has no web counterpart until S4); T-6 RECORD as intentional for S1 (S4 decides 403-details vs 404-hide); T-7 RECORD; T-8 FIX the handback command (`DB_PORT=5432`). |
| **Inventory M-1 — spec I7 fails on `cancelled`** (`freight_uncapitalized` stays 0) | **FIX**: `cancel()` sets `freight_uncapitalized = transfer_cost`; test in `StockTransferCloseTest`. |
| **Inventory M-2 — three new statuses reachable on unflagged `/receive` `/close` while the shipped web cannot name or terminate `partially_received`** | **ORCHESTRATOR DEFAULT, owner may overturn (OWNER-QUESTIONS 2026-09-10 entry): option (a)** — minimal web widening inside S1: the hand-rolled `StockTransferStatus` union in `apps/web/src/features/stock-transfers/types/index.ts` is replaced by the generated one (convention 11), three `statusVariant` rows, three `stock-transfers` i18n keys in en/fr (**rev 10: `ar` stays English-aliased — see §0AB T-9(r2)**), `canComplete` accepts `partially_received`; **plus** the T-4 disclosure. The S1 gate r2 therefore adds `frontend-conventions-reviewer` (§14 amended). Fallback if the owner wants zero web change in S1: option (d) — a validation guard refusing partial receipts (`received + damaged < remaining`) behind a flag lifted in S4, with a new `TransferReceiptFailureReason`. Option (b) is NOT a one-line change (nine of fourteen classes drive the two routes). |
| **Stock-gl G-1 — GL side of the new seam pinned only by row count** | **FIX**: six assertions (purpose→debit/credit, amount, `Posted`, `isChained()`, `JournalEntryPosted`) in both damage tests. |
| **Stock-gl G-2 — unmapped inventory/shrinkage purposes fail-soft to zero GL after stock destruction** | **ADOPT the preflight**: account-mappability check beside the valuation-mode preflight, new `TransferReceiptFailureReason`, typed 422 before any write; promotion-checklist line asserting both purposes resolve on the launch company. |
| **Stock-gl G-3 — nested caller commits the receipt with zero GL** | **ADOPT the nest guard**: `LogicException` when `DB::transactionLevel() !== 0` at the top of the receipt transaction (precedent `GeneralLedgerService`), one test. |
| **Stock-gl G-4 / G-7** | **DISCLOSE** (promotion checklist: destination-attributed shrinkage; GL-vs-subledger freight divergence with the ticket). **G-5** FIX (orphan docblock). **G-6** FIX (handback `:209-213` → "reviewer gates pending"). **G-8** RECORD. |
| **Inventory minors I-1..I-10** | I-1 FIX (named 4-dp money constant or `CurrencyScale::bcformatStrict` at persist; truncation direction stated); I-2 RECORD the decision (S4 joins on `transfer_line_id` for `quantity_decimals` — receipt DTOs stay scale-4 strings); I-3 FIX (per-site ratchet counts 2/1/1); I-4 FIX; I-5 RECORD in the handback; I-6 FIX (ledger); I-7 FIX; I-8 FIX (mark the three S2/S3 glossary rows "planned"); I-9 FIX (two assertions); I-10 n/a. |
...
|---|---|
| S1 | `inventory-costing-reviewer`, `stock-gl-interaction-reviewer`, `tenancy-authz-reviewer` — **gate r2 onward (rev 9, M-2 option a): plus `frontend-conventions-reviewer`** |
