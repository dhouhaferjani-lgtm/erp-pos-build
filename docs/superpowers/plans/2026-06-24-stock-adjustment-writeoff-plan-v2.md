...
- **A4 ✅ DONE (`94061c244`)** — owner chose "required now + update frontend". `AdjustStockRequest` requires a typed `reason_code` from the restricted manual set (`MovementReason::manualAdjustmentValues()`); rejects `CountCorrection`/document/POS reasons; optional free-text note → `reference`. Web `StockLevelsPage` dropdown now emits enum reason_codes and sends `reason_code` (en/fr i18n added) — shipped together so the live adjust call never broke. Tests: `StockMovementTest` (requires/rejects/persists) + updated `IngressPrecisionTest`; web tsc/eslint/vitest green.
