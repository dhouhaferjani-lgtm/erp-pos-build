...
>
> **SHIPPED to `origin/dev` (2026-06-24):** B0 (`d01c71d10`), A1+A1b-writeoff (`f9891f530`), A2 (`43fccac30`), A3 (`0954ef20a`) — each TDD, Pint + PHPStan L8 clean, 0 regressions (the ~9 pre-existing Inventory/BatchExpiry failures are unchanged & confirmed not ours). Note from execution: write-off GL is already **Posted** (not Draft) on dev → Codex HIGH 5 moot, re-scope B2. **Next / open:** A1b WAC+opening backfill (+ E1), **A4 (needs a breaking-API-contract decision — see Phase A)**, B1–B4, C, Phase G.
...
### Phase A — Reason persistence & direction.
- **A1 ✅ DONE (`f9891f530`)** — `?MovementReason` persisted through `recordMovement()` and the public `issue()`/`receive()`/`adjust()` contract (extended per §0.1 HIGH 1; `adjust()` uses `reasonCode` to avoid its free-text `$reason` collision). Tests: `StockMovementReasonTest`.
- **A1b ⚠️ PARTIAL** — write-off backfill **DONE** (`f9891f530`, `BatchWriteOffService` → Expiry/Damage/WriteOff). **Still TODO:** `WeightedAverageCostService` (`:254-274,:550-567,:737-755`) and `InventoryOpeningService` (`:279-292` → `OpeningBalance`, folds into **E1**). **Do NOT add a NOT NULL / CHECK** (A5 dropped).
- **A2 ✅ DONE (`43fccac30`)** — `MovementReason::CountCorrection` added (+ `label()`/`requiresGLEntry()`/TS union per §0.1 HIGH 2) and set in the counting listener; `COUNTING:` reference kept. Tests: `CountingReasonTest`.
- **A3 ✅ DONE (`0954ef20a`)** — `StockMovement::directionForRow(): 'in'|'out'|'flat'` from the signed `quantity_before→after` delta; handles `'flat'` (WAC cost-adjust). Tests: `StockMovementDirectionTest`.
- **A4 ⏸️ NEEDS A CONTRACT DECISION before building.** Plan: an `AdjustStockRequest` requiring a typed reason from the restricted manual set (`AdjustmentPositive/Negative, Damage, WriteOff, OpeningBalance`), rejecting `CountCorrection`/document/POS reasons, keeping absolute `new_quantity`. **But** the live `POST /stock-movements/adjust` (`StockMovementController::adjust`, inline-validated) currently takes a **free-text** `reason` (`max:500` → `reference`). Adding a *required typed* `reason_code` is a **breaking API change** for the web-admin adjust call — needs a frontend change + a decision: (a) add required `reason_code` + keep `reason` as optional note (breaking, cleanest); (b) phase in (nullable `reason_code` first, enforce later); (c) infer the enum from existing free-text (lossy). Decide before implementing.
