<!-- W-LOT-A-1a rev 7, Codex CLI fix round 6 (gpt-5.6-sol, workspace-write, in-place edits on a byte copy of rev 6) 2026-09-09. Rev 6 = 350da33a1 is the verbatim base. Status: awaiting gate r7. -->
<!-- W-LOT-A-1a rev 7; Tasks 1/2/6 only. Audited at c0ec5f90253c72cfe6aad715be05ad6f11208976 on 2026-09-09. -->
# Slice plan W-LOT-A-1a — lot action permissions, general-manager role delta, web gating (rev 7)

## 0. Round-6 change log
...
- Planning HEAD read in full: `c0ec5f90253c72cfe6aad715be05ad6f11208976`.
- Branch state: local `dev`.
- Commit: `docs(parapharmacy): Codex gate r6 on slice plan W-LOT-A-1a rev 6 — CHANGES-REQUIRED (B-B3b/N-B/N-C closed; open: B-B2 flag-off boot seeding must be non-destructive for marked tenants, B-B3a registration/disposable producers + explicit fail branches, M-E deployment correlation, M-F inventory) + U-1 fact sheet: observed Dokploy deployment record shape (description = 'Commit: <sha>')`.
- Plan preparation at the planning pin changed no production file, ran no test, and ran no git command; rev 7 is an in-place documentation edit over the rev-6 byte copy.
...
- Rev 6 is the verbatim base for this round-6 correction.
...
assert_preserved_post_delta_snapshot() {
  ...
  pg="$(resolve_container 'erp-staging-postgres-8x7pbx')"
  test -s "$manifest"; : > "$actual"
  while IFS= read -r tenant_id; do
    docker exec "$pg" psql -X -A -t -U autoerp -d "tenant_${tenant_id}" -v tenant_id="$tenant_id" -c "
...
for attempt in $(seq 1 120); do
  ...
  jq -e --arg description "Commit: ${PUSH5_SHA}" --arg pushed_at "$PUSH5_PUSHED_AT" '
    ...
    [.deployments[] | select(.description == $description and ((.createdAt | epoch) > ($pushed_at | epoch)))]
...
scp "$AUTO_SELECTED" "$AUTO_ACTUAL" "$AUTO_EXPECTED" "$AUTO_TEMPLATE" "$WORK/web-push5-started-at.txt" \
...
assert_preserved_post_delta_snapshot "$MANIFEST" 'p5-preserved-post-delta'
PUSH5_INERTNESS
for attempt in $(seq 1 90); do
...
for attempt in $(seq 1 120); do
  ...
  jq -e --arg description "Commit: ${PUSH5_SHA}" --arg started_at "$FALLBACK_STARTED_AT" '
    def epoch: sub("\\.[0-9]+Z$"; "Z") | fromdateiso8601;
    [.deployments[] | select(.description == $description and ((.createdAt | epoch) > ($started_at | epoch)))]
...
scp "$FALLBACK_SELECTED" "$FALLBACK_ACTUAL" "$FALLBACK_EXPECTED" "$WORK/fallback-deployment.template.tsv" \
  "$BEFORE" "$AFTER" "$WORK/fallback-fingerprint.json" \
  "$STAGING_HOST:/root/wlota1a/evidence/"
```
...
ROOT='/root/wlota1a'; source "$ROOT/helpers/host-helpers.sh"
MANIFEST="$ROOT/manifests/p4/expected-tenants.txt"; test -s "$MANIFEST"
capture_permission_snapshot "$MANIFEST" 'registration-pause.before'
...
ROOT='/root/wlota1a'; source "$ROOT/helpers/host-helpers.sh"
MANIFEST="$ROOT/manifests/p4/expected-tenants.txt"; test -s "$MANIFEST"
capture_permission_snapshot "$MANIFEST" 'registration-pause.after'
...
assert_preserved_post_delta_snapshot "$MANIFEST" 'registration-pause-preserved-post-delta'
API="$(resolve_container 'erp-staging-api-kghqex')"
...
scp "$WORK/after.json" "$WORK/validation.json" "$STAGING_HOST:/root/wlota1a/evidence/"
```
...
assert_preserved_post_delta_snapshot "$MANIFEST" 'rollback-preserved-post-delta'
...
27. Execute the exact three-application OFF aggregate `cmp`; create the disposable legacy tenant with the committed `curl`/JSON/`jq` producer; persist, transfer, and compare its one UUID; then execute its legacy marker `cmp`.
...
39. Set `CANDIDATE_SHA=PUSH5_SHA`; correlate the unordered automatic web records on `description="Commit: ${PUSH5_SHA}"` and `createdAt` after the push timestamp, pin that `deploymentId`, poll only it until `status=done`, compare `(deploymentId, description, status)` with the committed template, then require changed served asset, served feature string, exact build SHA, and feature fingerprint. Do not manually redeploy on the normal path.
40. Run only the pre-activation browser test.
41. Pause registration with the reversible cache-backed Laravel maintenance-mode/Dokploy API-app env procedure and prove ordinary registration returns 503.
...
47. Use the maintenance-secret cookie to execute the committed registration `curl`, validate the host response with `jq`, persist/transfer/compare exactly one UUID, capture every structured stderr `WLOTA1A-RESEED` marker from the log window, and `cmp` the initial activated `APPLIED` record against the committed manifest template.
...
51. Run the exact configuration-cache, permission-cache, and Horizon command/exit/heredoc-`cmp` gates; execute `artisan up`, prove registration reaches 422 validation, restore only the saved maintenance env lines through Dokploy, redeploy, and prove registration still reaches validation.
...
- [ ] Planning SHA `c0ec5f90253c72cfe6aad715be05ad6f11208976` and its commit subject are recorded.
- [ ] Actual `DISPATCH_SHA` is recorded and named seams are re-censused after drift.
- [ ] Only Tasks 1, 2, and 6 are implemented.
- [ ] The round-6 gate and amended resolved U-1 fact sheet are read in full and every item is CLOSED.
...
- [ ] Push 5 correlates the unordered web `deployments[]` array on `description="Commit: ${PUSH5_SHA}"` plus `createdAt` after the push, pins that `deploymentId`, polls it to `status=done`, and compares `(deploymentId, description, status)` with a committed template before asset/fingerprint checks; the normal path performs no manual web redeploy.
- [ ] The stale-edge incident fallback first revalidates that same automatic correlation and applies the same description/createdAt/deploymentId selection to its one permitted manual redeploy.
...
- [ ] Registration pause uses the reversible cache-backed maintenance-mode env/redeploy procedure; ordinary registration is 503 while paused, the operator request uses the secret bypass cookie, and resume/restoration returns ordinary requests to 422 validation.
