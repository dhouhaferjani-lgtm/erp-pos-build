...
2. **DueThisWeekWidget** ← §3 (`MaturingInstrumentsController.php:102`)
   ...
   - Route target (deep-link `to`): `/treasury/instruments?maturity_from=<today>&maturity_to=<today+7d>` — the existing instrument list route (`apps/web/src/routes/index.tsx:1741,1746`; `/treasury/maturing-instruments` is an API endpoint, not a page route). `maturity_from`/`maturity_to` are the params `InstrumentListPage.tsx:113` actually reads; location scope needs no param (the page consumes the global view scope after §3 Task 9). The widget test asserts `to` equals this parameterized URL (route + both maturity params), not the bare route.
