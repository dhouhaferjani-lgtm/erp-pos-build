...
Retirement acceptance: remove the `T1_RUN_KNOWN_REDS` skip from `ReplenishmentEdgeCasesTest::test_replayed_initiated_event_must_not_settle_demand_captured_after_dispatch` when this ticket lands, run that method on PostgreSQL, and update or retire the current companion `test_small_ordinary_transfer_pins_current_behaviour_until_ticket_t1_11` in the same change; if the owner chooses a different policy, encode that approved assertion instead of retaining a permanent skip.

Stock/GL consequence: replay double-settles **demand only**. `SettleRequestsOnTransferInitiated::settleLine` writes only `replenishment_requests`; `ReplenishmentFulfilled` has no registered listener. There is no second stock decrement, journal entry or cash effect. Do not compensate this demand-only defect with stock or GL writes. `handle()` catches and logs per-line failures (`apps/api/app/Modules/Replenishment/Application/Listeners/SettleRequestsOnTransferInitiated.php:33-40`) without surfacing them to the caller, so a partial settlement failure leaves stock moved and requests open silently from the operator’s perspective.
