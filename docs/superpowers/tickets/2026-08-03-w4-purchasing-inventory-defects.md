...
| # | Finding | Pinned by |
|---|---|---|
| R-10 | The counting apply's **`negative_at_apply`** flag arm is **NOT EXERCISED** by `MTP-INV-18`'s construction — `applyReplay()` runs the basket-window pre-check first (`ApplyStockAdjustmentsOnCountingCompleted.php:224-230`) and it matches, so the line is held there. **It is reachable, not structurally shadowed** (corrected after review N-1; the first draft of this row overclaimed). `MovementReplayService::hasMovementNear()` is a symmetric ±window at **second** granularity (`:194-200`, `boundary():232-234`) around `final_qty_as_of` (the count-submission instant), and `ambiguity_window_minutes` accepts `min:0` — so a window-0 counting whose consuming movement lands **more than one second** after the count submission should fall outside the basket window and reach `negative_at_apply`. The window-0 attempt made during authoring landed inside the same second, which is why it still reported `basket_window`. **Closable in a follow-up** (window 0 + explicit spacing); not authored this round. Both arms carry the same money contract (nothing posted, line flagged for review, stock never negative). | `MTP-INV-18` (PARTIAL) |
