...
- **TransactionCart panel restyle — STARTED** (973aa91ae, 3b61886d7; +2 ahead of origin/dev, NOT yet promoted): quick-actions toolbar decluttered to the mock §5.1 THREE labelled actions (Remise/En attente/Reprendre) + "Reprendre+count" badge; Returns/Exchange demoted to a compact icon next to Clear (was a 4th text button truncating "Retour/Échange"); mono typography on summary amounts + refund net-total. New QuickActions.test.tsx (5) + TransactionCart Returns-icon test. Review: `docs/superpowers/audits/2026-06-28-pos-cart-panel-restyle-review.md`.

NEXT on resume:
- **PaymentSummary mock §5.1 breakdown — NEEDS A DECISION (deferred, not done):** split the single discount line into **Remises produits** (sum of line discounts) vs **Remise panier** (transaction discount) — needs HomePage to plumb the two values separately; and the **"dont TVA 19%"** framing — touches HT/TTC fiscal-display semantics + mixed VAT rates (current generic "TVA" line is safe). Don't change money display unilaterally; confirm wording/rate handling with owner.
- Per tracker: P4 payments (`CashPaymentScreen` full-screen, `AdvancedPaymentsModal` 916L untokenized — HIGH effort), P5 per-modal restyles (+ exit transitions on the Modal shell + `Drawer`/`Toast` shells), P7 reports restyle (tokenize `TodaySalesPanel` + `ReportsMenu`).
