...
**Created:** 2026-05-14
**Last refreshed:** 2026-05-16 — Tasks 7–15 shipped + dual-reviewed; Task 5 deferred P2s closed; the schema-destructive gate is OPEN via owner attestation; branch HEAD `b888aa9e` on `feat/pos-fiscal-event-engine-phase1` (PR #124).
...
### 4.1 Tasks shipped (1–15 inclusive)
...
### 4.2 Per-task ground rules (carried forward from Tasks 7–15)

1. **Per-task TDD + dual review is non-negotiable.** Codex has now caught BLOCKERs Opus missed on **four** tasks: Task 8 parse-failure resolution bypass, Task 8 integrity_exception_class write-once, Task 14 silent-key-missing + float-leak in `fromArray()`, **and Task 15 opaque-payload + envelope-format gaps** (Opus APPROVE-WITH-MINOR-EDITS → Codex BLOCK on the same commit). On Task 13 both reviewers converged on the same hash-format GLOB BLOCKER independently. The Codex-finds-what-Opus-misses pattern is consistent enough that the dual gate is a hard requirement — never skip the Codex pass even when Opus is happy. **Patterns to treat as code smells:** `(type) $array['key']` PHP casts (Task 14 BLOCKER), `payload: unknown` TS seams without runtime validation (Task 15 BLOCKER), free-form string fields without regex format validation (Task 15 P1-1).
...
### 4.3 Open items going into Task 16
...
- **Task 16 inputs carried forward:**
  - Sub-array shape validation (per-line monetary fields in `SaleReceiptPayload.lines[i]`, `vat_breakdown[i]`, `payment_lines[i]`, `voucher_redemptions[i]`, plus the analogous sub-arrays on the chain-recovery + terminal-registry payloads) is the `StrictCanonicalParser`'s job. The DTO guards reject non-array values for the sub-array slots; per-item shape stays the parser's job. (Carried from Task 14 Opus P2-2.)
  - The `FiscalEventCanonicalEncoder.encode()` output is the verified canonical bytes; the server-side parser re-hashes the verbatim bytes and parses them with strict-mode JSON + shape assertions. The parser MUST match the engine's payload-validation contract; field-set drift surfaces as `canonical_parse_failure`.
...
- **Task 16 next.** `StrictCanonicalParser` (server PHP) — reads `canonical_bytes` verbatim, parses with strict JSON mode, asserts the §4 fourteen-field shape, looks up the payload DTO via `FiscalEventPayloadRegistry::dtoClassFor()`, constructs the DTO via `fromArray()` (which already enforces the top-level type contract from Task 14 + the new typed-input contract from Task 15), then validates the sub-array shapes. Plan §1166+. Inline implementation (small enough). Dual review as usual.
- **Subagent strategy:** Tasks 15/19/21/22/27/28/29/30 delegate to Opus subagents. Task 15 was the first delegation (round-1 took the subagent ~6 min; round-2 reconciliation was inline due to the Codex BLOCKER). Tasks 16-18 inline; 19 is the next delegation.
