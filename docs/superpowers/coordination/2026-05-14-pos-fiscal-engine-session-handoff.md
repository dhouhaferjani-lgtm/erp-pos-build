...
**Created:** 2026-05-14
**Last refreshed:** 2026-05-16 — Tasks 7–17 shipped + dual-reviewed; Task 17 was the first single-shot dual APPROVE (Codex BLOCKER-streak paused at 5); Task 5 deferred P2s closed; the schema-destructive gate is OPEN via owner attestation; branch HEAD `feat/pos-fiscal-event-engine-phase1` (PR #124) at the latest Task 17 round-2 + handoff push.
...
### 4.1 Tasks shipped (1–17 inclusive)
...
### 4.3 Open items going into Task 18
...
- **Task 17 standing pattern (NEW):** **PHP-side defensive contracts must wrap `Eloquent::find($uuid)` in `try/catch (QueryException)`** when the docblock pins "never throws". On PostgreSQL the `uuid`-typed primary-key column rejects malformed-UUID strings with a driver-level `QueryException`. SQLite local runners don't reproduce this — but production does. Pattern: catch + return the documented "missing" sentinel. (Opus P3-3 round-2 closure.)
- **Task 18 next.** `FiscalEventProjector` interface + `FiscalEventProjectionRegistry` (server PHP). The interface defines `name()`, `handlesEventType(FiscalEventType)`, `requiresModule(): ?string`, `apply(FiscalEvent)`. The registry is constructed over `iterable<FiscalEventProjector>` (tagged via the container) + `ModuleActivationResolver` (Task 17). `activeProjectorsFor($event)` filters by `handlesEventType()` then gates by `resolver->isActive(requiresModule(), tenant, company)`. Empty tagged set is valid in this commit — Tasks 21/22 add the real projectors. Plan §1284+. Inline. Dual review as usual.
...
**End of handoff (refreshed 2026-05-16 after Task 17 round-2 — `ModuleActivationResolver` shipped + dual-reviewed; first single-shot dual APPROVE; Codex BLOCKER-streak paused at 5; next: Task 18 — `FiscalEventProjector` interface + `FiscalEventProjectionRegistry`).**
