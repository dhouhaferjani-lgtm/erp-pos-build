...
**Created:** 2026-05-14
**Last refreshed:** 2026-05-16 — Tasks 7–14 shipped + dual-reviewed; Task 5 deferred P2s closed; the schema-destructive gate is OPEN via owner attestation; branch HEAD `023f4a93` on `feat/pos-fiscal-event-engine-phase1` (PR #124).
...
**Phase 1 implementation is in progress on branch `feat/pos-fiscal-event-engine-phase1` (PR #124).** Branch HEAD `023f4a93` as of 2026-05-16. Owner sign-off on the schema-destructive preflight gate is OPEN (`apps/api/docs/sessions/2026-05-14-fiscal-preflight-signoff.md` — attested 2026-05-15).

### 4.1 Tasks shipped (1–14 inclusive)
...
### 4.2 Per-task ground rules (carried forward from Tasks 7–14)

1. **Per-task TDD + dual review is non-negotiable.** Codex has now caught BLOCKERs Opus missed on **three** tasks: Task 8 parse-failure resolution bypass, Task 8 integrity_exception_class write-once, **Task 14 silent-key-missing + float-leak in `fromArray()`** (Opus APPROVE → Codex BLOCK on the same commit). On Task 13 both reviewers converged on the same hash-format GLOB BLOCKER independently. The Codex-finds-what-Opus-misses pattern is consistent enough that the dual gate is a hard requirement — never skip the Codex pass even when Opus is happy. **Treat `(type) $array['key']` casts as a code smell**: they silently coerce missing keys + wrong types and were the BLOCKER mechanism on Task 14.
...
### 4.3 Open items going into Task 15

- **Task 14 SHIPPED + reconciled at `023f4a93`** with the new `FiscalPayloadArrayGuards` helper as the standing pattern for DTO `fromArray()`-style construction. Any further DTO that comes through the canonical payload boundary must use the same `requireString` / `requireInt` / `requireArray` / `optionalString` guards. The `(type) $data['key']` cast pattern is treated as a code smell going forward.
- **Forward-looking input from Task 13 Codex P3 (carried):** `terminal_state` chain-head columns ship with empty-string sentinels (`fiscal_event_genesis_seed = ''`, `fiscal_event_last_hash = ''`, `fiscal_event_sequence = 0`). Task 15's `FiscalEventEngine.append()` boot path **must** treat these as "not yet initialized" — either issue an explicit seed-init transaction or refuse to append until a valid genesis seed is provisioned. The migration cannot enforce this; track at Task 15.
- **Task 16 input (carried as Opus P2-2 from Task 14):** sub-array shape validation inside the DTO `fromArray()` (e.g. `lines[i].unit_price` is a string, `vat_breakdown[i].rate` is a string, etc.) is deferred to the server-side `StrictCanonicalParser`. The DTO guards reject non-array values for sub-array slots; per-item shape stays the parser's job.
- **Task 15 next — first Opus-subagent delegation.** `FiscalEventEngine.append()` (device TypeScript). Plan §1075+, spec §6.1. Runs inside the caller's SQLite transaction; resolves the payload DTO + event_version via the new `FiscalEventPayloadRegistry`; reads the chain head from `terminal_state`; builds canonical_bytes; inserts `fiscal_events`; advances the chain head. **Must defend** against the empty-string genesis-seed sentinel from Task 13 Codex P3. **Idempotency** on `(source_event_class, source_event_id)`.
- **Subagent strategy:** Tasks 15/19/21/22/27/28/29/30 delegate to Opus subagents per the original brief. Each delegation gets a self-contained prompt referencing the plan section + spec sections + relevant existing files; the dual-review gate (claude + codex:codex-rescue) runs after each subagent commit, same as the inline tasks.
