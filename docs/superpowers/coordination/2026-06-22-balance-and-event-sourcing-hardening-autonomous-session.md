...
- 2026-06-22 — M-4 DONE: partner balance refresh now runs from `JournalEntryPosted` for normal posted GL entries, and draft-only GL builders no longer refresh cached balances. Added `RefreshPartnerBalanceOnJournalEntryPosted`, removed redundant direct refresh from post helpers and draft builders, and wrapped production invoice/credit-note posted GL creation in one transaction so refresh failure rolls back journal header/lines/hash/audit work. POS account charge keeps its direct refresh because it is transactionally equivalent and already rollback-covered. Red was observed first for posted partner journal entries leaving cached receivable at `0.000`, and for invoice GL refresh failure leaving a posted journal entry behind. Verification passed: M-4 GL regression filter (2 tests, 4 assertions), invoice/credit-note rollback filter (2 tests, 8 assertions), `GLIntegrationTest` (19 tests, 79 assertions), invoice/credit-note/document posting suites (46 tests, 253 assertions), POS account charge suite (9 tests, 72 assertions), fiscal treasury account charge bridge suite (11 tests, 47 assertions), `PaymentTest` (16 tests, 49 assertions), `PartnerBalanceServiceTest` (18 tests, 35 assertions; pre-existing PHPUnit 12 doc-comment warnings), PHPStan L8 on changed app files, Pint, and `git diff --check`. Codex review clean; fallback second review clean; `opus-review: PENDING`.
- 2026-06-22 — M-5 DONE: cached partner liability magnitudes now have domain and PostgreSQL non-negative guardrails. Added a tenant migration that clears stale negative `credit_balance`/`payable_balance` caches to zero, marks them stale via `balance_updated_at = NULL`, and adds named PG CHECK constraints; added a `Partner` saving guard for negative cached liability magnitudes; and extended partner money precision coverage with domain and direct-DB constraint assertions. Red was observed first for negative cached liability values saving through the model. Verification passed: default `PartnerMoneyPrecisionTest` (3 tests, 4 PG-only skips, 8 assertions), real PostgreSQL `PartnerMoneyPrecisionTest` against isolated `autoerp_m5_test` (7 tests, 22 assertions), `PartnerBalanceServiceTest` (18 tests, 35 assertions; pre-existing PHPUnit 12 doc-comment warnings), adjacent partner create/update/list/balance/deposit/B2B suites (70 tests, 275 assertions), adjacent accounting/POS suite (28 tests, 151 assertions), PHPStan L8 on `Partner`, Pint, and `git diff --check`. Codex review clean; fallback second review clean; `opus-review: PENDING`.
- 2026-06-22 — M-6 DONE: added a scheduled subledger reconciliation alert command. `accounting:check-subledger-reconciliation` iterates tenants, scans active tenant companies for customer receivable/customer advance/supplier payable discrepancies, logs structured alert context, prints discrepancy lines including `entries_without_partner`, returns failure when discrepancies exist, and performs no automatic repair. Registered it through `AccountingServiceProvider` and scheduled it daily at 02:30 with overlap protection. Red was observed first as a missing command. Verification passed: `SubledgerReconciliationCommandTest` (2 tests, 6 assertions), `PartnerBalanceServiceTest` (18 tests, 35 assertions; pre-existing PHPUnit 12 doc-comment warnings), `GLIntegrationTest` (19 tests, 79 assertions), PHPStan L8 on changed app/scheduler files, Pint, and `git diff --check`. Codex review clean; fallback second review clean; `opus-review: PENDING`.
- 2026-06-22 — M-7 DONE: both-type partner net balance now includes supplier payable exposure consistently across backend model accessor, partner index SQL sorting, and frontend display helper. The explicit convention is customer position `receivable_balance - credit_balance`, supplier position `payable_balance`, and both-type net exposure `receivable_balance - credit_balance - payable_balance` where positive means the partner owes the company. Red was observed first for the model accessor returning `900.000` instead of `650.000`. Verification passed: focused model test (1 test, 1 assertion), focused partner list sort test (1 test, 3 assertions), full adjacent partner entity/list balance tests (22 tests, 78 assertions), frontend partner test file (42 tests; pre-existing localstorage and `/partners/1` route warnings), PHPStan L8 on changed backend app files, web typecheck, scoped frontend ESLint with 0 errors and existing warnings, Pint, and `git diff --check`. Codex review clean; fallback second review clean; `opus-review: PENDING`.
