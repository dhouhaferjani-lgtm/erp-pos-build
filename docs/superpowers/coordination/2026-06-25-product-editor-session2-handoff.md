...
## Feedback round 1 (owner, 2026-06-26 — after live :8089 review)
- **DONE `133ddbc95`** — status fields (`is_active`, `is_active_for_ecommerce`) converted to Toggle switches; "Actif" relabeled **"Produit actif"/"Active product"**; the two toggles grouped/aligned. (Owner: checkboxes felt inconsistent with the batch switch; "Actif" was ambiguous.)
- **TODO — Editable linked margin** — brief `.superpowers/sdd/task-editable-margin-brief.md`. Make "Marge" an editable input two-way-linked to sale price. **Margin model = MARKUP on HT cost** (matches the existing `MarginService`: `price = cost×(1+m/100)`, `m=(sell−cost)/cost×100`), seeded from **`company.default_target_margin` (default 30%)**, basis = `purchase_price`, bridging HT↔TTC via the selected tax rate. Has backend surface: expose `default_target_margin` to the FE if not already; persist `target_margin_override` (add validation). SUBSTANTIAL — own session.
- **TODO — Create-mode image upload** — brief `.superpowers/sdd/task-createmode-images-brief.md`. Buffer files picked during create, auto-upload after first save via the `productImages` façade; edit-mode unchanged. SUBSTANTIAL — own session.
- **Demo-data gap (not editor code):** `GET /api/v1/uom/units` returns **403** for the Cafe Tunis demo user → unit dropdown empty. Grant `uom.view` to that role/tenant before any demo.
