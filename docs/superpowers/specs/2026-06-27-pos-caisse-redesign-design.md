...
2. **Two purposeful colours: `action` (blue, CTA) + `accent` (swappable, highlight).** **Product decision 2026-06-28: the primary CTA stays ocean-blue** ("for now"). So:
   - **`action`** = theme-aware ocean-blue (raw `--action*` vars, light + dark), drives **commit/CTA + interactive** (Encaisser, Valider, primary `Button`). White-on-`#1A6FB5` clears WCAG AA (also resolves the earlier accent-contrast concern).
   - **`accent`** = swappable `--accent*` tokens via **`data-accent`** = `orange`(default)|`green`|`blue`|`teal` (README §6), used for **selected/highlight** chrome only: active nav-rail item, selected category pills, in-cart accent treatment, merchandising highlights. NOT wired to the CTA. The Appearance accent picker still themes all of these live.
   - This is a coherent retail split: blue = "do/commit", accent = "chosen/highlighted". `Pill selected`, nav-active, in-cart use `accent`.
   - **Rogue `primary-*`/`blue-*` refs** (~63, e.g. `AdvancedPaymentsModal`, `HomePage` status pills): since `action` is now blue too, the split-brain risk is gone for primary chrome; still sweep these → `action`/`accent`/`surface` tokens per phase for dark-mode correctness, and add the file to ESLint `tokenMigratedGlobs`. `--color-primary-*` stays the blue ramp (header chrome). `brand` (copper) retired in favour of the IziPOS mark.
...
- **Cart-line collapse/expand [USE MOCK — product decision 2026-06-28].** Today steppers/discount/delete are always visible (nothing increments on row tap; qty tap opens numpad via `onQuantityTap`). **Decision: adopt the mock's pattern** — collapsed by default (avatar, brand, name, ×qty, line total, remove), **tap the row body to expand** and reveal the qty stepper, per-line discount, and fiche shortcut. Rationale (owner): a collapsed line is shorter, so **more cart items fit on screen**, and editing is still one tap away. Tapping the row toggles expand (does NOT increment). Preserve `onQuantityTap` (numpad) inside the expanded state. Only ONE line expanded at a time (accordion) to keep the cart compact. Add a behavior test (no current `CartLineItem` test). The 48px touch floor applies to the expanded controls; the collapsed row stays a comfortable tap height.
