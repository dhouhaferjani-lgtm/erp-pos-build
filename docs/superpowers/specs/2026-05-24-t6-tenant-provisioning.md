...
**Date:** 2026-05-24 (v7 — r7 Codex auth-review fixes applied: tenant-qualified verify/reset/invitation links [B1], central Sanctum PAT model [B2], identity-index full user lifecycle [P1], grep-based route coverage [P2], TenantInitializationService "extended-not-untouched" [P1])
...
- [ ] **Identity index full lifecycle (r7 P1):** `IdentityIndexService` writes `central_identities` from register **and** `UserController::store` (invited) / email-change / `destroy`; PG tests cover invited-user email-first login and deactivation.
