# POS Phase 1 — Foundation: Fiscal Event Engine + Receipt-Chain Clean Rebuild (v6)

**Date:** 2026-05-14
**Phase:** 1 of 5 (roadmap v2)
**Status:** Drafted. v1 → Codex BLOCK (3B/3P1/3P2); v2 → Codex BLOCK (2B/1P1/1P2); v3 → Codex BLOCK (1B/3P1/2P2); v4 → Opus APPROVE-WITH-MINOR-EDITS (1P1/2P2/2P3); v5 → Codex BLOCK (1B/0P1/0P2/1P3); **v6 resolves both v5 findings.** Awaiting Codex re-review → writing-plans.
**Supersedes:** v5 (`2026-05-14-pos-phase1-foundation-spec-v5.md`), v4, v3, v2, v1.
...
`[Codex v3 + Codex v5 — the server-authoring `SALE_RECEIPT` path is live, reached by both the browser web POS and the Tauri-online checkout branch]`. A server-recompute *new-sale* `SALE_RECEIPT` authoring path cannot coexist with the rebuilt device-authority `SALE_RECEIPT` chain `[SoT D8]`. There are **two client callers** of that path and **one backend surface**:
- the **browser web POS** — disabled here (it cannot device-author/seal; full device-authority parity is deferred, §18);
...
- the **backend new-sale-authoring routes** — `POST /pos/receipts` + `POST /pos/receipts/{id}/payments` — retired/rejected for new-sale `SALE_RECEIPT` authoring for **all** callers.
...
**Tauri client (`apps/pos`):** reworked per the §14 table — `offlineCheckoutService.ts` (`onlineCheckout` branch discarded; `executeCheckout` always authors locally) and `apps/pos/src/api/receiptApi.ts` (`createReceipt` / `processReceiptPayments` discarded for new-sale authoring). Listed here so the cross-app surface is complete in one place.
