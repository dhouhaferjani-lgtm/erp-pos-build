...
## T-1 regression retirement acceptance (fix round 1, 2026-09-09)

When a linked T-1 ticket is resolved, the same change must remove its `T1_RUN_KNOWN_REDS` skip guard, run the exact method below on PostgreSQL, and update or retire its named current-behaviour companion. Do not close a ticket while its desired-behaviour test is still skipped. The skip messages retain the complete ticket path. If an owner ruling changes the desired outcome, encode that ruling in the assertion rather than leaving a permanent skip.

| Ticket | Desired-behaviour method to enable | Current-behaviour companion to update |
|---|---|---|
| [T1-4](../tickets/2026-09-09-t1-recalled-in-transit.md) | `StockTransferEdgeCasesTest::test_recalled_after_dispatch_must_not_silently_land_at_destination` | `test_recalled_lot_receipt_pins_current_behaviour_until_ticket_t1_4`; review `test_expired_lot_receipt_pins_current_behaviour_until_ticket_t1_4` against the expiry policy |
| [T1-10](../tickets/2026-09-09-t1-idempotency-payload.md) | `StockTransferEdgeCasesTest::test_idempotency_key_replays_identical_payload_but_refuses_changed_quantity` | Identical-retry assertions in the same method must stay green |
| [T1-11](../tickets/2026-09-09-t1-settlement-replay.md) | `ReplenishmentEdgeCasesTest::test_replayed_initiated_event_must_not_settle_demand_captured_after_dispatch` | `test_small_ordinary_transfer_pins_current_behaviour_until_ticket_t1_11`; normal capture/bump test must remain green |
| [T1-14](../tickets/2026-09-09-t1-cancel-in-progress.md) | `ReplenishmentEdgeCasesTest::test_requester_can_cancel_in_progress_request_while_status_is_open` | `test_requester_cancel_in_progress_pins_current_behaviour_until_ticket_t1_14` |
