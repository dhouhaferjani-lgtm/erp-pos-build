...
- **Date:** 2026-08-08
- **Status:** Rev 7 — post Codex round 6 (REJECT: R6-01..R6-04). §4.2.1 is now the **complete 41-case partition**: 27 REQUIRED (voucher five + tolerance two + `PurchaseExpenses` promoted — all costless, no new fixture deltas), 1 SCOPE-REQUIRED (`SalesStampDutyPayable`, timbre countries), 2 CONDITIONAL (refund pair, verified `DOMAIN_PRECHECK_4XX`), 11 SOFT (`UninvoicedRevenue` proven dead — zero callers). `ALERT_AND_SKIP` gate kind deleted (its only member was tolerance, whose alert path is not fail-safe — now REQUIRED + hardening ticket). Pending Codex round 7 + owner sign-off. Register: `reviews/2026-08-08-country-defaults-spec-review.md`.
...
1. **Operational purpose set (F-02/R2-02):** the versioned manifest `ProvisioningRequiredPurposes::V1` is **enumerated here as the complete 41-case partition** (R6-03; from the 2026-08-08 code inventory of every throwing purpose resolution: `Account::findByPurposeOrFail`, the private wrapper `GeneralLedgerService::getAccountByPurpose()` (`GeneralLedgerService.php:4582-4584`) and its dynamic call sites, and `AccountingService::findAccountByPurpose` on the live invoice-posting path via `InvoicePostedListener.php:30-32`). The manifest class is the source of truth once committed; a conformance test asserts the spec table and the class agree (R6-04). **REQUIRED cases (27)** — reachable throwing resolution in ordinary operations with no allowed gate:
   ...
   **SCOPE-REQUIRED (1):** `SalesStampDutyPayable` — required only when the certified scope includes a timbre country (TN today): the throwing site fires only for credit notes actually carrying stamp duty (`GLS:291` inside `if ($hasStampDuty)`), and stamp-duty lines exist only where country tax configuration defines timbre. Publish validates it per scope exactly like protected codes (§4.2.4).
   **CONDITIONAL (2):** `SalesReturn`, `RefundWriteOff` — gate kind `DOMAIN_PRECHECK_4XX`: refund compensation prechecks both and raises `RefundCompensationRefusedException` rendered 422 (`RefundCompensationService.php:184-193`; `bootstrap/app.php:487-496`; Codex-verified round 5/6).
   **SOFT (11):** `OfficeExpense`, `TravelExpense`, `MealsExpense`, `UtilitiesExpense`, `RetainedEarnings`, `RealizedFxGain`, `RealizedFxLoss`, `VoucherBreakageIncome` (no Expired arm wired), `SalesRoundingDifferenceIncome`, `SalesRoundingDifferenceExpense` (nullable-tolerated absorbers, `AS:223-225`), and `UninvoicedRevenue` — **`UninvoicedDeliveryNoteService` has zero callers in the codebase (verified 2026-08-08): dead/unwired compliance code**, so the purpose is unreachable; flagged as a cleanup note.
   27 + 1 + 2 + 11 = 41.
...
5. **Dead-code cleanup note:** `Compliance/Services/UninvoicedDeliveryNoteService` has zero callers (verified 2026-08-08) — wire it or remove it.
