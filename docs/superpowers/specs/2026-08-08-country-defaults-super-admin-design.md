...
- **Date:** 2026-08-08
- **Status:** Rev 12 — post Codex round 11 (REJECT: R11-01 + R11-02 only; full round-1..10 sweep found no other open item). R11-01 closed: registry-bump blast radius made honestly **GLOBAL** — activation blocks ALL assignments (incl. unchanged countries and `*`) until every one is re-pointed; transition tests cover the unchanged-country and wildcard block. R11-02 closed: SalesDiscount consistently described as a dead-lettered treasury GL projection (spec §5.4 + handover sequencing), with the replay obligation (`fiscal:retry-projections`) stated where C is scheduled. Pending Codex round 12 + owner sign-off. Register: `reviews/2026-08-08-country-defaults-spec-review.md`.
