...
- **Date:** 2026-08-08
- **Status:** Rev 13 — **Codex round 12: ACCEPT-WITH-CHANGES** (58 prior finding IDs resolved; the single remaining P2, R12-01, is applied in this revision: §6 drift scan = every current assignment row incl. `*`, superseded unassigned versions don't block, registry-bump verification test specified). Round-13 confirmation + owner sign-off pending; then → writing-plans. Register: `reviews/2026-08-08-country-defaults-spec-review.md` (12 rounds).
...
| capability_registry_version | string nullable | **set at publish, immutable** (R8-03/R9-01): the `CountryAccountingCapabilities` version the certification was decided under. **Version drift blocks PROVISIONING, not just new assignments:** the resolver (§5.1 step 2b) refuses any assignment whose template's stored version differs from the current registry version, throwing a typed `TemplateRecertificationRequiredException`; `country-defaults:verify` FAILS (non-zero) on drift. **Registry-bump transition protocol (R10-01/R11-01 — deliberately fail-closed, no staged-version machinery, GLOBAL blast radius):** the scalar version equality means activating a new registry version immediately blocks NEW-company provisioning for **every assignment — all exact countries AND the wildcard, including countries whose capabilities did not change** (typed exception; existing companies untouched per owner ruling). The runbook must therefore clone → recertify (publish records the NOW-current version) → re-point **every assigned template**, not a selected subset; provisioning is fully restored only when the last assignment is re-pointed. This global maintenance window is an accepted trade-off — registry bumps are rare (adding a timbre country), unchanged templates recertify as trivial clone→publish passes, and fail-closed beats provisioning from a stale certification. Transition tests prove: after activation ALL assignments (incl. an unchanged exact country and `*`) are blocked; after re-pointing all, provisioning is restored — not just the changed country. |
...
- **Known baseline defects (R2-02/R4-01/R4-02/R5-01 + 2026-08-08 inventory, Codex-verified):** against the manifest (§4.2.1) — the R6-01/R6-02/R6-03 additions (voucher five, tolerance two, `PurchaseExpenses`) change NO deltas because all three frozen charts already define them, and scope-required `SalesStampDutyPayable` is present in TN — so: **Tunisia is missing ONE required purpose (`SalesDiscount`), Generic is missing NONE, France is missing FIVE required purposes plus one protected code**:
  ...
  - TN + FR `SalesDiscount` — unprechecked throwing lookup on POS account-charge with a positive transaction discount (`GLS:3807-3827`). On the live device path this is NOT an HTTP 500: ingestion commits the event first, then the treasury GL projection throws, retries, and **dead-letters** — receipt exists, accounting projection silently absent (R10-02/R11-02; `OutboxIngestor.php:228-241,960-977`, `ApplyFiscalEventProjectionJob.php:391-421,433-448`). **Publish blocker per the manifest (R4-01).**
...
- **Release 2 (reader switchover):** consumers switch to the resolver ONLY after `php artisan country-defaults:verify` passes on staging and production. `verify` asserts: three bootstrap-keyed templates exist; `TN`/`FR`/`*` assignments reference **published** templates whose certification fields (incl. `capability_registry_version`) are all present and whose scope covers the assignment; every **currently assigned** template passes the full current publish-gate invariants (re-executed, not just hash-compared — incl. the non-timbre stamp-purpose negative rule); superseded **unassigned** published versions are integrity-checked only (stored `content_hash` matches their rows — they were valid under their stored `capability_registry_version` and are non-blocking immutable history, per the drift-scan rule below); capability-version drift makes `verify` **FAIL non-zero** and provisioning is blocked by the resolver until the drift is resolved via the §3.1 transition protocol (R10-01 — no report-only mode). **Drift-scan scope (R12-01):** the scan compares **every current `country_template_assignments` row — all exact countries AND `*`, not only the three bootstrap rows —** against its referenced template's stored version; superseded published versions that are no longer assigned are immutable history and do NOT block verification (no archive step required). Registry-bump verification test: with `TN`, `FR`, `*`, and at least one additional unchanged exact assignment, `verify` stays non-zero until the LAST assignment is re-pointed, then passes even though superseded versions remain published. Drafts-only state fails. **Capability-drift CI test (R8-03):** every country in `CountryTaxConfigurationRegistry` whose tenant tax seeder writes an active `is_stamp_duty=true` configuration MUST be flagged timbre in `CountryAccountingCapabilities`, and vice versa — adding a timbre tax seeder without the capability flag fails CI. The release-2 gate is a deploy-checklist item (added to the launch-program consolidated deploy list), enforced by runbook + the verify command — explicitly NOT by the entrypoint.
...
3. **Existing-tenant backfill tickets for live purpose gaps found in review (§5.4):** FR `CostOfGoodsSold` (silent missing COGS), FR `GeneralExpense`, TN+FR `SalesDiscount` (POS account-charge GL projection retries → dead-letters; receipt exists without its accounting projection — R10-02), FR `CustomerAdvance` + `SupplierAdvance` (ordinary treasury advance/prepayment-refund flows 500), FR `624` (categories silently omitted, couples with `GeneralExpense` — R10-03). Full detail: `docs/handoff/HANDOVER-live-accounting-gaps-country-defaults-review-2026-08-09.md` (fact-checked by Codex round 10).
