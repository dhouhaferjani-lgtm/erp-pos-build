...
- **Date:** 2026-08-08
- **Status:** Rev 11 — post Codex round 10 (REJECT: R10-01..R10-04; template layer of R7-02 CLOSED, R9-02 disposition CLOSED, handover fact-checked 7 PASS / 2 PARTIAL / 1 FAIL). R10-01 closed: registry bumps now use an explicitly **fail-closed maintenance sequence** (activate → provisioning blocked for affected countries → clone/recertify/re-point → restored; no staged-version machinery; §6 verify fails non-zero, no report-only mode). Handover corrected per R10-02/03/04 (item C = dead-lettered projection not HTTP 500 + replay obligation; item F = categories omitted not collapsed, couples with `GeneralExpense`; item E citation fixed). Pending Codex round 11 + owner sign-off. Register: `reviews/2026-08-08-country-defaults-spec-review.md`.
...
| capability_registry_version | string nullable | **set at publish, immutable** (R8-03/R9-01): the `CountryAccountingCapabilities` version the certification was decided under. **Version drift blocks PROVISIONING, not just new assignments:** the resolver (§5.1 step 2b) refuses any assignment whose template's stored version differs from the current registry version, throwing a typed `TemplateRecertificationRequiredException`; `country-defaults:verify` FAILS (non-zero) on drift. **Registry-bump transition protocol (R10-01 — deliberately fail-closed, no staged-version machinery):** activating a new registry version immediately blocks NEW-company provisioning for affected countries (typed exception; existing companies untouched per owner ruling); the runbook then clones → recertifies (publish records the NOW-current version) → re-points each affected assignment, restoring provisioning. The brief provisioning block is an accepted maintenance window — registry bumps are rare (adding a timbre country) and fail-closed beats provisioning from a stale certification. A transition test exercises this exact activation ordering, not just the eventual resolver refusal. |
