...
6. **Batch traceability shape:** `opening_balance_batches.import_file_reference` is a **jsonb column cast to array** (not a scalar). Import-created batches store `{"import_job_id": "<job uuid>", "source": "unified-import"}`; lookup is by the `import_file_reference->import_job_id` json path; the §1 "partial unique index on (import_file_reference, type)" is implemented as a unique **expression** index over `(import_file_reference->>'import_job_id', type)`.
