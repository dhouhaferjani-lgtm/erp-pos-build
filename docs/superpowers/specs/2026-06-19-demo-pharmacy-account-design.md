...
- ✅ **Branch tax-identity rendering FIXED** by the parallel bug session (2026-06-20, branch `fix/seller-tax-identity-rendering` off `dev`, not yet committed/merged). It was bigger than the Z-report — **4 unfixed render surfaces** (Z-report, document PDFs invoice/credit-note/delivery-note/quote/SO, document footer + PO seller, receipt label) all now re-resolve via `TaxIdentityResolver(location)`, plus a **per-country label** from `countries.tax_id_label` (TN→"Matricule Fiscal"). **No data bug** — the sealed SALE_RECEIPT correctly captures branch identity (device `sellerIdentity.ts`); Z_REPORT seals `seller:null` by design. **Dependency:** that branch must reach `dev` before the staging build (§11). Our seeder must ensure the TN `countries.tax_id_label` is populated.
...
- Stock: warehouse holds bulk (~90% of catalog); each shop holds ~60% in small front-of-house quantities. **Variant SKUs (sized goods) DEFERRED** — they live in the `final` `ParapharmacyMultiBranchSeeder` (France), not the base `ParapharmacySeeder` we extend, so the Tunisia catalog has none. Porting `seedVariantProducts` (compression stockings / orthopedic sizes — legit parapharmacy items) is a clean follow-up if the client wants sized-goods + variant-aware transfers.
...
Stock transfers are **variant-aware** in the code (`stock_transfer_lines.variant_id` + service threads it through initiate/complete/cancel, batch-scoped), but the demo uses **non-variant** products (variants deferred — §5.1). Seed:
- 1–2 transfers **warehouse → shop**, **completed** (stock visibly moved).
...
- 🐞 **Store-credit / customer-advance balance sign bug (NEW, found 2026-06-20).** `PartnerBalanceService::refreshPartnerBalance` stores `partners.credit_balance` as a signed value (`debit − credit` = **−N** for a `CustomerAdvance` credit). Then `PartnerListPage.tsx::getNetBalance` computes `receivable − credit_balance = 0 − (−N) = +N` and renders it **red as "owes us"** — the exact opposite of store credit. So a customer with store credit looks like a debtor. Pre-existing production bug (not our seeder). The demo therefore **omits store-credit customers** until fixed. Needs: store `abs()` magnitude (or invert for liability purposes) + verify the display. Candidate for a fix-prompt like the tax-id one.
- ✅ **Branch tax-identity rendering — FIXED** by the parallel bug session (branch `fix/seller-tax-identity-rendering`, off `dev`, uncommitted): all 4 render surfaces re-resolve via the location, with per-country labels (TN→"Matricule Fiscal"). No longer a gap — now a **merge dependency** (§11).
- **Tunisia stamp duty (timbre fiscal) seeded but NOT applied to POS receipt totals** — confirmed by the bug session (out of their scope). `TunisiaStampDutySeeder`/`TunisiaTaxConfigurationSeeder` seed it but it isn't added to `pos_receipts` totals at sale time. Demo receipts omit the legally-required stamp. Caveat or follow-up dev item; not a demo blocker.
- **Matricule-fiscal validator mismatch** — confirmed by the bug session: `TunisianParapharmacySeeder:170` seeds `TN1234567ABC`, which fails `CountryTaxNumberRules` TN regex → 422 in settings UI. **Our `DemoPharmacySeeder` avoids it** (validator-conformant `1234567AM000`, §5.1); the *old* Tunisian seeder is the buggy one. Follow-up: fix that seeder too.
...
- **Pre-existing precision drift in goods-receipt (found 2026-06-20).** `GoodsReceiptService::processReceiptLines` casts `unit_price`/`quantity` to `(float)` before passing to `WeightedAverageCostService::recordPurchase` (which takes `float`). Violates the monetary/quantity precision contract at the service boundary (not the seeder — the seeder passes strings). Pre-existing; the PO-receipt demo exercises this path. WAC values will be float-derived. Flag for the inventory/costing owner; not a demo blocker.
...
- **Merge dependency:** branch `fix/seller-tax-identity-rendering` (parallel bug session — branch tax-identity render fix + per-country labels) must reach `dev` **before** the staging build, or the demo Z-report/documents show the wrong establishment tax id. Also ensure `countries.tax_id_label` has TN→"Matricule Fiscal".
