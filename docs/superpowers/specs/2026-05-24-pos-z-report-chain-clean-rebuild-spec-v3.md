# POS Z-Report Chain Clean Rebuild Spec (v3)
...
- Clarifies canonical hash boundaries: `chain_context` is sealed in the pre-hash canonical object; `canonical_bytes` and `current_hash` are storage/transport fields outside that object.
- Adds explicit migration requirements for existing single-chain ingest, verification, quarantine, immutability, and chokepoint machinery.
- Defines separate training chain contexts so production exports can omit training closures without apparent sequence gaps.
- Makes Phase 4 operational-chain `CASH_OUT`/`SAFE_DROP` authoring a mandatory cutover disposition instead of a loose compatibility note.

Changes from v1:

- Makes `chain_context` a persisted local/server fiscal-event discriminator with uniqueness rules.
- Clarifies that `X_REPORT` advances the Z-session chain sequence but never advances `z_number` or closure totals.
- Adds an explicit compatibility rule for Phase 4 interim `CASH_OUT`/`SAFE_DROP` authoring.
- Simplifies `SESSION_OPEN` and `OPENING_FLOAT` ordering.
- Locks implementation to device-authored `DAY` Z reports; month/year GRANDTOTAL exports derive from canonical day reports.

Locked v3 decisions:

- `SESSION_OPEN`, `SESSION_CLOSE`, `X_REPORT`, and `Z_REPORT` become device-authored fiscal events.
- Z/session events use a **separate Z-chain head** from receipt/operational fiscal events. This is not currently supported by `FiscalEventEngine.append()`; the engine must be extended with an explicit persisted chain context.
- Training events use separate training chain contexts and never consume production sequence numbers.
...
**Recommendation locked in v3 unless owner overrides:** Z-report/session chain owns cash drawer movement fiscal events.
...
1. **Device-authored session lifecycle.**
   - `SESSION_OPEN` starts a fiscal session and records the declared opening float amount.
...
3. **Z-chain engine support.**
   - Extend device fiscal authoring with explicit chain context: `operational`, `z_session`, `training_operational`, and `training_z_session`.
   ...
   - Add an immutable `chain_context` discriminator to local and server fiscal events.
...
- `training_operational`: training-mode equivalent of `operational`.
- `training_z_session`: training-mode equivalent of `z_session`.
...
`chain_context` is persisted in both local SQLite and server PostgreSQL fiscal-event rows. It is not inferred at verification time from `event_type`.

Training contexts are mandatory, not optional. A production export includes only `operational` and `z_session` rows. Training rows can be retained, synced, and audited, but they must not consume production sequence numbers or produce apparent gaps when production exports ignore them. `training_flag` and `chain_context` must agree:

- `training_flag=false` is allowed only for `operational` and `z_session`;
- `training_flag=true` is allowed only for `training_operational` and `training_z_session`;
- a mismatch is a strict parser or ingest rejection.

Required constraints:

- local unique index: `(tenant_id, company_id, terminal_id, chain_context, sequence_number)`;
- server unique index: `(tenant_id, company_id, terminal_id, chain_context, sequence_number)`;
- server ingest rejects a row whose `event_type` is not allowed for the submitted `chain_context`;
- canonical envelope includes `chain_context`, so the chain assignment is part of the sealed bytes;
- sync cannot rewrite `chain_context`.
- event type allow-lists are context-specific. Production session/movement/Z types are allowed only in `z_session`; training session/movement/Z types are allowed only in `training_z_session`; existing receipt/account/approval types remain in `operational` or `training_operational` according to `training_flag`.

Existing single-chain assumptions must be removed or explicitly bridged before implementation is considered complete:

- local `FiscalEventEngine.append()` must select and advance a chain head by `chain_context`, not by terminal alone;
- `FiscalEventEnvelope` and generated TS/PHP contracts must carry `chain_context` as an immutable sealed field;
- server `OutboxIngestor` prior-row lookup, conflict detection, idempotency lookup, and retry/quarantine slot keys must include `chain_context`;
- database uniqueness must migrate from the existing single-chain `(tenant_id, terminal_id, sequence_number)` shape to the chain-context shape without losing pre-cutover rows;
- `VerifyEventChainCommand` must verify each `(tenant_id, company_id, terminal_id, chain_context)` stream independently;
- fiscal event update/immutability whitelists must allow only metadata updates that cannot alter `chain_context`, canonical bytes, sequence, hashes, or payload;
- registry drift tests must prove every event type has exactly one permitted production context and, where supported, exactly one permitted training context.
...
- training operational head: `training_fiscal_event_genesis_seed`, `training_fiscal_event_last_hash`, `training_fiscal_event_sequence`;
- training Z-chain head: `training_z_chain_genesis_seed`, `training_z_chain_last_hash`, `training_z_chain_sequence`;
...
- `X_REPORT` does not close a session and can be repeated. It advances the `z_session` fiscal chain sequence because it is an appended fiscal event, but it does not advance `z_number`, mutate grand totals, or freeze the session.
...
All new events use a two-layer canonical contract.

**Pre-hash canonical object.** This object is encoded with the existing canonical JSON rules and becomes `canonical_bytes`. It includes:
...
`chain_context` is sealed inside this pre-hash object. Reassigning an event to another chain changes `canonical_bytes` and therefore changes the hash.

**Storage/transport fields.** These are persisted or transmitted beside the canonical object, but they are not included inside their own hash input:

- `canonical_bytes`: the JCS/canonical JSON encoding of the pre-hash canonical object;
- `current_hash`: `sha256(canonical_bytes)`;
- parser and ingest metadata such as received time, sync batch id, verification status, projection status, or quarantine reason.

Implementers must not place `current_hash` or `canonical_bytes` inside the pre-hash canonical object. The payload itself must include enough business identifiers to be exported without rereading mutable operational rows.
...
`SESSION_OPEN` records the declared opening float amount. A distinct `OPENING_FLOAT` movement event references the `session_id` and carries the auditable cash movement. `Z_REPORT` validates and summarizes both; no circular `SESSION_OPEN -> OPENING_FLOAT -> SESSION_OPEN` reference is required.
...
`X_REPORT` is explicitly non-closing. It advances the `z_session` chain sequence like every appended event, but it must not advance `z_number`, update cumulative grand totals, or set any close/freeze marker.
...
- `z_number`
- `formatted_z_number`
- `period_type` (`DAY` in this implementation)
- `session_id`
- `shift_id`
- `business_date`
- `period_start`
...
This implementation authors only `DAY` Z reports. NF525 month/year GRANDTOTAL export sections are deterministic derived artifacts over canonical day `Z_REPORT` bytes. A later phase may introduce explicit device-authored month/year closure events if certification review requires them.
...
- verify chain context continuity independently for `operational`, `z_session`, `training_operational`, and `training_z_session`;
- verify `chain_context` is sealed in the canonical envelope and matches the event type allow-list;
- verify `training_flag` matches the submitted chain context;
...
The existing single-chain ingest path must be migrated as part of this work. The implementation must update at least:

- `OutboxIngestor` prior-event lookup, duplicate detection, sequence conflict detection, idempotency, quarantine identifiers, and projection dispatch keys;
- the fiscal event schema migration that currently enforces one sequence stream per terminal;
- `FiscalEventEnvelope` and any data objects or casts that assume one terminal sequence;
- sync payload validators and error responses so a missing `chain_context` is a hard failure for cutover schema versions;
- `VerifyEventChainCommand` and related repair/audit tooling so each chain context is verified independently;
- generated type drift gates and payload registry tests so PHP and TS agree on context membership.
...
Add a chain-context infrastructure chokepoint test/gate that inventories:

- local fiscal append paths, including `FiscalEventEngine.append()`;
- server ingest, including `OutboxIngestor`;
- fiscal event schema/index definitions;
- `FiscalEventEnvelope` and generated payload contracts;
- quarantine slot keys and conflict classifiers;
- `VerifyEventChainCommand`;
- fiscal event update/immutability whitelists;
- PHP and TS event registries and context allow-lists.
...
The gates must fail if a new server authoring caller, single-chain sequence assumption, or context-less fiscal event path appears without disposition.
...
- Phase 4 interim cash drawer fiscal emissions are reconciled before cutover: a cash movement must be authored either as a legacy/interim operational-chain event or as a new `z_session` movement event, never both. Cutover terminals use only the `z_session` movement path.
- `authorCashDrawerApproval()` must be dispositioned before cutover. For non-cutover terminals it may keep authoring the Phase 4 operational-chain `CASH_OUT`/`SAFE_DROP` evidence events. For cutover terminals it must either stop authoring those operational events and call the new Z-session movement authoring path, or reject with a clear upgrade error until that path is available.
- PHP and TS registries must mark `CASH_OUT` and `SAFE_DROP` context membership explicitly. They may keep legacy DTOs for pre-cutover verification, but production cutover terminals cannot accept both an operational event and a `z_session` movement event for the same `cash_drawer_operation_id` or `movement_id`.
- Already-synced operational-chain `CASH_OUT`/`SAFE_DROP` rows remain historical pre-cutover evidence. Projection and Z aggregation can reference them for migration diagnostics, but must not count them as authoritative Z-session cash movements after cutover.
- Local POS pulls Z-chain state from server for online-only tenants without clobbering stronger local state.
- Training-mode closures append only to `training_z_session`; training-mode operational events append only to `training_operational`. Production chain/export excludes training contexts without leaving production sequence gaps. If a jurisdiction requires training exports, they must be generated from training contexts with a visible test/training marker and separate sequence continuity.
...
- Training chain heads advance independently from production heads.
- Production exports omit training contexts without sequence gaps.
- Ingest rejects `training_flag`/`chain_context` mismatches.
...
- Cutover terminal cannot author duplicate operational-chain and `z_session` cash events for one drawer operation.
- Pre-cutover operational-chain `CASH_OUT`/`SAFE_DROP` events remain verifiable but are not counted as post-cutover Z-session movements.
...
- Server ingest verifies all chain contexts independently and allows parallel sequence `1..N` streams for the same terminal only when `chain_context` differs.
- `VerifyEventChainCommand` reports per-chain status and does not collapse production/training chains.
...
- new chain-context infrastructure chokepoint gate
...
3. Phase 4 implemented `CASH_OUT`/`SAFE_DROP` DTO registration for legacy drawer approval evidence. This spec requires an explicit pre-cutover versus cutover disposition, but implementation still needs careful migration tests around already-synced rows.
4. Month/year GRANDTOTAL export is locked as derived-from-day in this spec. The residual risk is certification interpretation: if an auditor requires explicit chained month/year closures, a follow-up spec must add new event semantics rather than overloading day `Z_REPORT`.
