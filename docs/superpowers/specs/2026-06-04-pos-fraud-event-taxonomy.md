...
| `pos.no_sale_drawer_open` | CashDrawer / terminal_id | `{ reason: string|null }` | drawer-kick-without-sale path | skimming |
| `pos.manager_override_denied` | Override / context | `{ scope: 'void'|'discount_limit'|'tender_tolerance'|'refund', requested_amount, cart_total, reason }` | operator-approval verify failure paths | failed override (approvals already server-side) |
| `pos.manager_pin_failed` | Operator / attempted-context | `{ context, attempt_count }` (NO pin/hash) | `operatorStore.verifyPin` / manager-PIN verify mismatch | brute-force / guessing |
| `pos.refund_no_original_receipt` | Refund / refund-session-id | `{ refund_total, reason, customer_id }` | `refundFlowStore` manual (no scanned original) path | no-receipt refund fraud |
