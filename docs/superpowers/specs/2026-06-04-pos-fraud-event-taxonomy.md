...
| `pos.manager_override_offline_approved` | Override / terminal_id | `{ scope, reason, degraded_kind: 'server_error'\|'transport' }` (NO pin/hash) | `verifyScopedManagerPin` offline-first fallback (server 5xx OR transport failure) | override approved on a valid LOCAL manager PIN **without server confirmation** — offline-first is intended, but the bypass must be visible to fraud (server-side caps/revocation were not checked). `degraded_kind` distinguishes a reachable-but-erroring server (`server_error`, 5xx) from a genuinely-unreachable one (`transport`). |
