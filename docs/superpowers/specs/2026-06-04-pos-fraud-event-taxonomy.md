...
| `pos.manager_override_denied` | Override / context | `{ scope: 'void'|'discount_limit'|'tender_tolerance'|'refund', requested_amount, cart_total, reason, denied_kind: 'server_rejected'\|'service_unavailable'\|'no_local_match' }` (NO pin/hash) | `verifyScopedManagerPin` no-local-match / explicit-rejection / fail-closed branches | failed override. `denied_kind` distinguishes: `no_local_match` (no cached operator PIN matched the requested scope); `server_rejected` (reachable server gave an explicit authorization rejection — `valid:false`/4xx such as 403/422); `service_unavailable` (reachable server kept erroring — 5xx/408/429 — and retries were exhausted, so the override **failed closed** rather than downgrading to a PIN-only approval; a possible downgrade-attempt / outage signal). |
| `pos.manager_override_offline_approved` | Override / terminal_id | `{ scope, reason }` (NO pin/hash, NO degraded_kind) | `verifyScopedManagerPin` offline-first fallback (GENUINE transport failure only) | override approved on a valid LOCAL manager PIN **without server confirmation** — offline-first is intended, but the bypass must be visible to fraud (server-side caps/revocation were not checked). **Anti-downgrade:** emitted ONLY when the server is GENUINELY unreachable (a transport-level failure — network down / timeout). A reachable-but-erroring server (5xx/408/429) no longer falls back here: it retries and then **FAILS CLOSED** (see `pos.manager_override_denied` with `denied_kind: 'service_unavailable'`), so a forgeable PIN cannot bypass a reachable server's checks. |
