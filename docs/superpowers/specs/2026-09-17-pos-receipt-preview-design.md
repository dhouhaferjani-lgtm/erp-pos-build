...
| Golden-bytes regression | Capture `format_receipt_with_settings` output for the fixture (sale, refund, Z) at 42 and 32 columns on **unchanged `origin/dev`** and commit them under `apps/pos/src-tauri/tests/golden/`. A Rust test encodes the same fixture through `doc_encoder` and asserts the diff is confined to five whitelisted regions: the dropped `N° TVA` line, the dropped fiscal-hash QR block, the `ESC t` prologue, the currency cells, and the added QR caption line. Comparison is on decoded text lines plus per-line style attributes (bold/size/align/font), because the segment model legitimately reorders style toggles; `ESC t` and cut/feed sequences are asserted separately. Any other byte difference fails the test. | `apps/pos/src-tauri/src/printing/doc_encoder.rs` tests + `tests/golden/` |
