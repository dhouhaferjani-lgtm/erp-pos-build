...
- **Rev 3.1 (2026-07-03):** adversarial review of the Rev 3 deltas (`docs/superpowers/reviews/2026-07-03-procurement-completeness-rev3-review.md`, NEEDS-REVISION, R3D-1..11) — ALL findings addressed inline: award txn = ordered FOR-UPDATE sibling locking + re-check under lock (R3D-1); re-award via explicit group `reopen` endpoint (R3D-2); v1 `two_way` = PO-price-basis matching with the 408 quantity invariant untouched, wired as net-new matcher behavior in Wave 9 (R3D-3); Wave 8 updates document-chain readers for multi-PO + posting-level partner/currency guard + ordered line locking (R3D-4/5/11); comparison-view line correlation by (product, variant) + immutable group_id (R3D-6); fan-out numbering + first-sequence notes (R3D-7); pgsql-gated index (R3D-8); presets = per-company + both settings endpoints are new (R3D-9); DocumentType citation refreshed (R3D-10).
...
- **Index (Rev 3):** `CREATE INDEX documents_rfq_group_idx ON documents (tenant_id, ((payload->'rfq'->>'group_id'))) WHERE type = 'purchase_rfq';` — the one migration Gap 1 now carries. **`if pgsql`-gated per the house migration pattern** (local test runs may use SQLite — R3D-8).
- **`group_id` is immutable** after fan-out creation: the PUT endpoint must reject any attempt to change `payload.rfq.group_id` (R3D-6).
- **Numbering note (R3D-7):** fan-out draws N numbers from the row-locked `purchase_rfq` sequence inside one transaction — verified safe (savepoint per document, rollback leaves no gaps), but the sequence row lock is held for the whole fan-out; keep fan-out N small (UI caps supplier picker at ~10) and note the first-ever-sequence `create` race in `DocumentNumberingService.php:33` (pre-seed the sequence row in the migration/seeder).
...
- **Award side-effect (Rev 3, hardened per review R3D-1/R3D-2):** `convert-to-po` (winner must be `Responded`) runs as **one transaction that first `SELECT … FOR UPDATE`s ALL group siblings ordered by `id`** (deterministic order → no deadlock between concurrent awards), **re-checks under lock** that no sibling already has a live converted PO (`RFQ_GROUP_ALREADY_AWARDED` 422 otherwise), then converts via the registry and closes every other sibling (`Cancelled` + `closed_reason='lost'`). The existing converter path has no locking of its own (`DocumentConversionController.php:27-51` pattern), so the RFQ award endpoint owns this transaction — it is NOT inherited for free. Test: two concurrent awards of different siblings → exactly one PO, one 422.
- **Re-award (Rev 3.1 — replaces the contradictory "re-award allowed" wording):** lost siblings are `Cancelled` and the converter requires `Responded`, so re-award needs an explicit **`POST /purchase-quote-requests/groups/{groupId}/reopen`** (`can:purchase-quote-requests.convert`): allowed only when the group has no live converted PO (winning PO cancelled); it restores `lost` siblings to `Confirmed` and clears `closed_reason` (their `Responded` presentation re-derives from the intact payload response flags). After reopen, award proceeds normally. Without reopen, an awarded group is final.
...
- **Comparison view (Rev 3):** `/purchases/quote-requests/groups/{groupId}` — one column per supplier, one row per line; responded unit prices side by side with best-price-per-line highlighting; validity date + lead time + total per supplier; award button per responded column. List page groups siblings (chip "3 fournisseurs — 2 réponses"). **Line correlation (R3D-6):** rows correlate across siblings by `(product_id, variant_id)`; a line added/removed on one sibling (per-sibling PUT edits are allowed) renders as an unmatched row with no highlight; when quantities diverge between siblings, highlighting applies to **unit price only** and each cell shows its own quantity.
...
- **Feature (RefreshDatabase + seeder):** create RFQ draft; edit/record response persists prices + `validity_date` + status→Responded; `send` marks Sent; `convert-to-po` yields a `PurchaseOrder` with matching lines and `source_document_id`; RFQ never appears on any receivable/payable/fiscal report (§1.12); permission matrix; tenant isolation. **(Rev 3 group tests:)** fan-out with `partner_ids=[A,B,C]` creates 3 documents sharing one `group_id`, each with its own number; group endpoint returns all siblings + response state; award of B closes A and C with `closed_reason='lost'`; award on an already-awarded group → 422 `RFQ_GROUP_ALREADY_AWARDED`; **two concurrent awards of different siblings → exactly one PO + one 422 (lock test)**; reopen rejected while the winning PO is live, allowed after it is cancelled, restores lost siblings to Responded presentation, then a different sibling can be awarded; single-supplier fan-out (N=1) behaves exactly like the plain flow.
...
- **Matcher basis** becomes the receipt line's `accrual_unit_cost` (the price we actually booked), consumed via the receipt lines the invoice line links to. **Rev 3.2 correction (Wave-5 review W5T-2/W5I-2):** paid and free consumption are tracked SEPARATELY on the receipt line, mirroring `document_lines`' `quantity_invoiced`/`free_quantity_invoiced` split — priced invoice lines consume the **paid window** `received_qty − quantity_invoiced` (408 accrues on paid units only, so a free-inclusive window would let over-paid billing classify as Matched and desync the accrual); zero-priced bonus invoice lines consume the **free window** `free_qty − free_quantity_invoiced`. Invoice lines consume receipt lines **FIFO** within a PO line. (The Rev 2 formula `received_qty + free_qty − quantity_invoiced` is superseded. Supplier credit notes with GoodsReturn must decrement the receipt-line counters under lock — the PO-line counters are derived and get overwritten by posting.)
...
- `documents.source_document_id` (single column) keeps the **first** PO for backward compatibility; **"first" = `source_document_ids[0]` exactly as submitted** (deterministic, stored). The full list persists in `payload->supplier_invoice.source_document_ids`. Line-level linkage (`source_line_id`, and the §2.5 `matched_receipt_line_id` snapshot) is already per-line and therefore PO-count-agnostic — review-verified: the matcher never reads `source_document_id` (R3D report, "verified-TRUE" list).
- **Document-chain readers must be updated (R3D-4):** `Document::getDocumentChain`/`childDocuments` (`Document.php:358-401`) and the Related-Documents endpoint (`DocumentController.php:218`) resolve children via `source_document_id` only — without a read-side union over `payload->supplier_invoice.source_document_ids`, POs #2..N would silently lose the invoice in their Related-Documents tab. Wave 8 updates these readers (and the FE `RelatedDocumentsTab` consumes the result unchanged).
- **Posting-level guard (R3D-5):** posting re-asserts that every parent PO of the invoice's lines shares the invoice's company, partner, and currency — today that's enforced only in the create FormRequest (`SupplierInvoiceMatcher.php:335-341` validates type+company only); Wave 8 and the future OCR entry point need the assertion at the posting boundary, not just at create.
- **Lock ordering (R3D-11):** posting's `lockForUpdate()->whereIn(...)` over PO lines (`SupplierInvoicePostingService.php:70-75`) spans multiple POs after Wave 8 — add `orderBy('id')` so concurrent multi-PO posts acquire locks in a deterministic order (deadlock avoidance).
- Posting is otherwise unchanged structurally: it clears each consumed receipt line at its own immutable `accrual_unit_cost`; the B3-style basis guard, over-clear invariant, and PPV routing (§2.4) all operate per line.
...
- **Schema:** add `procurement_policies.preset varchar(20) NULL` (records the applied bundle; NULL = custom/hand-tuned fields). New enum `ProcurementPreset` (rule 9) with a `values(): array` mapping preset → policy field values. Policies are **per-company** rows (not per-tenant) — preset application and the settings endpoint operate at company scope (R3D-9).
- **`two_way` semantics — v1 definition (R3D-3; this is NET-NEW matcher behavior, not just a test):** `match_mode` currently has ZERO consumers (the matcher reads only tolerances). Wave 9 wires it, with a deliberately narrow v1 meaning: under `two_way`, the **price basis switches from receipt-line accrual to the PO line's contractual `unit_price`** (invoice vs order price — the classic 2-way price control), and price-variance classification/preview use that basis. **The 408 quantity invariant is UNTOUCHED:** receipt lines are still created on receiving, GR-IR still accrues, posting still clears receipt lines FIFO, and invoiced qty is still hard-capped by received qty (the R3-1 invariant is not mode-dependent). What v1 `two_way` does NOT enable: invoicing ahead of receipt — that is the reserved `bill_control_mode='ordered'` Phase-2 mode (and pairs with S5 receipt-first / PO-less service invoices). The `MatchMode.php:11` docblock ("no GR requirement") must be corrected to this definition when Wave 9 lands, so the enum, the presets table, and §3.2's Phase-2 note stop describing three different things.
- **Seeding:** preset application in the procurement policy seeder — per company, driven by tenant/vertical (demo seeders: parapharmacy = `Standard`); `DemoPharmacySeeder` asserts the preset it expects.
- **Settings (R3D-9 — these endpoints do NOT exist today, both are new):** `GET /api/v1/procurement-policies` (current company policy + preset) and `PUT /api/v1/procurement-policies` — accepts either `preset` (applies the bundle, stamps the column) or raw fields (clears `preset` to NULL). Rule-12 middleware; `can:company-settings.update` (or the module's settings permission). Company-settings UI: a three-card preset picker + an "avancé" section exposing the raw fields.
- **Tests:** applying each preset writes exactly the mapped fields; raw-field edit nulls the preset; seeder idempotency; matcher under `two_way` classifies price variance against PO `unit_price` while a receipt at a different price still clears 408 at its own accrual basis; invoiced-qty-beyond-received still rejected under `two_way`.
...
- **Wave 1 — Gap 1 backend (RFQ core + groups).** `PurchaseQuoteRequest = 'purchase_rfq'` enum + helper arms (+ length assertion) + exhaustive-`match` fixes; RFQ invariants (§1.12) incl. NonFiscal test + zero-GL/zero-stock test; **fan-out create (`partner_ids[]` → N siblings, one `group_id`, single txn) + group read endpoint + group index migration (Rev 3)**; `PurchaseQuoteRequestToPurchaseOrderConverter` + registration + **award txn with ordered FOR-UPDATE sibling locking + `RFQ_GROUP_ALREADY_AWARDED` re-check under lock + sibling auto-close + group `reopen` endpoint (R3D-1/2)**; `CreatePurchaseQuoteRequestRequest`; controller + routes + seeder permissions; blade. Exit: fan-out→respond→compare(API)→award green; siblings close; PO carries `source_document_id` + carried prices; RFQ posts nothing.
...
- **Wave 9 — Procurement presets (Rev 3/S8).** `ProcurementPreset` enum + `procurement_policies.preset` column; seeder preset application (per company, tenant/vertical-driven); **NEW `GET`+`PUT /procurement-policies` endpoints** (none exist today — R3D-9); preset picker UI card in company settings; **wire `match_mode` into the matcher with the v1 `two_way` = PO-price-basis definition (net-new behavior — R3D-3) + correct the `MatchMode.php:11` docblock**; 408-invariant-under-two_way tests. Runs after Wave 5's matcher lands; parallel to Waves 6–8.
