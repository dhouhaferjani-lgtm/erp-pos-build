...
- **Recoverable-VAT source (review B2-4 / R3-8 — use the right column):** book `VatDeductible` from the **recoverable tax**, **NOT** `documents.tax_amount` (which = line tax **+ stamp duty**, `DocumentTotalsCalculator.php:45` — using it would wrongly recover the timbre). Note `line_tax_amount` lives on **`documents`** (not lines). Correct source: use `documents.line_tax_amount` only if **all** line tax is recoverable; otherwise **sum `document_lines.recoverable_tax_amount`** (column exists, migration `2026_01_02_100004…`; **expose it in the `DocumentLine` fillable/casts** — currently not surfaced). Timbre comes from `documents.stamp_duty_amount` → `PurchaseStampDuty`.
...
**Persistence (reviews B2-9 / R3-7 — pick ONE shape now):** a **tenant-scoped `procurement_policies` table keyed by `company_id`** (NOT a loose company column) with: enum-backed `bill_control_mode`/`match_mode`/`match_enforcement` columns each with a **PG CHECK** allowlist; `variance_tolerance_percent decimal` + `variance_tolerance_max_amount decimal` (currency scale) with **PG CHECK `>= 0`** (a negative tolerance would invert match behavior); seeded per-vertical defaults; one row per company resolved by the policy resolver (company/vertical default in Phase 1; vendor/item override seam unbuilt). PG-only CHECKs need real-PG verification (SQLite won't catch them).
...
- **Status modeling (review B2-5 — `DocumentStatus` has NO `matched` case → hydration would break):** use `DocumentStatus` for `draft → posted → paid` only; carry the match result in a **separate `match_status` field** (own enum, used everywhere — §6: `unmatched`/`matched`/`price_variance`/`quantity_variance`/`exception`). Do NOT add `matched` to `DocumentStatus`. Enums only, no magic strings.
...
- **Single `match_status` enum (review R3-6), used everywhere** (PG CHECK, PHP enum, generated TS type, filters, tests): `unmatched | matched | price_variance | quantity_variance | exception`.
- **HARD quantity invariant — NOT subject to `match_enforcement` (review R3-1, BLOCKER):** the 408 over-clear guard `invoiced_qty <= (quantity_received − quantity_invoiced)` is a **hard accounting rule** that ALWAYS blocks an over-received/unreceived posting (`quantity_variance`/unreceived `exception`), regardless of the `warn`/`block` setting. `warn` must NEVER let a quantity-over-clear post. (If a business genuinely needs to invoice-ahead-of-receipt, that is the Phase-2 invoice-first mode, not a `warn` bypass.)
- **ADVISORY price/amount variance — governed by `match_enforcement`:** for `price_variance` within nobody-disputes territory, `warn` surfaces it and lets the user post (Odoo "Should be paid" pattern); `block` refuses. Variance tolerance = config (small percentage + max-amount, dual-threshold like the tender tolerance), **defaulted, not hand-typed**, with non-negative CHECKs (§4).
- **Concurrency + idempotency (review R3-2, BLOCKER):** invoice posting runs in ONE DB transaction that `SELECT … FOR UPDATE`-locks the matched PO lines, recomputes matchable from the locked rows, increments `quantity_invoiced`, and posts the GL — so two concurrent posts cannot both pass the guard. Posting/allocation carries a **unique idempotency anchor** so a retry cannot double-increment `quantity_invoiced` or double-post the GL.
...
- **Damaged goods / price disputes — supplier credit note (reviews B2-2 / R3-3 / R3-4 — do NOT reuse the customer path):** the existing `credit_note` + `createFromCreditNote()` is **customer-side** (Dr sales revenue / VAT-payable, Cr AR — `GeneralLedgerService.php:183,237`). A supplier credit note needs its **own type/branch**. Resolve adjustments by posting this supplier-credit-note (a new immutable entry) — never by editing a posted invoice (event-immutability).
  - **Quantity reversal (R3-3):** a **returned-goods** credit note **decrements `quantity_invoiced`** on the PO line (reopening it for re-invoicing) and removes inventory; a **price-only** credit note does **NOT** touch `quantity_invoiced`. Block credits exceeding the invoiced/received balance (no negative `quantity_invoiced`, no over-credit).
  - **GL matrix (R3-4) — to implement exactly (partner-tagged 401):**

    | Scenario | Dr | Cr | qty effect |
    |---|---|---|---|
    | Price-only reduction (post-invoice) | `SupplierPayable` (401) | `VatDeductible` (recov. portion) + `Inventory` *or* purchase price-variance (the HT delta) | none |
    | Returned goods, still in stock | `SupplierPayable` (401) | `VatDeductible` + `Inventory` | `quantity_invoiced` −= returned qty |
    | Returned goods, no longer in stock | `SupplierPayable` (401) | `VatDeductible` + purchase price-variance/expense | `quantity_invoiced` −= returned qty |
    | Timbre | (non-recoverable, generally not reversed unless supplier credits it) | — | none |

  - Scale the partner tagging, the recoverable-vs-non-recoverable VAT split, and the inventory-vs-variance choice (WAC implications) precisely in the plan; the matrix above is the contract.
...
- **Round once with `bcround`, not `bcformatStrict` (review R3-5):** `qty × unit_cost` can exceed currency scale. `CurrencyScale::bcformatStrict` **truncates** (`bcadd(…, scale)`); use `CurrencyScale::bcround($amount, $scale)` (half-up, the COGS-boundary pattern) **once** to produce the equal debit and credit legs. Same for the invoice legs (HT, recoverable VAT, timbre). Truncation here would silently lose sub-millime value and could break debit==credit.
