# POS Phase 1 — Foundation: Fiscal Event Engine + Receipt-Chain Clean Rebuild (v4)

**Date:** 2026-05-14
**Phase:** 1 of 5 (roadmap v2)
**Status:** Drafted. v1 → Codex BLOCK (3 BLOCKER / 3 P1 / 3 P2); v2 → Codex BLOCK (2 BLOCKER / 1 P1 / 1 P2); v3 → Codex BLOCK (1 BLOCKER / 3 P1 / 2 P2); v4 resolves all 6 v3 findings. Awaiting self-review → owner review → Codex re-review → writing-plans.
**Supersedes:** v3 (`2026-05-14-pos-phase1-foundation-spec-v3.md`), v2, v1.
...
- **Codex v3 review** — `reviews/2026-05-14-pos-phase1-foundation-spec-v3-codex-review.md`. Its file:line citations are cited `[Codex v3]`.
...
**`PosCoreReceiptProjection`** — always runs; **POS module**. For a `SALE_RECEIPT` fiscal event it creates the POS-core business effects, which are the business-effect logic of the old `ReceiptSyncService` (its batch transport/orchestration is *not* reused — only its business effects `[Codex v3 confirms `ReceiptSyncService` owns these effects]`):
- the `pos_receipts` projection row + lines + VAT breakdown,
- `ReceiptPayment` rows — the **POS's own payment record** (`ReceiptSyncService.php:598-609`),
- voucher redemption (`ReceiptSyncService.php:683-710`),
- stock movement (`ReceiptSyncService.php:713-735`).
It is **idempotent**, keyed on `(fiscal_event.id, 'pos_core_receipt')`. Its dependencies on other modules are **inbound reference data only** `[SoT §13.6 as refined]`: `ReceiptPayment` references the mirrored `payment_method_id` / `repository_id` setup data (`ReceiptPayment` imports Treasury's `PaymentMethod` and `pos_receipt_payments.payment_method_id` FKs to `payment_methods` `[Codex v3 — `ReceiptPayment.php:8,148-154`, migration `:29-32`]` — a permitted inbound setup dependency, *not* a coupling to the Treasury operational module). It has **zero** dependency on the Treasury *operational* module (Treasury `Payment` rows, GL, allocation) — a deployment with Treasury inactive runs this projector to completion.

**`TreasuryReceiptBridge`** — runs only when the Treasury module is active (per the `ModuleActivationResolver`); **Treasury module**. For a `SALE_RECEIPT` fiscal event it creates the Treasury-operational effects, which are the logic of `ReceiptPaymentService` `[Codex v3 — `ReceiptPaymentService.php:35-43,252-276`]`:
...
> **Resolves v2 BLOCKER 1 and v3 BLOCKER.** The POS-core effects (`ReceiptSyncService` logic) and the Treasury-operational effects (`ReceiptPaymentService` logic) are correctly attributed and split across the two projectors. The POS-core projector's only cross-module dependency is *inbound mirrored reference data*, which `[SoT §13.6 as refined]` explicitly permits; the *outbound* dependency on the Treasury operational module is gated entirely behind the bridge + resolver.
...
- **Resume after parse-failure resolution (resolves v3 P1):** when an authorized operator/developer resolution writes `payload` and transitions `payload_parse_status → parsed` (the §3.3 immutability trigger permits exactly this transition), the resolution path **must** then create the `pending` `fiscal_event_projections` rows for all currently-active projectors and enqueue them. This is done by the resolution action invoking the named command **`fiscal:enqueue-resolved-event-projections`** (§15), which is also runnable standalone for operator recovery. Resolution that flips `payload_parse_status → parsed` without enqueuing projections is incomplete — a test asserts the resolve→project path (§17.3).
...
| `ReceiptPaymentService` — Treasury `Payment` + GL logic | **REUSE (relocated → `TreasuryReceiptBridge`)** | Extracted into the pluggable Treasury bridge (§7.4), invoked by `OutboxIngestor` only when the Treasury module is active (per the `ModuleActivationResolver`); accepts `fiscal_event_id` / `origin`. **Treasury module.** |
...
`[Codex v3 — the web-POS server-authoring path is live]`. The web POS authors receipts **server-side** (a browser cannot device-author/seal), so it is a server-recompute path that cannot coexist with the rebuilt device-authority chain `[SoT D8]`. Phase 1 does **not** rework it onto device-authority — that is deferred parity (§18) — but the preflight gate (§2) must hide/disable its **write** path. Concrete targets:
...
**Disposition outcome recorded in the preflight sign-off (§2):** either "web POS write path confirmed unused — no action" or "web POS write path hidden/disabled per this checklist; parity deferred (§18)."
...
```
- For a `fiscal_events` row whose `payload_parse_status` has been resolved to `parsed` (§7.5) and that has **no** `fiscal_event_projections` rows, creates one `pending` row per currently-active projector (via the `FiscalEventProjectionRegistry` + `ModuleActivationResolver`, §7.3) and enqueues the projection jobs.
- Invoked by the parse-failure resolution action automatically; also runnable standalone for operator recovery.
- Idempotent: if `fiscal_event_projections` rows already exist for the event, it is a no-op (it never duplicates or resets an `applied` / `running` row).
