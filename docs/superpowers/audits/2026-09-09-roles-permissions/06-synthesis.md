...
> Lane: roles & permissions management. Date: 2026-09-09. Base: local `dev` at `e85641a66` (audit reads); branch `docs/rbac-audit-2026-09-09` is based on `73f2040c6`, at which the Opus verification ran (HEAD moved to `bd986f898` during the audit; nothing permission-related changed in between except lane T-1 gate merges). Status: **AUDIT COMPLETE — awaiting owner direction. No implementation has started.**
>
...
An Opus `tenancy-authz-reviewer` pass re-verified the twelve headline claims (C1–C12) against code before this document was put in front of the owner. Verdicts (full table and evidence in `07-verification-opus.md`): C1, C2, C5, C6, C7, C8, C9, C10, C11 CONFIRMED; C3, C4, C12 PARTIALLY — corrections applied in §1, §2 and §6 below and marked **[corrected]**. Five findings the Sonnet pass missed were added as I-18..I-22.
...
**Enforcement.** Of 1054 API routes: 642 gated by `can:` middleware, 130 in-controller, 46 FormRequest, 8 policy, 68 super-admin-only, 18 public, and **142 authenticated routes with no permission check found by one-hop static analysis; 178 write routes carry no route-level permission middleware; of the 53 write routes the sweep flagged as unguarded at every layer, Opus spot-checked 12 and found 2 false positives (`POST`/`DELETE /uom/units` are gated in-controller), so the verified floor is 51** [corrected]. Thirty-three permission strings are enforced in middleware on some routes and in controllers on others. 180 FormRequests return `true` from `authorize()`. No coverage ratchet test exists. (02 §"Classification totals", §"Live write AUTH_ONLY routes"; 01 §11)
...
**Sync and drift.** New tenants receive the full catalogue at provisioning. The provisioning guard `seedRolesAndPermissionsIfMissing()` returns early once an `admin` role exists, so the application code never re-syncs an existing tenant. **However the API container entrypoint (`apps/api/docker/entrypoint.sh:162-165`) runs the full seeder across every tenant on every deploy when `SYNC_PERMISSIONS_ON_BOOT=true` — which staging sets.** That is how new permissions currently reach live tenants, and because the seeder calls `syncPermissions()` unconditionally, every push also silently reverts any tenant customisation of the seven seeded roles [corrected]. Where the flag is unset, existing tenants receive nothing. The frontend map is generated from the seeder with a CI + preflight drift guard. (01 §2, §11; 03 §1)
...
| I-2 | 51+ live write routes with no permission at any layer (verified floor; 178 write routes have no route-level middleware): categories, services, service-categories, coupons, promotions, channels, `POST /companies` (FormRequest `authorize()` returns `true` and the controller self-grants `MembershipRole::Owner`, `CompanyController.php:151`), batches delete/recall, progression, purchase-hub, notifications [corrected] | P0 authz | 02 "Live write AUTH_ONLY"; 07 C4 |
...
| I-10 | No last-admin protection; role create/update/delete unaudited; role read endpoints open (`RBACTest:137-165` acts as admin, so openness is untested, not asserted); `RoleController::update` guards only the rename — `syncPermissions` at `:246` runs unconditionally even on `admin`, so a `roles.manage` holder can strip the admin role (latent: only `admin` holds `roles.manage` today) [corrected] | P1 | 01 §5; 07 C6, missed #3 |
...
| I-13 | The `add-permissions` slash command prescribes a non-existent `permission:` middleware and `settings.edit`; convention 03's real defect is a canonical route example with no authorization at all (`:30-41`) plus a superseded hand-maintained FE map description [corrected] | P2 process | 01 §11; 07 C12 |
...
| I-18 | `GET /users/{userId}/roles` returns any user's full effective permission list to any authenticated caller; `Identity/routes.php:58,77` comments claim `roles.view`/`roles.manage` gates that do not exist on the routes | P1 info-disclosure | 07 missed #1 |
| I-19 | `admin` is granted `Permission::all()` (not the declared list), so with the every-deploy seeder run any stray permission row in a tenant DB is auto-attached to admin | P2 | 07 missed #2 |
| I-20 | `SYNC_PERMISSIONS_ON_BOOT=true` on staging = destructive full reseed of every tenant on every deploy | P1 data-loss | 07 C3 |
| I-21 | `tests/Feature/Document/RefundResidualTenantIsolationTest.php:136-142` `firstOrCreate`s `credit-notes.cancel`, masking the seeder omission (I-3) and keeping the suite green | P2 test quality | 07 missed #5 |
| I-22 | `uiAliasPermissions.ts` maps to role names no seeder creates (`sales, purchases, inventory, treasury, user`); for `/services` this dead map is the only authorization that exists | P1 | 07 missed #4 |
...
- **Wave 0 — stop the bleeding** (Direction A content): tenant-scoped cache (S-1, brief already exists), gate the 53 write routes, seed `credit-notes.cancel`, delete `PermissionSeeder.php`, fix convention 03 + slash command, add the route-coverage ratchet with today's baseline, replace `SYNC_PERMISSIONS_ON_BOOT` with a non-destructive additive sync (or disable it until wave 1), gate `GET /users/{userId}/roles` (I-18), remove the masking `firstOrCreate` in the refund test (I-21).
