...
| **R-02** `[N]` ⚙ | **Marketplace admin routes have super-admin semantics under tenant auth** | `apps/api/app/Modules/Marketplace/Presentation/routes.php:47-65` — `POST/PATCH sellers`, `POST sellers/{id}/suspend` run under `['api','auth:sanctum',SetPermissionsTeam,EnforceTokenTenantClaim]` + `can:marketplace.admin`. `RolesAndPermissionsSeeder.php:385` puts `marketplace.admin` in `permissionNames()`, and `admin` syncs `Permission::all()` → **every tenant admin holds it**. `store` accepts arbitrary `tenant_id`/`company_id`. **Blast radius is tenancy-mode dependent** (corrected 2026-08-05 after adversarial review): in **legacy row-level** mode (`config/tenancy_resolver.php:30`, default `false`) this is a live fleet-wide create/update/suspend of every tenant's sellers; under **db-per-tenant** (the live mode) `marketplace_sellers|listings|orders` are per-tenant tables so queries stay in the caller's own DB, and the residual is **attribution spoofing** — planted rows carrying another tenant's `tenant_id` that become fleet-visible if the registry is ever promoted central (ticket option 1(a)). `marketplace.browse` is `#[CrossTenantRoute]` and seeded to essentially every role (`:382,563,612,653,711`). Creating a seller row **arms the 15-min scheduler fan-out** (`MarketplaceServiceProvider.php:55-56`) — that fan-out is guarded by data presence, not by capability. `config/marketplace.php:6` defines `'enabled' => env('MARKETPLACE_ENABLED', false)`, which until 2026-08-05 **nothing read**; it is now the kill-switch for the whole surface | Privilege-escalation/authz hole reachable by tenant #1's own admin account, fleet-wide in legacy row-level mode. P1 by blast-radius grading, **pre-launch** by policy | M | **contained** — the flag is wired (`1d6798a8b` routes + schedule, `5f5e94108` Cart marketplace-checkout, plus the Cart `items.store` marketplace payload 422 from the review fix round) and defaults **false**, so the surface is de-registered; `MARKETPLACE_ENABLED=false` documented in `.env.example`. Pinned by `tests/Feature/Security/MarketplaceFlag*Test.php` + `MarketplaceCartItemGatingTest.php`, now CI-gated (`ci.yml` `backend-test` runs the whole `tests/Feature/Security` dir). The **super-admin guard swap was ruled out** (it 500s under db-per-tenant — per-tenant tables in central context); redesign ticketed in `docs/superpowers/tickets/2026-08-05-marketplace-admin-surface-redesign.md`. Canonical `module:Marketplace` gating folded into the vertical-gating audit. **Residual: do not set `MARKETPLACE_ENABLED=true` until the ticket lands** |
