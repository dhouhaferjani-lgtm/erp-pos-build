...
## Round-5+ additions from api.console-commands triage (2026-05-06) — RESOLVED 2026-05-07

Both POS-module commands originally deferred from `api.console-commands` were
addressed in api.pos-stabilization round-5:

- **`pos:verify-chains`** (`App\Modules\POS\Commands\VerifyPosChainCommand`) — cat-(b),
  by-design fleet-wide iteration for NF525 fiscal hash chain integrity verification.
  Annotated class-level with `@cross-tenant-by-design` per master plan §14 invariant 1.
- **`pos:expire-held-orders`** (`App\Modules\POS\Infrastructure\Commands\ExpireHeldOrdersCommand`) —
  cat-(a-per-tenant-iter). Refactored to extend `TenantScopedCommand` and iterate
  `Tenant::all()->each(fn ($t) => Company::where('tenant_id', $t->id)->each(...))`,
  delegating to `HeldOrderService::expireOrders($tenantId, $companyId)` which now
  carries both predicates in the WHERE clause. Fleet-wide UPDATE eliminated
  (was 96/day on the everyFifteenMinutes() cron).

Tracking: deferrals fixture (`apps/api/tests/Architecture/fixtures/console-command-deferrals.json`)
emptied to `[]` in the same commit as the round-5 fixes. Round-5 manual callsites
seeded under `api.pos-stabilization.050–.053`.
