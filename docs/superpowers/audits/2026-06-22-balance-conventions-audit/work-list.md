...
- H-7.4 DONE — batch write-off GL entries now post with the real write-off actor. `BatchWriteOffService::writeOff()` forwards its `userId` and company currency into `createInventoryWriteOffEntry()`, and the GL helper validates the actor before creating the entry, then posts through the canonical after-commit path. Red was observed first via the batch write-off service leaving `batch_write_off` Draft.
...
- Residual scan after H-7.4 found separate draft compliance adjustment writers in `UninvoicedDeliveryNoteService`; track outside this batch write-off slice before closing the whole hardening session.
