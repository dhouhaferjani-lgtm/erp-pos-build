...
**Verified:** CONFIRMED — `AttachmentService::getStorageDisk()` `return 'local';` (`:197`, comment "Could be extended to support S3 per tenant"); `upload()` takes no disk param. The `s3` disk IS configured (`config/filesystems.php` with `AWS_ENDPOINT`). **Adjudication:** spec revised — Phase 1 includes a small, contained change to `getStorageDisk()` to return the configured object-storage disk (env-driven, default still safe) so supplier-invoice attachments land on MinIO/S3, NOT local. This is NOT the media-system unification (still a separate session); it's the minimal routing fix the spec already implied.
