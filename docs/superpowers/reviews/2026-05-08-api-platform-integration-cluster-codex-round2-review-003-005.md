# api.platform-integration cluster - Codex round-2 review (.003 + .004 + .005 fix-commit pin)
...
with the "Commit reviewed:" line pinned to `38a89b0c` — the fix_commit
for `.003` products UNIQUE migration, `.004` billing_payments UNIQUE
migration, and `.005` ProcessEnrichmentEventListener `->sole()`.
...
HEAD `9921b617`, which transitively contains `38a89b0c`.)
