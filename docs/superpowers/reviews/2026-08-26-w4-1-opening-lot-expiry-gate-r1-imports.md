...
## r3 scoped re-review (fix range fba317686..a7d9f9a79) — imports lens

**VERDICT: spec ✅ + quality APPROVED.** All 8 r2 items verified ADDRESSED against the actual diff:

1. **CI allowlist landed in the lane** — ADDRESSED. `git diff 9d0d08ae5..HEAD -- .github/` (run inside the lane worktree) shows exactly one changed file, `.github/workflows/ci.yml`, and the `backend-test-pgsql` `--filter` regex now ends `...|PosCoreReceiptProjectionBatchLotTest|OpeningLotExpiryW41Test|NullInventedDefaultLotExpiryMigrationTest|SpreadsheetParserDateCellTest)::/'`. Round 1's false claim (edit had landed in the shared main-repo `dev` worktree via a relative-path heredoc, not the lane) is documented and reverted; the lane's own `git diff 9d0d08ae5..fba317686` was empty, confirming the defect existed before this round's `09e51de94`.

2. **`expiry_is_past` rendered in wizard preview + i18n en/fr/ar + vitest** — ADDRESSED. `apps/web/src/features/opening-balances/components/BatchPreview.tsx:210-226` renders a warning badge (`data-testid="expiry-past-{row}"`) plus a table-footer hint when any line has `expiry_is_past`, and a danger badge/hint for the sibling `expiry_conflicts_with_existing_lot` flag. Keys `openingBalances.preview.expiryInPast`/`expiryInPastHint`/`expiryConflict`/`expiryConflictHint` added in `apps/web/src/locales/{en,fr,ar}/common.json`. `BatchPreview.test.tsx` adds 3 cases: past-expiry badge shown (and conflict badge absent), conflict badge shown, and a clean line carrying neither badge/hint.

3. **`expiry_ignored_no_default_lot` distinct reason, truthful wording** — ADDRESSED. New enum case `OpeningLotExpiryOutcome::IgnoredNoDefaultLot` (`apps/api/app/Modules/Inventory/Domain/Enums/OpeningLotExpiryOutcome.php:44-53`) is returned from `OpeningBalancePostingService::classifyExpiryOutcome()` when no DEFAULT lot was minted because real lots already covered the quantity (`OpeningBalancePostingService.php:364-365`, previously mis-reported as `IgnoredNotBatchTracked`). `ProductOpeningStockPhase::expiryWarnings()` (`apps/api/app/Modules/Import/Services/ProductOpeningStockPhase.php:193-203`) renders a separate, accurate message ("this product's existing lots already account for the whole opening quantity... Set the expiry on the relevant lot directly") rather than falsely claiming the product isn't batch-tracked. `isNoteworthy()` match is exhaustive over all 6 cases. Pinned end-to-end by `OpeningLotExpiryW41Test::test_a_batch_tracked_product_is_never_told_it_is_not_batch_tracked`.

4. **Accented FR aliases + docs** — ADDRESSED. `MigrationWizardService::getColumnAliases()['expiry_date']` (`apps/api/app/Modules/Import/Services/MigrationWizardService.php:197-207`) now lists both `péremption`/`peremption` and `date_péremption`/`date_peremption` alongside `dlc`/`dluo`; verified `findBestMatch()` (`MigrationWizardService.php:134-162`) normalizes only `_`/`-`, never accents, so this was a real bug now fixed. `docs/modules/imports.md:221-230` rewritten to list the actual auto-mapping aliases (minor: the doc text doesn't separately spell out the unaccented `date_peremption` alias, only `date_péremption` — cosmetic, not functional, since the code alias array does have it).

5. **Provenance-window test fixture after cutoff** — ADDRESSED. `NullInventedDefaultLotExpiryMigrationTest::test_it_never_touches_a_lot_created_after_it_ran` (`apps/api/tests/Feature/Inventory/NullInventedDefaultLotExpiryMigrationTest.php:148-154`) now uses `CarbonImmutable::now()->addMinute()` instead of a same-instant `now()`, with an inline comment explaining the prior fixture proved nothing against a strict `<` cutoff predicate.

6. **Blank date-cell case** — ADDRESSED. `SpreadsheetParserDateCellTest::test_a_blank_cell_in_a_date_formatted_column_stays_blank` (`SpreadsheetParserDateCellTest.php:111-131`) confirms a blank cell in a date-formatted column parses to `''`, not serial-0 (1899-12-30) or today. Verified against `SpreadsheetParserService::parse()` (`apps/api/app/Modules/Import/Services/SpreadsheetParserService.php:172-173`): `$value === null` is checked before `isDateCell()`, so a blank cell never reaches the date-serial path regardless of column number format.

**Also verified clean (not required by the r2 list but touched by this round's diff):** `expiry_notices` POST-response wiring (`InventoryOpeningService::expiryNoticesFor()` at `apps/api/app/Modules/Inventory/Application/Services/InventoryOpeningService.php:383-413`, wired at `OpeningBalanceBatchController.php:623-630`) reads back `mapped_data['expiry_outcome']` written during `postBatch()` (`InventoryOpeningService.php:335-341`) — confirmed this write happens before the batch's separate SHA-256 lock step (`OpeningBalanceBatchService::lockBatch()`/`calculateBatchHash()`, a later manual action, not invoked inside `InventoryOpeningService::postBatch()`), so the hash seal (when eventually computed) reads the post-mutation `mapped_data` and is not invalidated. Multi-location conflict/fill flow is pinned end-to-end (`OpeningLotExpiryW41Test::test_the_wizard_names_a_conflicting_expiry_instead_of_discarding_it`, `test_the_preview_flags_a_date_that_the_post_will_refuse_to_apply`). Residual-count fix (`residualQuery()` shared by count and list, `2026_08_26_100100_...php:177-224`) and resurrection census are correct and match their stated intent.

**No new Critical/Important findings** in `fba317686..a7d9f9a79`.

**VERDICT: spec ✅ + quality APPROVED — mergeable** (imports lens; no open items).
