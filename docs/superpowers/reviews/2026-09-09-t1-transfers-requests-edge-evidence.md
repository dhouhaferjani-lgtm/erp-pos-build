...
| 6 | P.freight_capitalization_uses_ten_owned_units_not_fourteen_inside_transaction | GREEN PG | Inside outer transaction, completed status, freight +10 over exactly 10 owned units, WAC=6; adjustment quantity_before/after=10. Denominator must not become 14. Brief capitalization ordering. | — |
...
| 11 | R.capture_and_bump_during_transit_do_not_reopen_original_or_settle_new_demand; R.small_ordinary_transfer_pins_current_behaviour_until_ticket_t1_11; R.replayed_initiated_event_must_not_settle_demand_captured_after_dispatch | Normal bump/current quantity-blind settlement GREEN; hypothetical replay RED → SKIP | Original stays fulfilled, new POS-grain demand 3+1 stays pending under ordinary capture; the synchronous listener also fulfills a five-unit request from an ordinary two-unit transfer (reachable, now live-pinned). Manual handler replay fulfills newer demand again; no production redelivery path is established. Brief must not double-settle/reopen. | [T1-11](../tickets/2026-09-09-t1-settlement-replay.md) |
...
| 14 | R.requester_can_cancel_in_progress_request_while_status_is_open; R.requester_cancel_in_progress_pins_current_behaviour_until_ticket_t1_14 | Desired RED → SKIP; current refusal GREEN | Real PO-to-warehouse action starts processing, requester-only cancel returns 422 despite isOpen(). Current behavior recorded, not silently changed. Brief: Odoo cancel until done / ERPNext stop material request. | [T1-14](../tickets/2026-09-09-t1-cancel-in-progress.md) |
| 15 | R.restricted_processor_cannot_source_transfer_from_hidden_location | RED → GREEN | Previously HTTP 200 from hidden source; fix round 1 now pins 403 `LOCATION_ACCESS_DENIED` plus exact location/user details, request pending/no fulfillment/no transfer. The initial round’s 422 was replaced to align with the sibling transfer endpoint. Existing ValidLocationAccess rule added to replenishment action request. Brief StockTransferLocationAccessRuleTest precedent. | Fixed in lane |

## Initial-round verification (historical; fix-round results are recorded below)

All test runs were by explicit file; no full PHPUnit or Vitest suite was run. PG connections, including central, were pinned to **autoerp_test_t**, loopback port **5433**. No test reset was directed at the demo database. PostgreSQL concurrency uses DatabaseMigrations (fresh schema before each test, whole-schema db:wipe and migration-state reset afterwards), committed fixtures and an independent Symfony Process; it does not fake a row lock or rely on a sleep alone. **[Superseded in fix round 2 — see HANDBACK-T1-2026-09-09.md §Fix round 2: the class no longer uses DatabaseMigrations or db:wipe; cleanup is bounded to the fixture tenant and the schema is preserved.]**
...
## Initial-round preflight result
...
## Fix round 1 — case 5 reader identity and side-by-side transcripts (M2)

The three readers compared are **LocationStockQueryService::read** (`incomingTransfer`, destination feed), **StockMatrixQueryService::matrix** (`incoming`, destination cell), and **WeightedAverageCostService::companyOwnedQuantity**, exercised via public **recordCostAdjustment**. `stockDistributionForProduct` is an additional view implemented by the first service; its focused tests are supplementary. WAC has no public HTTP incoming-quantity endpoint. The following are actual **service-call transcripts**, not curl responses, captured on the same fixture per action in isolated PostgreSQL `autoerp_test_t`; the enclosing transaction was rolled back. Each starts with 10 source units, transfers 4, and capitalizes +10 before and after the terminal action.

Calls: `read(tenant, company, destination, null, 1, 50)`; `matrix(tenant, company, [source,destination], '', 1, 50, true)`; `recordCostAdjustment(product, '10.000', 'T1-R1 denominator', tenant, company)`. WAC `owned_quantity` is the returned movement's `quantity_before`; `derived_transit` is **inferred as owned minus physical on-hand**, not an API field. This proves 4 transit units in the denominator without inventing an HTTP accessor.

| Action / phase | LocationStockQueryService::read transcript | StockMatrixQueryService::matrix transcript | WeightedAverageCostService transcript |
|---|---|---|---|
| complete / in_transit | `{"stock":[],"incoming":[{"productId":"5a269516-ae87-4873-9504-439707216255","variantId":null,"incomingTransfer":"4.0000","incomingPo":"0.0000"}]}` | `{"source":{"on_hand":"6.0000","reserved":"0.0000","available":"6.0000","min_quantity":null,"max_quantity":null,"incoming":"0.0000"},"destination":{"on_hand":"0.0000","reserved":"0.0000","available":"0.0000","min_quantity":null,"max_quantity":null,"incoming":"4.0000"}}` | `{"owned_quantity":"10.0000","physical_quantity":"6.0000","derived_transit":"4.0000","avg_cost_before":"5.000000","avg_cost_after":"6.000000"}` |
| complete / complete | `{"stock":[{"productId":"5a269516-ae87-4873-9504-439707216255","variantId":null,"quantity":"4.0000","reserved":"0.0000","available":"4.0000","updatedAt":"2026-09-09T17:29:54+00:00"}],"incoming":[]}` | `{"source":{"on_hand":"6.0000","reserved":"0.0000","available":"6.0000","min_quantity":null,"max_quantity":null,"incoming":"0.0000"},"destination":{"on_hand":"4.0000","reserved":"0.0000","available":"4.0000","min_quantity":null,"max_quantity":null,"incoming":"0.0000"}}` | `{"owned_quantity":"10.0000","physical_quantity":"10.0000","derived_transit":"0.0000","avg_cost_before":"6.000000","avg_cost_after":"7.000000"}` |
| cancel / in_transit | `{"stock":[],"incoming":[{"productId":"273b1d8f-9d8b-48a9-bab3-80716be3ab14","variantId":null,"incomingTransfer":"4.0000","incomingPo":"0.0000"}]}` | `{"source":{"on_hand":"6.0000","reserved":"0.0000","available":"6.0000","min_quantity":null,"max_quantity":null,"incoming":"0.0000"},"destination":{"on_hand":"0.0000","reserved":"0.0000","available":"0.0000","min_quantity":null,"max_quantity":null,"incoming":"4.0000"}}` | `{"owned_quantity":"10.0000","physical_quantity":"6.0000","derived_transit":"4.0000","avg_cost_before":"5.000000","avg_cost_after":"6.000000"}` |
| cancel / cancel | `{"stock":[],"incoming":[]}` | `{"source":{"on_hand":"10.0000","reserved":"0.0000","available":"10.0000","min_quantity":null,"max_quantity":null,"incoming":"0.0000"},"destination":{"on_hand":"0.0000","reserved":"0.0000","available":"0.0000","min_quantity":null,"max_quantity":null,"incoming":"0.0000"}}` | `{"owned_quantity":"10.0000","physical_quantity":"10.0000","derived_transit":"0.0000","avg_cost_before":"6.000000","avg_cost_after":"7.000000"}` |

**Explicitly different concept:** `/api/v1/products/{id}/stock-levels.incoming` counts confirmed purchase-order remainder (`apps/api/app/Modules/Product/Presentation/Controllers/ProductController.php:1073`). In the original curl transcript below it reports `totals.incoming=0.0000` and no destination row while the matrix reports transfer incoming 4.0000. That is a real product-detail visibility gap, not reader agreement. Its fallback `'0.00'` at `:1091` is separately inconsistent with quantity scale. Both are recorded in [T1-5 product-detail incoming ticket](../tickets/2026-09-09-t1-product-detail-transit.md); no production change to that surface is included here.

## Local API evidence (historical snapshots; fixture removed in fix round 1)

The following are actual historical curl request/response transcripts. The synthetic demo product and all its related transfers, movements, stock and requests were deleted in fix round 1 after identity/count checks; the records below are no longer present in the demo tenant. Authentication used the existing demo tenant per `reference_local_db_per_tenant_demo_launch`, with a fresh synthetic T1 product and seeded source stock only. API served from this worktree at **8011**, central `iziposcentral`, tenant resolved from the Bearer claim. Token is redacted. Case 12's curl captures use the web capture endpoint, which exercises the same capture/merge service; its PostgreSQL test explicitly captures with the POS channel and replays the client UUID.

Observed: initial 20 source units; transfer 4 → source 16/incoming 4; complete → destination 4/incoming 0; +10 freight produces WAC **5.500000** across 20 owned units. Duplicate completion and both illegal terminal transitions return **422 INVALID_TRANSFER_STATE**. A second transfer is cancelled back to source. Request collision leaves one pending **5.0000**, count **2** row. The three-reader agreement and WAC-owned transit denominator are established by the focused tests; the original API transcript checks matrix and product WAC output. The product-detail stock-levels endpoint is a DIFFERENT concept: its `incoming` counts confirmed-PO remainder only, not transfer stock in transit. Its zero incoming and missing destination row during transit do not demonstrate agreement with the transfer readers.
...
### Case 5 DIFFERENT CONCEPT: product-detail PO incoming during transfer transit
...
### Case 5 DIFFERENT CONCEPT: product-detail stock after completion
...
### Case 5 DIFFERENT CONCEPT: product-detail stock after cancellation
...
|---|---|
| StockTransferEdgeCasesTest, SQLite | 17 tests, 79 assertions, 2 ticket skips |
| ReplenishmentEdgeCasesTest, SQLite | 9 tests, 57 assertions, 2 ticket skips + PG-only case 12 skip |
| StockTransferCompleteConcurrencyPostgresTest, SQLite | 2 PG-only skips, 0 assertions; skips before schema refresh |
| ReplenishmentActionsTest, SQLite | 16 tests, 67 assertions, no skips |
| StockTransferEdgeCasesTest, PostgreSQL autoerp_test_t | 17 tests, 79 assertions, 2 ticket skips |
| ReplenishmentEdgeCasesTest, PostgreSQL autoerp_test_t | 9 tests, 77 assertions, 2 ticket skips |
| StockTransferCompleteConcurrencyPostgresTest, PostgreSQL autoerp_test_t, final source | 2 tests, 15 assertions, no skips |
| Pint on all four touched PHP files | PASS |
| PHPStan level 8 on all four touched PHP files, including tests | PASS, no suppressions added |
