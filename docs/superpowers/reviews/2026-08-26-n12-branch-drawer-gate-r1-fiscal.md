...
- Fix range `e8a7a2dca..079ef36f5` (5 commits, 12 files, +688/-151). Package `.superpowers/sdd/PLAN/review-e8a7a2dca..079ef36f5.diff`; every claim below re-read at HEAD in the worktree. `task-1-report.md` **was** found this round at the stated path (`.superpowers/sdd/PLAN/task-1-report.md`) — its "Fix round 2" section was read.
- Read-only. No files in the worktree touched.

## VERDICT: spec ✅ + quality APPROVED

All r2 open items ADDRESSED. One new MINOR (not blocking).

## r2 findings — disposition

| r2 finding | Status | Evidence |
|---|---|---|
| F-r2-1 [CRITICAL] step 2 provisions on ambiguous companies; test neutered by `company()` | **ADDRESSED** | The restored `continue` (`…backfill_payment_repository_location_n12.php:225-234`) sits *before* both step 1's attribution loop (`:236-245`) and step 2's provisioning loop (`:247-286`) — an ambiguous company now gets neither. Verified by reading the file directly, not just the diff hunk. Test re-pointed at `companyWithChart()` (`BackfillPaymentRepositoryLocationN12Test.php:171`, which genuinely seeds the chart via `ChartOfAccountsService::seedForCompany` at `:407-419` — not a no-op path) and now asserts `PaymentRepository::query()->where('company_id', $company->id)->count()` === 3 unchanged (`:194-198`). The task report's RED-first evidence (5 repositories, not 3) is consistent with the code before the fix. |
| F-r2-2 [IMPORTANT] census dead code, always zero | **ADDRESSED** | `$ambiguousCompanies++` at `…n12.php:226`, per-company `status=ambiguous-skipped` line with the offending codes at `:227-231`, and the fleet-wide summary line logs `ambiguous_companies` at `:295`. Pinned by `test_it_censuses_the_companies_it_skipped_and_the_codes_that_made_them_ambiguous` (`BackfillPaymentRepositoryLocationN12Test.php:206-242`), which reads real `MessageLogged` events rather than mocking `Log::`, and asserts both the per-company line (carrying `CASH-01`, `CASH-OPERATOR`, company id) and `"ambiguous_companies":1` in the summary line. |
| F-r2-3 [IMPORTANT] cash tender at drawer-less location can resolve to a BANK repository | **ADDRESSED, verified with no residual path** | Traced `TenderRepositoryResolver::resolve()` end to end (`TenderRepositoryResolver.php:97-220`). `$cashTender` (`:155`) gates a `whereIn('type', self::DRAWER_TYPES)` filter on BOTH the mapped-repository lookup (`:163-165`, applied before `find($mappedRepositoryId)` at `:167` — so an operator-mapped bank is invisible to a cash method, not merely deprioritized) and the fallback query (`:194-196`), and since `$cashTender`/`$nonCashTender` are mutually exclusive (`is_cash_tender` is `NOT NULL DEFAULT false`), the `$nonCashTender` branch (`:198-208`, which puts non-drawers first) is never entered for a cash tender — it falls to the default return at `:210-216`, which orders only within the already drawer-filtered set. `scoped()`'s `whereNotIn('type', DRAWER_TYPES)` OR-arm (`:306`) that admits company-wide banks is a top-level-ANDed sub-clause; the outer `whereIn('type', DRAWER_TYPES)` still excludes it, confirmed by reading the generated query shape, not assumed. Also confirmed `PostShiftCashVarianceAdjustment` shares this same resolver by constructor injection (`PostShiftCashVarianceAdjustment.php:13,268`), so the shift-close leg inherits the fix rather than duplicating it. Pinned both directions by `test_a_cash_tender_refuses_rather_than_settling_into_a_bank_account` (`BranchCashRepositoryRoutingTest.php:1200-1227`): refuses with no drawer; falls through to the branch's own drawer when the cash method is explicitly mapped to a bank. |
| F-r2-4a [MINOR] bridge docblock false; refusal names attribution location, not resolution location | **ADDRESSED** | `TreasuryReceiptBridge.php:1317-1318,1338` — the refusal now interpolates `$repositoryLocationId` (the location resolution actually ran against), not `$terminalLocationId`; docblock at `:1595-1608` corrected to describe the DEFAULT-location fallback accurately. |
| F-r2-4b [MINOR] `TerminalController` claims a web handler that does not exist | **ADDRESSED** | Docblock at `TerminalController.php:990-1006` rewritten to state only `apps/pos` translates the code and that `claim()` is a device-only endpoint apps/web never calls. |
| F-r2-4c [MINOR] POS test asserts on source text, not behaviour | **ADDRESSED** | `terminalClaimNoCashRegister.test.tsx` now renders `TerminalSetupPage`, clicks Claim, and asserts the translated message renders (`:1686-1702`) plus a companion case that a different refusal still surfaces the server's raw message (`:1704-1711`) — genuine behavioural assertions, not a `readFileSync`/`toContain` string match. |
| F-r2-4d [MINOR] hand-rolled canonical encoder + stray `terminal_id` literal | **ADDRESSED** | `canonicalEncode()`/`sortRecursive()` deleted from `BranchCashRepositoryRoutingTest.php` (diff `:1296-1318` net removal); replaced by `BuildsPosSaleReceiptEvents::canonicalEncodePosEvent()` (confirmed to exist at `tests/Traits/BuildsPosSaleReceiptEvents.php:209`), and the fixture's `terminal_id` now uses `$this->branchTerminalId` (`:1251`) instead of the unrelated literal. |

Also re-verified clean in this fix diff: no `apps/api/app/Modules/Fiscal/**` production file touched; no canonical-payload, `previous_hash`, `signature_version`, or Event-class change (rule 8) — the only fixture using those stays a test file; no `onQueue(`; no new money/quantity math or bare scale resolution (rule 19); no per-line TTC-vs-HT assertion added. `PosBridgeLocationAttributionTest.php` fixture correctly re-pinned with an honest `is_cash_tender` flag on the CASH method (`:911-915`) and moved the "other location" scenario onto the CARD (non-cash) method (`:944-969`), consistent with the new cash-only-to-drawer rule — a CASH leg could no longer legitimately land at another location's bank, so the test's own subject required this change.

## New findings (fix diff only)

### [MINOR] `apps/api/app/Modules/Treasury/Domain/Enums/RepositoryWriteRefusal.php:7` — a Domain enum imports a Presentation controller

`use App\Modules\Treasury\Presentation\Controllers\PaymentRepositoryController;` is a real `use` import (not a backticked reference) from a `Domain/Enums` file into `Presentation/Controllers`, solely to back a `{@see PaymentRepositoryController}` docblock line (`:15`). This is the same "pint hoists `{@see}` into a real import" trap this very fix round deliberately avoided elsewhere by converting to backticks (`TerminalController.php:66-67`, the migration's `{@see TenderRepositoryResolver}` reference) — here the real import was left in, inverting the hexagonal dependency direction (Domain should have zero dependency on Presentation, per `.claude/context/architecture.md`). Harmless at runtime (PHP enums don't cycle-fail on this), and it's intra-module rather than cross-module, but it's new in this fix round (new file) and inconsistent with the rest of the round's own standard. Fix: backtick the reference, matching the pattern the round already applied twice.

## One line to fix before merge

None blocking — all r2 CRITICAL/IMPORTANT items are addressed and independently verified; optionally backtick the new `RepositoryWriteRefusal` → `PaymentRepositoryController` docblock import before merge to stay consistent with the round's own standard.
