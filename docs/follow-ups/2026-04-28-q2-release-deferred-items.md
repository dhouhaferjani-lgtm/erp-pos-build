...
| **Status** | Done (session4, merge `de4533e2`) — Option 2 (inject `StripSubToleranceDiscountsService`); adapter also corrected to derive `discount_amount` from WO `discount_percent` |
| **Owner** | session4 |
...
| **Status** | Done (session5, merge `bc029598`) |
| **Owner** | session5 |
...
| **Status** | To-do (owned by refund-flow coordination session — touches `TaxCalculationService`; needs accounting input on TN/FR draft-stamp-duty convention) |
| **Owner** | refund-flow coord |
...
| **Status** | Done (session3, PR #61, merge `83529617`) |
| **Owner** | session3 |
...
| **Status** | Done (session5, merge `bc029598`) — agent reversed the doc's recommendation: introduced new backend `App\Modules\Compliance\Application\DTOs\CompanyFraudSettingsData` mirroring live admin payload, deleted hand-written `fraud.ts`, migrated 5 FE consumers, fixed latent `is_configured` gap on `update`/`reset` |
| **Owner** | session5 |
...
| **Status** | Done (session6, PR #63, merge `aec3c45f`) — opt-in trait `Tests\Traits\AssertsApiValidation`; 8 call sites migrated |
| **Owner** | session6 |
...
|---|---|
| **Origin** | Pre-merge audit Cat B Medium |
| **Status** | Done (session4, merge `de4533e2`); backfill staged under `migrations/manual/` awaiting owner sign-off |
| **Owner** | session4 |
...
| **Status** | Done (session-h3, PR #64, merge `90ca7926`) |
| **Owner** | session-h3 |
...
**Status:** Done (session6, PR #63, merge `aec3c45f`). `Carbon::setTestNow()` + `CarbonImmutable::setTestNow()` pinned in `setUp`; both files re-included in `phpunit.xml`; root cause was Eloquent enum casts misresolving without a bound container — fixed by switching parents to `Tests\TestCase`.
...
**Status:** Done (session3, PR #61, merge `83529617`). Triage in [`2026-04-28-taxation-test-triage.md`](2026-04-28-taxation-test-triage.md): 18 re-enable now (endpoints shipped), 5 still blocked (no `PurchaseInvoice` document type yet), 1 owned by refund-flow / M3 session.
...
**Status:** Done (session4, merge `de4533e2`). All 11 errors swept across `tests/Feature/Workshop/`; `phpstan analyse tests/Feature/Workshop/` returns zero errors.
...
|---|---|---|---|---|
| M1 — Workshop converter bypass | Done | session4 | merge `de4533e2` | Option 2 (inject `StripSubToleranceDiscountsService`) — adapter now derives `discount_amount` from WO `discount_percent` and strips before posting; regression suite covers €0.20 strip + €5 above-tolerance preserve |
| M2 — LoyaltyMember morph migration | Done | session5 | merge `bc029598` | `LoyaltyMemberController::show` migrated to `loyaltyable` morph; deprecated `customer()` removed; wire-format byte-stable; contract test pins DTO key set |
| M3 — Stamp duty draft guard | To-do | refund-flow coord | — | Owned by refund-flow coordination session per orchestrator decision; needs accounting input on TN/FR draft-stamp-duty convention |
| M4 — `unit_categories` partial unique | Done | session3 | PR #61 / merge `83529617` | Migration `2026_04_28_120000_fix_unit_categories_partial_unique` + 2 partial unique indexes; test in `tests/Feature/Uom/UnitCategorySystemRowUniquenessTest.php` |
| M5 — FraudSettings shape decision | Done | session5 | merge `bc029598` | New `App\Modules\Compliance\Application\DTOs\CompanyFraudSettingsData` mirrors live admin payload; hand-written `fraud.ts` deleted; 5 FE consumers migrated; latent `is_configured` gap on `update`/`reset` closed in passing |
| H1 — JsonValidationErrors envelope | Done | session6 | PR #63 / merge `aec3c45f` | Trait `Tests\Traits\AssertsApiValidation` reads from `error.errors`; 8 call sites migrated; opt-in (no global override) |
| H2 — `work_order_line_id` whitelist | Done | session4 | merge `de4533e2` | `DocumentLine::$fillable` + `CopiesDocumentData::copyLine()` whitelist + property docblock. **Backfill migration** at `apps/api/database/migrations/manual/2026_04_28_120000_backfill_work_order_line_id_on_document_lines.php` is held under the `manual/` subdir (Laravel won't auto-run); run with `--path=` after owner sign-off |
| H3 — Compliance Rule #6 cleanup | Done | session-h3 | PR #64 / merge `90ca7926` | Contract `App\Shared\Contracts\Compliance\Nf525DataProviderContract` + 19 DTOs published; POS impl at `Modules/POS/Application/Services/Nf525DataProvider`; `Nf525ExportController` + `Nf525JetExportService` + `Nf525XmlBuilder` now zero POS-Domain imports; byte-stable XML locked by `tests/Fixtures/Nf525/jet_export_v1.xml`; refund-flow extension points reserved (`exchangeGroupId`, audit fields, voucher ledger, `instrument*`, Z-report v3 versioned JSONB) |
| H4 — preflight.sh PHPStan memory | Done | session1 | merge `8bb3cca3` | `--memory-limit` restored to `2G` in `scripts/preflight.sh` (commit `4751b648`) |
| L1–L2, L6, L7, L9 | To-do | — | — | Bundle when convenient |
| L3 — Coupon/Promotion date-dependent exclusions | Done | session6 | PR #63 / merge `aec3c45f` | `Carbon::setTestNow()` + `CarbonImmutable::setTestNow()` pinned in `setUp`; `phpunit.xml` exclusions removed; suite green |
| L4 — Skipped Taxation test triage | Done | session3 | PR #61 / merge `83529617` | Triage in [`2026-04-28-taxation-test-triage.md`](2026-04-28-taxation-test-triage.md): 18 re-enable, 5 still blocked (no `PurchaseInvoice` document type yet), 1 owned by refund-flow / M3 session |
| L5 — PHPStan in TechnicianTimeEntryControllerTest | Done | session4 | merge `de4533e2` | All 11 errors across `tests/Feature/Workshop/` swept (Mockery typing + missing array param types + redundant `assertIsString`); `phpstan analyse tests/Feature/Workshop/` returns zero errors |
| L8 — TN backfill `Company::all()` chunking | Done | session1 | merge `8bb3cca3` | `Company::query()->chunk(50, ...)` + `backfillCompany()` extraction (commit `539045bb`) |
| L10 — Node 24 actions migration | Done | session7 | PR #59 / merge `905106f5` | All 5 listed actions bumped to v5; `actions/upload-artifact@v4` covered by `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` env var pending v6 follow-up |
