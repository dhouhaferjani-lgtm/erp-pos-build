...
| H1 — JsonValidationErrors envelope | Done | session6 | TBD | Helper added to `tests/Traits/AssertsApiValidation`; 8 call sites migrated; safe-by-default for `error.errors` |
...
| H3 — Compliance Rule #6 cleanup | In progress | session-h3 | PR #64 | Contract `App\Shared\Contracts\Compliance\Nf525DataProviderContract` + 19 DTOs published; POS impl at `Modules/POS/Application/Services/Nf525DataProvider`; `Nf525ExportController` + `Nf525JetExportService` + `Nf525XmlBuilder` now zero POS-Domain imports; byte-stable XML locked by `tests/Fixtures/Nf525/jet_export_v1.xml`; refund-flow extension points reserved (exchangeGroupId, audit fields, voucher ledger, instrument*, Z-report v3 versioned JSONB) |
...
| L3 — Coupon/Promotion date-dependent exclusions | Done | session6 | PR #63 | `CarbonImmutable::setTestNow()` pin in `setUp`; phpunit.xml exclusions removed; suite green |
