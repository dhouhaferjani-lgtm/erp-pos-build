...
> **Branch:** `chore/dev-go-live-remediation` at the time of writing.
...
| Disposition | **fix-pre-launch** — bind a CompanyContext stub in the test's setUp so the unit test mirrors production wiring. |
| Owner | TBD |
| Re-check date | TBD |
...
| Disposition | **fix-pre-launch** — must resolve before first-tenant. Confirm intended contract with platform owner; either fix code to mint `pos:*` or update test + cashier-side ability checks to consume `tenant:<uuid>` consistently. |
| Owner | TBD |
| Re-check date | TBD |
...
| Disposition | **fix-pre-launch** — bundle with F1 fix; if the new tenant-prefixed ability is intended platform-wide, update Spatie/Sanctum ability checks across the codebase. |
| Owner | TBD |
| Re-check date | TBD |
...
| Root cause | Same code path as F1. |
| Disposition | **fix-pre-launch** — bundle with F1/F2 fix. |
| Owner | TBD |
| Re-check date | TBD |
...
| Disposition | **fix-pre-launch** — small fix: update the test expectation to 422 OR move the check back to route binding. UI side effects must be checked. |
| Owner | TBD |
| Re-check date | TBD |
...
| Disposition | **fix-pre-launch** — workshop is in Otospex scope; appointment mirroring must work end-to-end on the first-tenant pilot. |
| Owner | TBD |
| Re-check date | TBD |
...
| Disposition | **fix-pre-launch** — register the V2 listener (or update the test to reflect three listeners with documented justification). |
| Owner | TBD |
| Re-check date | TBD |
...
| Disposition | **fix-pre-launch** — **BLOCKER** for first-tenant POS pilot. Fiscal-chain byte match cannot be skipped. |
| Owner | TBD |
| Re-check date | TBD |
...
| --- | --- | --- |
| fix-now | 0 | |
| fix-pre-launch | 8 | F1/F2/F3 (auth tokens — HIGH), F5/F6 (workshop listeners — MEDIUM), F7 (fiscal chain bytes — HIGH/BLOCKER), F4 (category status code — LOW), E1 (document conversion test fixture — LOW) |
| accept-with-doc | 0 | |
| defer | 0 | |
