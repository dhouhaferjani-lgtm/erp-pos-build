...
| **Lot evidence** | The provenance of a line's lot identity is `operator_captured`, `system_fefo_estimate` or `unknown`; a FEFO suggestion/allocation must never be displayed as captured physical evidence. | Existing `pos_receipt_line_batch_allocations` / POS with BatchExpiry; the receipt projection remains the primary allocation writer; provenance annotation is proposed in [remediation W-LOT](superpowers/specs/2026-09-05-parapharmacy-readiness-remediation-design.md), and captured-evidence transport/storage remains open under D7. | Existing receipt/lot traceability detail is the primary evidence surface; read-only POS tile/cart guidance and exports consume the same provenance, gated by `BatchExpiry`. | lot provenance, batch evidence |
...
| **Repository** | A place money sits: **cash register (drawer)**, **safe**, **bank account**, virtual. A drawer is attributed to exactly one POS location; registration seeds one drawer + one safe for the FIRST company (`TenantInitializationService`); an additional company created via `POST /api/v1/companies` gets its own drawer + safe since G-3c (dev `9badbe294`, 2026-08-29); a new `pos_enabled` location gets its drawer. **Open gap:** `GET /payment-repositories` is tenant-scoped (company B lists company A's rows — LEDGER D-J0-8) (`LocationController::provisionCashRegisterIfPosEnabled`). | `payment_repositories` (`RepositoryType`) / `Treasury` | Treasury → Repositories | caisse, coffre, drawer, till |
...
| **Session reconciliation** | A separately recorded comparison of sealed device close totals with complete fiscal-event-derived totals, with explicit pending/blocked/mismatch/matched coverage; it is distinct from orphan-session closure, bank reconciliation and the separately gated Treasury repository-balance check. | **Proposed, not shipped:** `pos_session_reconciliations` / POS, written only by proposed `PosSessionReconciliationService`, reusing `ShiftExpectedCashService` for expected cash; [remediation W7](superpowers/specs/2026-09-05-parapharmacy-readiness-remediation-design.md). | Existing shift/Z detail, with append-only reconciliation runs/resolutions; no second close-authoring or automatic money-write surface. | POS close reconciliation |
