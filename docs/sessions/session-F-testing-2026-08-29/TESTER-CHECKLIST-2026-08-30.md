...
> ⚠ **D-J0-2 (P1, hotfix in progress):** posting an opening batch / opening stock in a company that is **not the first** to do so may fail with a **500** (duplicate `OB-` / `INV-OB-` number across companies — `journal_entries.entry_number` is still tenant-wide unique). If hit: report **tenant + company + time** and continue with the non-quantity import path (§A step 5 without quantities) — do **NOT** retry repeatedly. **Sequencing for today: run everything else first; do §A step 5 (quantity import in company 2) and any AR/AP opening batch in a non-first company ONLY after the orchestrator announces "HOTFIX LIVE" (expected ≈ 10:00 Tunis, ~3 min API blip at push).**
