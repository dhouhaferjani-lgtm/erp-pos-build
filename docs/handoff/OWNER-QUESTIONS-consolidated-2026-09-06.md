...
| A10 | **Precision lane assumption A: Eloquent casts stay `decimal:3` after storage widens to `decimal(15,4)`** (writers format at the country preset, max 3 today; 153 test assertions compare 3-decimal literals). A "casts follow storage" lane opens when a 4-decimal country preset is seeded. Brief: `docs/superpowers/plans/2026-09-07-precision-widening-gl-repository-scale-4.md` §1. | precision-4 lane | Recommend: keep casts at 3 for now, tracked in the follow-up ticket | |
