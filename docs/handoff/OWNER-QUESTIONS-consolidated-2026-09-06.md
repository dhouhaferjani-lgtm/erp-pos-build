...
**2026-09-09 later rulings (chat):** accountant NOT in the default seed for PO revert (`purchase-orders.confirm`) — "operational, nothing to do with an accountant" — grantable per tenant; freight default (uncapitalized residual, no journal) CONFIRMED; multi-receiver attribution default CONFIRMED; benchmark `docs/superpowers/reviews/2026-09-09-benchmark-owner-decisions-po-revert-freight-attribution.md`. Owner asked whether confirmed → draft is legitimate at all → benchmark says yes before any downstream document, with an audit trail; ours keeps the guards but erases confirmation evidence and records no actor/reason → ticket `docs/superpowers/tickets/2026-09-09-po-revert-audit-trail.md`.

**2026-09-09 evening rulings (chat, after the T-2/T-3 spec gate r7 / rev 8):** the three §12 owner-open items are **RULED — defaults ACCEPTED**: **OD-1** blind actors receive `BLIND_REQUIRES_COUNTED_RECEIPT` (422) from quantity-less `complete`, the web hides "Receive all" in blind mode; **close authority** = BOTH `inventory.transfers.reconcile` AND `inventory.transfers.close`, both seeded to manager + admin and independently grantable; **OD-4** activation is effective server-side at commit, `visibility_version` is an advisory cache hint, clients converge on their next successful response. Owner framing: "we are still in a green field, nothing to worry about like previous clients" — no legacy-device / legacy-tenant compatibility concern applies to any T-2/T-3 activation or backfill choice. Consequence: the execution plan treats §12 as closed; no `## Owner decisions required` row may remain open at the spec gate.

**2026-09-09 late ruling (chat) — T-2/T-3 spec gate r9 B1, current-stock residual:** owner chose option (b), "the Oracle approach" (benchmark `docs/superpowers/reviews/2026-09-09-benchmark-blind-receiving-on-hand-visibility.md`): **blind receiving hides the expected/sent/remaining quantities of the specific document being received only** — on the mobile app (the intended primary receiving surface) and on the web receipt surfaces. "The goal is not to omit quantities everywhere … everywhere else it's okay": destination on-hand, available and lot stock stay visible under the existing permission model, and current-stock delta inference (incl. the multi-receiver composition with `NOTHING_TO_RECEIVE`) is an ACCEPTED residual. Consequence for rev 10: narrow the §5.0 guarantee to document quantities on transfer/receipt/replenishment/movement/notification surfaces (keep §5.10 movement-feed masking — movement rows are transfer data), add residual R5, make T9 assert absence only on transfer-derived surfaces with positive controls on stock positions, and state that the mobile receipt screen is the primary receiving surface and must be blind by construction (contract commitment in §9, implementation stays M-1).

**2026-09-09 night — W-LOT-A-1a implementation question (Codex Desktop via owner): may legacy NULL-team roles be adopted (tenant_id set) after proving assignments belong to the tenant?** Orchestrator ruling with the owner's delegation: **NO adoption.** NULL-team is the shipped shape of every seeded role (`RolesAndPermissionsSeeder.php:567` `firstOrCreate` without team); Spatie matches `tenant_id IS NULL OR = current` (`vendor/spatie/laravel-permission/src/Models/Role.php:174-180`); database-per-tenant makes ownership unambiguous; adoption would hide roles from console/queue contexts (rule 20) and changes an application-wide lookup contract outside §2 scope. Plan addendum §8.4: delta resolves legacy roles with `(tenant_id IS NULL OR tenant_id = :tenantId)` and preserves the value; only the new marked `general_manager` is tenant-scoped. Text: `docs/handoff/CODEX-PROMPT-WLOTA-1a-ruling-legacy-null-team-roles-2026-09-09.md`. Plan rev 9 fold-in + tenancy-authz gate owed.

**2026-09-10 — T-2/T-3 S1 implementation question (Codex Desktop via owner): may S1 expand to fix movement creation before extracting the helper, because the verbatim move of `markMovementAsTransfer` trips the protected append-only movement ratchet?** Orchestrator ruling (owner delegation, same pattern as the A-1a NULL-team ruling): **APPROVED, bounded** — eliminate the rewrite (create transfer movements with final type + linkage inside `StockAdjustmentService::receive()/issue()` behind a backward-compatible parameter), delete the helper, shrink the DPA baseline (no re-pin, no scanner change), real event labels after a consumer census, T-1 + ratchet regression gate; plan rev 8 §6.7b/§7.2 amendment; reviewed by stock-gl-interaction-reviewer at the S1 gate. Text: `docs/handoff/CODEX-PROMPT-T2T3-S1-ruling-movement-creation-2026-09-10.md`.

**2026-09-10 — W-LOT-A-1a gate r1 M-2 (technician loses `batches.view`):** owner ruled "leave it" — the `technician` role is NOT granted `batches.view` by the delta (automotive role; batch management not needed today). Owner note for later: automotive may need batch management at some point → re-grant then via the normal role-delta path, not now. Consequence: M-2 closes as RULED; the fix round records the deliberate exclusion in the seeder comment and the handback; no owner-open item.

**2026-09-10 — T-2/T-3 S1 implementation gate r1 (inventory-costing M-2) — web status gap:** S1 ships `POST /stock-transfers/{id}/receive` and `/close` unflagged, so a transfer can enter `partially_received` / `closed_*`, but the shipped web still carries a hand-rolled four-member `StockTransferStatus` union (`apps/web/src/features/stock-transfers/types/index.ts:12`): no badge, no filter, no Complete/Close button for the new states until S4, while the remainder keeps inflating incoming readers and the WAC denominator. Options: **(a) RECOMMENDED** — allow a minimal web widening inside S1 (replace the hand-rolled union with the generated one per convention 11, three badge rows, three i18n keys, `canComplete` accepts `partially_received`; adds frontend-conventions-reviewer to the S1 gate); **(b)** keep S1 backend-only and gate the two routes behind a flag or withheld permissions until S4; **(c)** disclose only in the promotion checklist (insufficient per both reviewers). Register: `docs/superpowers/reviews/2026-09-10-t2-s1-impl-gate-r1-inventory.md`. **OPEN — owner ruling requested.**

**2026-09-10 — W-LOT-A-1a implementation gate r2 (tenancy-authz B-2(r2)) — undeclared flag-off change:** `POST /users/{userId}/roles` now returns 422 without an active membership in the current company (`RoleController.php:374-376`), for every tenant at Push 3, with no flag to roll it back; plan §8.8 asks for the rule but §6.1 promises Push 3 is inert. **Orchestrator ruling: gate the requirement behind `activation->enforced()`** (Push-3 dormancy contract wins; the rule activates with the flag). Owner may overturn to "declare it as a third Push-3 exception" — then §6.1, the release note and REALIGNMENT-LOG get the entry instead of a code gate. Register: `docs/superpowers/reviews/2026-09-10-w-lot-a-1a-impl-gate-r2-tenancy.md`. **RULED by orchestrator, overturnable.**

**2026-09-10 — W-LOT-A-1a fix round 2 blocking decision (Codex Desktop via handback): pre-existing per-lot batch transfer failure.** `POST /batches/{uuid}/transfer` returns 500 on PostgreSQL pre-lane (`BatchStockService::transferBatchStock` records batch movements without the NOT NULL `movement_id`; ticket `docs/superpowers/tickets/2026-09-10-batch-transfer-missing-movement-reference.md`). The lane's new real-HTTP regression test for the transfer response is therefore red. **Orchestrator ruling: OUT OF SCOPE for A-1a** (rule 4; the repair belongs to the inventory movement seam T-2 S1 is changing — fixing it here collides at merge). Known-red precedent applied (`T1_RUN_KNOWN_REDS`): the real test is kept but skipped unless `WLOTA1A_RUN_KNOWN_REDS=1`; a sibling test pins the current 500 + rollback to the ticket; ticket scheduled after T-2 S1 merges; no first-party client calls the route today (API/mobile exposure only). Prompt: `docs/handoff/CODEX-PROMPT-WLOTA-1a-fix-round-2b-2026-09-10.md`. **RULED by orchestrator, overturnable** (overturn = fix it inside A-1a, which then needs the inventory-costing + stock-gl reviewers on the batch writer and a merge-order decision against S1).
