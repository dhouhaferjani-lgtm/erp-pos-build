...
| Field | Value |
|---|---|
| Base SHA | `82a032e04e1e0a42d67503fcf2bf91095826e7f8` — **sixth rebase, 2026-08-20**, onto the max-10 manifest commit for the round-13 tag-consumption reallocation (see §8.13); the fifth was the dispatch-red reconciliation tip `9eef1f077` (§8.12), the fourth the manifest-raise commit `28c84edb3` (prior bases `e3eea67f9`, `cea84740c`, `67d18746a`; original `41fb478c2`) |
...
| Final SHA | `e55a1c8aee706237a3222485b74f72e71b274423` — the handover tip after final-gate round 13 (1 operative tag-class finding + 4 record-class notes folded) plus the sixth rebase; **the rebase is not-a-round**, and `fix_rounds` is `10 of 10` on the parent ruling (the handback itself is untracked by contract) |
...
| 2 | `git merge-base --is-ancestor 1e8c0fa03 82a032e04` | exit 0 — **re-derived at the current base** (sixth, tag-reallocation rebase). M0 was originally evaluated at `41fb478c2`, which is an ancestor of `82a032e04`, so the predicate held then and holds now; the row evidences the base it names |
...
| 7 | `commit_series`, `ratchet_trust_model_ack`, `control_manifest` non-null; `docs/handoff/enforcement-control-manifest.yaml` present at base with `sha256 = 7d53797b1849b6adeb2fcb39789935a48fc32a6f4804a9833d4bb8a78f1aa2b7` (re-derived at THIS base: `git show 82a032e04:docs/handoff/enforcement-control-manifest.yaml | shasum -a 256` — the sixth-rebase base IS the max-10 raise, so the digest moves and both mirrors move with it), matching the `control_manifest` pin **and** the dispatch receipt. *(The base contains every raise to date — three owner-side (5→6, 6→8, 8→9) and the parent-side 9→10 under the blanket delegation — so the base value, the pin and the receipt are one number, and the historical-vs-live split this row carried at rounds 9–10 no longer exists.)* | satisfied |
...
pass (commit `47fc8da2c`) so the seed was generated by a scanner whose documentation matches its behaviour.
...
|---|---|---|
| pre | `47fc8da2c` | M1 round-5 P3 closures + the bootstrap-only baseline WRITER (kept out of the seed so the seed commit contains exactly one file) |
| **commit 1 — SEED** | `9cab548f86d10037822eec38eb08035f2a8a8eb2` | `apps/api/tests/Architecture/baselines/document-per-action-baseline.json` — 34 keys, sorted, unique, line-number-free |
| **commit 2 — PINS + CHECKER** | `6ced0d514` | the YAML mirror pins, the ratchet test, and the scan parse-cache |
...
```
dpa_baseline_seed_commit    = 9cab548f86d10037822eec38eb08035f2a8a8eb2
dpa_baseline_protected_blob = 1381983d463e6c546535be907d4aa1c7ca94c597
dpa_baseline_pin_tag        = ci-pin/enforcement-p1-r2   # REALLOCATED at round 13 — r1 was consumed
```

The pin-tag NAME is pre-allocated and non-null in the accepted candidate (gate-r5 R5-C-2). The OWNER
creates the annotated tag at exactly A at promotion step 3; the executor cannot push tags.

**⚠️ THE ORDINAL MOVED AT ROUND 13 — `r1` IS CONSUMED, AND IT STAYS CONSUMED.** The M2-era allocation
evidence (`git tag -l 'ci-pin/enforcement-p1-*'` → 0, `git ls-remote --tags origin 'refs/tags/ci-pin/*'` → 0,
so `n = 1`) was true when it was taken and is **false at this tip**. Under the round-12 ACCEPT the owner ran
the ceremony: the repository variable was set, the annotated tag `ci-pin/enforcement-p1-r1` was pushed at
the then-accepted SHA `d65244fe2`, and the pre-promotion dispatch on that SHA came back **RED** on three
unrelated invisible-red gates (§8.12) — which is what forced the red-run re-gate. Two rebases later,
`d65244fe2` is no longer an ancestor of this tip, so the accepted SHA has moved out from under that tag.

Two rules meet here and neither bends: **pin tags are never deleted and never reused** (gate-r5
R5-C-1/R5-C-2), and **the closing tag must resolve to the accepted A**. So `r1` is left exactly as it
stands — it remains a *true record* of the round-12 acceptance of `d65244fe2`, not an error to be tidied
away — and the next free ordinal is allocated for the new A. Re-derived live at this tip:

```
$ git tag -l 'ci-pin/enforcement-p1-*'
ci-pin/enforcement-p1-r1
$ git ls-remote --tags origin 'refs/tags/ci-pin/*'
44853facf38455b7b243acd26b237226433fa64c        refs/tags/ci-pin/enforcement-p1-r1
d65244fe27d3c0c24f045b820f64f9541e563e01        refs/tags/ci-pin/enforcement-p1-r1^{}
$ git tag -n20 ci-pin/enforcement-p1-r1        # the consumed tag, left untouched
ci-pin/enforcement-p1-r1 enforcement-p1 ACCEPT r12
      accepted_sha: d65244fe27d3c0c24f045b820f64f9541e563e01
      register_sha256: f969dfcbf179d856a028a1507325c24e54f8891001fa4befeb416a9a3e47f1b3
      handback_sha256: a5521c960fe77c298712ff25cd5df38c431c08c744738c2c400f8393ebbe4c1c
      control_sha256: manifest=c57416b8d0385e839b1c801bbf1e2f26f865e3f5b546c5c942593f8fae99ddcb
$ git merge-base --is-ancestor $(git rev-parse ci-pin/enforcement-p1-r1^{}) HEAD; echo $?
1                                              # NOT an ancestor — the accepted SHA moved
=> highest existing ordinal = 1, therefore n = 2 -> ci-pin/enforcement-p1-r2
```

This is the runbook's foreseen **ordinal-re-pin class**, arriving for exactly the reason the runbook
predicted: a ceremony that begins before the accepted SHA is final consumes an ordinal per attempt. The
reallocation lands in its own metadata commit (Phase 4.3.27), and the CI "Fetch the durable baseline pin
tag" step reads the name **from the progress YAML**, so nothing in the workflow needed editing — verified by
executing that step's own bytes (§7.3 layer 2), which now prints `ci-pin/enforcement-p1-r2`.
...
> **Line anchors in this section were produced at `6ced0d514`** (the pins+checker commit). The subsequent
...
```
$ seed_blob=$(git rev-parse 9cab548f8:apps/api/tests/Architecture/baselines/document-per-action-baseline.json)
...
**`ci-pin/enforcement-p1-r2`** (the ordinal moved at round 13 — `r1` is consumed, §6.1) at A **before**
dispatching — otherwise the ratchet step fails closed on the unset
...
**✅ IT RAN, AND IT WAS GREEN — on the round-11 tip. Parent-reported, and reported here as received.**
The owner dispatched the workflow on the previous candidate tip and the parent relayed the result: the
`backend-dpa-guard` job **executed and passed** — its first live execution anywhere — alongside
`deptrac`, `pint`, `phpstan`, `types-drift` and `typecheck`. I have no CI access by contract, so this is
someone else's observation faithfully recorded, not my verification; the authoritative artifact is the run
itself, which the parent digests into `pre_promotion_ci_dispatch`.

Two things follow, and the second is a gate rather than a formality:

1. **What it settles.** No local evidence layer can prove the Actions *event graph* — that the job is
   selected, scheduled and executed rather than merely well-formed. Round 3 proved the converse the hard
   way: every local claim was true while the job could not run at all. That question is now answered from
   the only source that can answer it.
2. **What it does NOT settle.** The same run surfaced three unrelated red gates (§8.12), the parent fixed
   them on dev, and this candidate rebased onto the fix — so **the accepted SHA has moved**. S-14 requires
   the green run to be on *exactly* the accepted SHA, and a run on a superseded tip does not discharge that
   for the new one. **The dispatch must be repeated on the new A before promotion**, and I state it here
   rather than leaving the green result to imply the check is closed.
...
```
$ ./vendor/bin/phpunit tests/Architecture/DocumentPerActionWriteGuardTest.php
OK (6 tests, 8 assertions)   # 10.1 s, 447 MB
  nonzero-test assertion (guard): PASS
$ ./vendor/bin/phpunit tests/Architecture/DocumentPerActionBaselineRatchetTest.php
OK (2 tests, 113 assertions)  # 11.1 s, 447 MB
...
exit: 0
Fetching pin tag: ci-pin/enforcement-p1-r2
(would run) git fetch --no-tags origin refs/tags/ci-pin/enforcement-p1-r2:refs/tags/ci-pin/enforcement-p1-r2
```
...
**Re-derived VERBATIM at `e55a1c8aee706237a3222485b74f72e71b274423` against the NEW base `82a032e04` (the max-10 manifest commit — see §8.13; the fifth-rebase base `9eef1f077` is §8.12, the fourth `28c84edb3` is §8.11)**, after this round's last tracked commit.

```
$ git rev-parse HEAD
e55a1c8aee706237a3222485b74f72e71b274423

$ git diff --stat 82a032e04e1e0a42d67503fcf2bf91095826e7f8..HEAD
 ...
 ...TICKET-dpa-scanner-depth-burndown-2026-08-20.md |   85 +
 docs/handoff/progress/enforcement-p1.progress.yaml |  201 +-
 17 files changed, 5507 insertions(+), 26 deletions(-)
$ path allowlist check — run as a FILTER, not an assertion
$ git diff --name-only 82a032e04e1e0a42d67503fcf2bf91095826e7f8..HEAD | grep -v -e '^apps/api/tests/Architecture/' -e '^\.github/workflows/ci\.yml$' -e '^docs/handoff/' | wc -l
       0
(total files: 17)
...
$ git merge-base --is-ancestor 82a032e04e1e0a42d67503fcf2bf91095826e7f8 HEAD; echo $?
0
```

**✅ ALLOWLIST-CLEAN — the round-10 widening is closed at its root, not restated away.** Rounds 10 and 11
tell one story and it is worth stating plainly because this package spent eleven rounds on exactly this
class. The brief's binding allowlist (gate-r1 M-2) is `apps/api/tests/Architecture/**` ·
`.github/workflows/ci.yml` · `docs/handoff/**`, and any other path is a scope FAIL, not a footnote. The
round-10 fold placed the scanner-depth burn-down ticket at
`docs/superpowers/tickets/2026-08-20-dpa-scanner-depth-burndown.md` — the repo's normal home for tickets,
and outside that allowlist. This handback disclosed the widening prominently at round 10; the defect
round 11 found was narrower and worse: the machine-readable record, `m3_evidence.scope_proof`, still
certified `15 files, all inside … 0 allowlist violations` over a 17-file tree with one path outside — the
YAML doing precisely what §7.4 promised it would not do, in the artifact that lands verbatim in the
closing commit and that P3-M0 later reads as authoritative.

The owner ruled the out-of-allowlist path a **PARENT instruction error**, recorded as such, and directed
the file be moved INSIDE the allowlist rather than the allowlist be widened for it. So at this tip:

- the ticket lives at `docs/handoff/TICKET-dpa-scanner-depth-burndown-2026-08-20.md` (Phase 4.3.23,
  `git mv`, so history follows the file), and states in its own header why it sits in `docs/handoff/`
  and that relocating it to the house directory is a free move once P1 closes;
- blind spot I in the scanner docblock re-points at the new path (comment only — the only line of code
  this round touched);
- `m3_evidence.scope_proof` is re-derived at the true tip and states 17 files / 0 allowlist violations,
  **with the allowlist check run as a `grep -v` FILTER whose output is zero lines** — an assertion that
  cannot be satisfied by restating the globs, because the globs are the filter;
- there is **no allowlist widening anywhere in the record**: not in the YAML, not here.

The stat counts above are the tip-accurate ones. They appear HERE and not in `m3_evidence.scope_proof`
for a structural reason worth naming: that field ships inside the record commit, and a commit cannot
state the `--stat` of a tree that includes itself — the same reason `M3.commit` names the last CODE
commit rather than the handover tip. The handback is untracked by contract and written after the final
commit exists, so it is the only artifact that can carry a self-inclusive stat honestly.
...
| 3 — §6.3's line anchors were pre-fix-commit with no commit pin | §6.3 now states the pastes were produced at the pins commit `6ced0d514` and to check assertion COUNTS, not `:NNN` anchors |
...
Re-run for round 11; every number below is substituted from live output, none carried. The guard,
ratchet and census legs, the PHPStan, Pint and fail-closed legs, and all FIVE sweeps were re-executed at
the exact handover tip `e55a1c8ae` after the sixth rebase — nothing in this section is carried.
...
--- guard ---     OK (6 tests, 8 assertions)     10.3 s, 447 MB
--- ratchet ---   OK (2 tests, 113 assertions)   10.9 s, 447 MB

--- phpstan level 8, CONFIGURED run (phpstan.neon) ---
 [OK] No errors
true exit code, captured under `set -o pipefail`: 0

--- phpstan pointed EXPLICITLY at tests/Architecture (out-of-lane, for disclosure only) ---
  larastan.noModelMake advisories belonging to this package: 4
  errors belonging to OTHER packages' fixtures in the same path: 19

--- pint ---      {"result":"pass"}

--- unset-variable negative case, ISOLATED so the export survives ---
  unset -> exit 1: fail-closed CONFIRMED
  export survived: DPA_BASELINE_PROTECTED_BLOB=1381983d463e6c546535be907d4aa1c7ca94c597
```

**The PHPStan line, stated precisely, because round 10 required the qualification and round 11 found the
YAML mirror still unqualified.** The *configured* run — `php vendor/bin/phpstan analyse --no-progress`,
which `phpstan.neon` scopes to `paths: app/` — is `[OK] No errors` with a **true** exit 0, captured under
`set -o pipefail` rather than read off a pipe (the piped-exit-code mistake this package made once and
reported against itself). Pointing PHPStan at `tests/Architecture` explicitly is NOT a configured lane and
reports **21** raw error lines (counted with an anchored `^/.*\.php:[0-9]+:` match — an earlier draft of
this paragraph said 23, from a loose `grep -c ':'` that also counted two of PHPStan's own note lines; the
re-derivation at this tip caught it), of which:

- **4 are this package's**, all `larastan.noModelMake` at `FixtureJournalEntryWrites.php:190,205,216,227`:
  *"Called `Model::make()` which performs unnecessary work, use `new Model()`"*. Deliberate and
  load-bearing — `q3_make_then_save`, `a_chained_make_then_save`, `d_chained_make_fill_save` and
  `e_aliased_fresh_then_save` exist precisely to pin the `make()` shapes that final-gate rounds 9 and 10
  ruled must be caught, and the fixtures are **parsed, never executed**, so runtime-efficiency advice does
  not apply. Suppressing via `@phpstan-ignore` is forbidden by house rules, so it is disclosed rather than
  hidden, and the count is **stated** (4, up from 1 at round 9) rather than left as "one advisory" after
  three more `make()` fixtures landed.
- **17 belong to other packages' fixtures and tests** in the same directory (`WebhookFixtures/…`,
  `ControllerFixtures/…`, `ControllerTenantContextTest`), pre-existing and outside this package's range.

Because no configured lane analyses `tests/`, **no lane goes red** on either group; the out-of-lane run is
performed only so the disclosure can carry a number.
...
full PHPUnit suite crashes the owner's laptop (house rule). The non-PHPUnit stages this package can affect
were each run directly and pass, with the PHPStan result qualified exactly as above and Pint clean on the
same paths. Nothing in this package touches `apps/web`, `apps/pos`, TypeScript, ESLint or migrations, so
those stages are unaffected by construction.

**Receipt / manifest field-check, simulated against the live artifacts** (the bridge performs the
authoritative version):

| Field | Candidate YAML | Parent receipt / manifest | Match |
|---|---|---|---|
| `base_sha` | `82a032e04e1e0a42d67503fcf2bf91095826e7f8` | receipt `base_sha` — same (the parent re-pins the receipt each rebase; verified against the live file, not assumed) | ✅ |
| `control_manifest.path` | `/Users/houssamr/Projects/syneriva/apps/erp/docs/handoff/enforcement-control-manifest.yaml` | receipt `manifest_path` — same | ✅ |
| `control_manifest.sha256` | `7d53797b1849b6adeb2fcb39789935a48fc32a6f4804a9833d4bb8a78f1aa2b7` | receipt `manifest_sha256`, and `shasum -a 256` of the live file — same | ✅ |
| top-level `max_fix_rounds` | `10` | manifest `p1.max_fix_rounds: "10"` (and receipt `max_fix_rounds: 10`) | ✅ |
| `milestones[M3].fix_rounds` | `10` | ≤ `max_fix_rounds` | ✅ |
| `progress_path` · `final_milestone` · `lenses` | this file · `M3` · `stock-gl-interaction,inventory-costing` | receipt — same | ✅ |
...
directions** (0 added, 0 removed). Therefore **no seed revision and no re-pin**: seed commit `9cab548f8` and
blob `1381983d463e6c546535be907d4aa1c7ca94c597` stand. *(The tag clause this paragraph carried — "the
pre-allocated tag `ci-pin/enforcement-p1-r1` is re-verified free, so the same name carries forward" — was
true at that rebase and is **superseded**: round 13 found the ordinal consumed by the owner ceremony and
reallocated to `ci-pin/enforcement-p1-r2`. See §6.1 and §8.13.)*
...
the same (P3-M0 included), would have broken. Re-pointed to `9cab548f86d10037822eec38eb08035f2a8a8eb2`, the
...
stand. The two-commit topology survives the rebase intact and adjacent: seed `9cab548f8`, then pins
`6ced0d514`.
...
### 8.10 ✅ RESOLVED — the dispatch receipt is re-pinned (again) to the current base

After each rebase this section flagged that the receipt still named the old base and would fail the bridge
closed on its first predicate. **The parent re-pinned it each time, most recently for the fourth rebase.**
The receipt now reads `base_sha: 28c84edb3105624b917a44efa88254cedd617d41` and
`manifest_sha256: c57416b8d0385e839b1c801bbf1e2f26f865e3f5b546c5c942593f8fae99ddcb`, both matching this
candidate's `base_sha` and `control_manifest` mirror, and the full field-check simulated against the live
receipt and manifest passes on every predicate — see §7.6.

### 8.11 ✅ FOURTH REBASE — onto the manifest-raise commit `28c84edb3` (not-a-round)

The owner's FINAL continuation raised the control manifest's `p1 max_fix_rounds` **8 → 9**. That raise is
itself a dev commit — `28c84edb3`, a single manifest-only commit atop the previously frozen tip
`e3eea67f9` (`git diff --name-only e3eea67f9 28c84edb3` → `docs/handoff/enforcement-control-manifest.yaml`,
the one path) — so the accepted-then-stale tip had to be rebased onto it one more time. The parent declares
this the **last possible parent-side dev commit by construction**: any further raise would be post-ACCEPT.
Per the standing ruling the rebase is **not-a-round**; the `fix_rounds` increment this round carries is the
owner ruling's, not the rebase's.

**Mirrors that move together or not at all** (the round-9 lesson, applied without being re-taught):
`base_sha`, `control_manifest.sha256` → `c57416b8d0…`, and top-level `max_fix_rounds` → `9` all landed in
**one** commit, Phase 4.3.22.

**Re-derived, never carried:** the replayed seed commit `7dc0e5ca1` (was `85f0e63f3`) and pins commit
`1781f15b8` (was `2b7dc05a6`) — both superseded one rebase later by §8.12's, and recorded here as the
fourth rebase actually happened rather than as it now reads — the M1/M2 `commit:`/`updated:` pointers, and every M0 predicate — ancestry
both hops, the S0 seam (`StockAdjustmentService.php` is byte-identical across `e3eea67f9..28c84edb3`, so the
anchors `:1769`/`:1784`/`:1785`/`:1788` re-verify unchanged at the base the row now names), and the manifest
digest at base.

**Unchanged and verified:** the protected blob `1381983d463e6c546535be907d4aa1c7ca94c597`
(`git rev-parse HEAD:…/document-per-action-baseline.json`), so **no re-seed, no re-pin, and the owner-set
repository variable does not move**; the pre-allocated tag name — `ci-pin/enforcement-p1-r1` *as it stood
at that rebase; round 13 reallocated it to `r2` after the owner ceremony consumed the ordinal, §8.13*; and
the census
`34 / 67 / 15` with the violation key set byte-identical to the baseline in both directions.

**The record family a rebase disturbs is closed mechanically, not instance by instance.** All four sweeps
were re-run at this tip after the substitutions: SHA-ancestry `0 dangling`, `file:line` anchors
`65/65 carry the claimed write token, 0 stale`, regenerated counts `7 checked, 0 missing`, control-file
digests `3 checked, 0 stale`. That is the whole point of having built them — the fourth rebase cost one
pass of four scripts instead of four rounds of chased instances.

### 8.12 ✅ RED-RUN RE-GATE — rebased onto the dispatch-red reconciliation tip `9eef1f077` (not-a-round)

The owner's pre-promotion dispatch on the round-11 tip produced the first live run of this package's CI job
— **green** (§7.2) — and, in the same run, three **invisible-red** gates that had been failing on dev
unseen, because a direct `dev` push starts no workflow (`ci.yml:3-8`, `:740-747` — the very event-graph
property this package's own §7.1 documents). None of the three belongs to P1. Parent-reported, fixed by the
parent on dev at `9eef1f077` with true exit codes (T6 filter: 83 passed / 0 failed on PG):

| Red gate | Cause |
|---|---|
| The section-14.3 chokepoint manifest anchor (POS sale-receipt lane) | the anchored call had gone multiline; the manifest anchor was never re-reconciled |
| route-manifest drift | the manifest needed regenerating on the merged tree |
| `PendingSealMigrationTest` | asserted a stale schema default — the ruled value moved 2 → 3 in the 2026-07-31 Lane D1 migration, and the test name and assertion were both stale |

Per the red-run protocol this candidate rebases onto that tip. **It is not-a-round** — `fix_rounds` stays
`9 of 9` — because nothing in this package was found wanting; the same standing ruling that covered the
four earlier rebases covers this one.

**The dev fix touches three paths — `apps/api/scripts/saleReceipt-chokepoint-manifest.json`,
`apps/api/tests/Feature/POS/Migrations/PendingSealMigrationTest.php`,
`scripts/factory/manifests/routes-web.yaml` — and none of them is under `app/`,** so the scanner's corpus is
untouched by construction and the census *cannot* move. Re-derived anyway rather than argued: **34 / 67 / 15
(116 sites)**, violation key set byte-identical to the baseline in both directions.

**Re-derived, never carried:** `base_sha`, the replayed seed `9cab548f8` (was `7dc0e5ca1`) and pins
`6ced0d514` (was `1781f15b8`), the M1/M2/M3 `commit:`/`updated:` pointers, and every M0 predicate — both
ancestry hops exit 0, and the S0 seam file is byte-identical across `41fb478c2..9eef1f077`, so the anchors
`:1769`/`:1784`/`:1785`/`:1788` re-verify unchanged at the base the row now names.

**Unchanged and verified:** the protected blob `1381983d463e6c546535be907d4aa1c7ca94c597` — **no re-seed, no
re-pin, and the owner-set repository variable does not move** — the pre-allocated tag name
the pre-allocated tag name — *and here the "unchanged" claim needs correcting, which is round-13 note 1's
sibling: at this rebase the name still read `ci-pin/enforcement-p1-r1`, but that ordinal had **already been
consumed** by the owner ceremony described above (variable set, tag pushed at `d65244fe2`, dispatch RED), so
it was not in fact available to carry forward. Round 13 caught it and reallocated to
`ci-pin/enforcement-p1-r2` (§6.1, §8.13). What IS unchanged is the tag's SUBJECT — the protected blob it
attests — not its name* — **and this time the control manifest digest too**: the fifth-rebase base does not
touch the manifest, so unlike §8.11 neither the `control_manifest` mirror nor `max_fix_rounds` moves.

**Sweeps at this tip:** SHA-ancestry `0 dangling`, `file:line` anchors `65/65 carry the claimed write token,
0 stale`, regenerated counts `7 checked, 0 missing`, control-file digests `3 checked, 0 stale`, and a **new
fifth class** below. The fifth rebase cost one pass of scripts rather than a round of chased instances — and
the re-derivation caught a real number error of my own in §7.6 (`23` errors where the anchored count is
`21`, the loose `grep -c ':'` having counted two of PHPStan's note lines), which is the argument for
mechanical closure in one line.

<!-- sha-sweep:historical-block-start sweep:illustrative-block-start (the SHAs in the paragraph below are QUOTED examples of the defect, not live claims) -->
**⚠️ A NEW DEFECT CLASS THE FIRST FOUR SWEEPS CANNOT SEE — found and closed during this rebase.** Re-pointing
the rewritten SHAs mechanically is the right move, but a blind map **over-applies**: it rewrote the
superseded halves of history statements too, turning §8.11's *"the replayed seed `7dc0e5ca1` (was
`85f0e63f3`)"* into *"`c1bded36a` (was `c1bded36a`)"* and the YAML's fourth-rebase record into a claim about
the fifth. Every rewritten value is a **live ancestor**, so the SHA-ancestry sweep passes on all of them —
the defect is invisible to ancestry by construction, and it corrupts exactly the fields a reviewer reads to
reconstruct what happened.
<!-- sha-sweep:historical-block-end sweep:illustrative-block-end -->

Closed mechanically rather than by care: a **fifth sweep class** asserts the inverse property — a pointer
NAMED as replaced (`(was X)`, `was \`X\``, `seed/pins commit X -> Y`, `superseding X`) must **not** resolve to
an ancestor of the tip, because a superseded pointer is dangling by definition. It reported `12 claims
checked — 0 resolve to a live ancestor` after the four statements above were corrected, and its first draft
also taught the scoping rule: base-to-base statements like *"rebased `67d18746a` -> `cea84740c`"* are
excluded, since a dev base legitimately stays an ancestor. Disclosed here rather than quietly fixed, because
I introduced it in this same rebase.

### 8.13 ✅ SIXTH REBASE + PIN-TAG REALLOCATION — the ordinal-re-pin class, arriving as foreseen

Round 13's operative finding, and the rebase folded with it. The parent raised the control manifest's
`p1 max_fix_rounds` **9 → 10** under the owner's blanket delegation; that raise is a manifest-only dev
commit (`82a032e04`, `git diff --name-only 9eef1f077 82a032e04` → `docs/handoff/enforcement-control-manifest.yaml`,
the single path), so the candidate rebases onto it. **Not-a-round** — the `fix_rounds` increment this round
carries belongs to the ruling, not the rebase.

**Both mirrors moved together this time, in one commit with `base_sha`** (Phase 4.3.26):
`control_manifest.sha256` → `7d53797b18…` and top-level `max_fix_rounds` → `10`. §8.12's rebase was the
opposite case — the manifest was untouched, so neither moved — and getting that distinction right is the
round-9 lesson still holding.

**The finding itself: `ci-pin/enforcement-p1-r1` was CONSUMED, and the record still claimed it was free.**
The full derivation and evidence are in §6.1. In short: the round-12 ACCEPT led to a real ceremony that
reached step 3 — variable set, annotated tag pushed at `d65244fe2` — and then the dispatch came back RED,
which forced the re-gate that moved the accepted SHA out from under that tag. The YAML still read `r1` with
an M2-era justification asserting no such tag existed. Left uncorrected, the *next* ceremony would have
attempted to push a name that already exists at a different commit — a collision the promotion protocol
treats as a re-gate trigger, discovered at the worst possible moment, in the middle of a promotion.

Fixed in a dedicated metadata commit (Phase 4.3.27), the shape the two-phase protocol wants for pin
metadata: `dpa_baseline_pin_tag` → **`ci-pin/enforcement-p1-r2`**, the justification rewritten to record the
consumption, `m2_evidence.pin_tag_allocation` keeping the M2-era derivation as historical beside the round-13
one, and `m3_evidence.owner_prerequisites` naming the new ordinal while stating that **the repository
variable value is unchanged** — the protected blob has not moved across any of the six rebases, so if the
owner already set it during the earlier ceremony, that half needs no action.

**Nothing in the workflow needed editing, and that is by design rather than luck:** the "Fetch the durable
baseline pin tag" step derives the name *from the progress YAML*. Verified by executing that step's own
bytes with the fetch stubbed (§7.3 layer 2) — it now prints `ci-pin/enforcement-p1-r2`.

**`r1` is not cleaned up.** It stays exactly as pushed, a true record of what was accepted at `d65244fe2`.
Its annotation's digests and `control_sha256: manifest=c57416b8d0…` remain correct *for that acceptance*;
they simply describe an earlier tip. Deleting or moving it to tidy the record would destroy evidence and
break the never-reuse rule in the same stroke.

**Unchanged and verified at this base:** the protected blob `1381983d463e6c546535be907d4aa1c7ca94c597` — no
re-seed, no re-pin — and the census **34 / 67 / 15**, violation key set byte-identical to the baseline both
directions. The base adds only the control manifest, so the scan corpus is untouched by construction; the
census was re-derived anyway. **Re-derived, never carried:** seed `9cab548f8` (superseding `c1bded36a`),
pins `6ced0d514` (superseding `78e1f08ac`), the M1/M2/M3 pointers, and every M0 predicate.

**Sweeps at this tip:** SHA-ancestry `82 non-ancestor tokens, 0 dangling`, anchors `65/65, 0 stale`, counts
`7 checked, 0 missing`, control-file digests `3 checked, 0 stale`, superseded pointers `16 checked, 0
resolve to a live ancestor` — the fifth class added in §8.12 earning its place immediately, since this
rebase's mechanical re-point had to skip the `(was X)` halves it would otherwise have corrupted.

**The SHA sweep needed one principled extension, not a loosened threshold.** After the reallocation the
record legitimately names `d65244fe2` seven times — the commit the consumed `r1` tag points at — and that
SHA is non-ancestor **by design**: a pin tag is never deleted and never reused, so its target is a real past
acceptance that a later rebase rewrote. Rather than mark seven sites by hand or relax the check, the sweep
now resolves `git rev-parse <ci-pin/enforcement-p1-*>^{}` for every existing pin tag and treats those exact
SHAs as authenticated historical values. That is a fact read out of a git object, not a keyword heuristic —
the same standard the rest of this package holds itself to. Tokens on lines that say *superseded*,
*superseding* or *consumed* are recognised for the same reason.
...
> ### ✅ `fix_rounds: 10` of `max_fix_rounds: 10` — AUTHORIZED AND MIRRORED; the bridge field-checks clean
>
> The counter has been raised four times, never by me, and each raise is recorded in full in the progress
> YAML beside the M3 row — the first three by the OWNER, the fourth by the PARENT under the owner's
> standing blanket delegation:
>
> | Raise | Ruling field | Owner's words | Scope |
> |---|---|---|---|
> | 5 → 6 | `stop_a_owner_ruling` | *"Authorize continuation i.e. 1"* | ONE continuation for round 8's single Important, with a **HARD RIDER**: any further finding of any class parks the package |
> | 6 → 8 | `stop_a_rescope_ruling` | re-scope after the rider fired | a FRESH 2-round budget for the round-9 detector hole (new substance, not record repair) + the manifest-digest Minor; Codex CLI authorized as a second reviewer |
> | 9 → 10 | `parent_ruling_r13` | parent, under the owner's *"freedom to drive whatever needs to be done until everything checks"* | ONE continuation for round 13's operative **tag-class** finding (the consumed `r1` ordinal) + four record-class notes; the terminal condition carries over unchanged |
> | 8 → 9 | `stop_a_final_continuation_ruling` | *"authorized"* | ONE continuation for round 11's four record findings, plus a **TERMINAL CONDITION**: round 12 accepts even if it surfaces further findings of the self-referential record class (they ship as named notes), while any DIFFERENT-family finding — code, guard, census, control, scope-of-substance — parks the package permanently |
>
> Every raise moves **two mirrors or neither is any use**, because the bridge field-checks both: top-level
> `max_fix_rounds` against the manifest projection, **and** `control_manifest.sha256` against the receipt.
> At the 6 → 8 raise, raising only the first would have left the pin at the pre-raise digest and the gate
> would still have failed closed — a lesson applied without being re-taught at 8 → 9, where `base_sha`, the
> digest `c57416b8d0…` and `max_fix_rounds: 9` all landed in one commit (Phase 4.3.22). The 8 → 9 raise is
> also the first that sat **at** this candidate's base rather than outside its range, because the parent's
> manifest commit `28c84edb3` became the base — see §8.11; the 9 → 10 raise repeats that shape at
> `82a032e04` (§8.13). Simulated against the live receipt and manifest, every field-check predicate passes,
> including `fix_rounds 10 <= max 10` (§7.6).
>
> **On the terminal condition, stated so the record is not read as self-serving:** it is an instrument of
> the ruling authority, not an executor licence. It does not excuse a defect of any other family, and
> round 13's finding-plus-four-notes were the entire authorized scope of this round — nothing else was
> touched beyond the not-a-round rebase re-pin the parent folded in. Round 13 is also the round that proves
> the condition is not a rubber stamp: the gate found something **operative** (a tag collision waiting to
> fire mid-promotion), and it was fixed in code-of-record rather than noted and waved through.
>
> ### The budget history — how the counter got here (HISTORICAL, kept because it was corrected against me)
>
> At final-gate round 7 (D-1a) the counter was corrected from an untrue `3` to `5 of 5`. Final-gate rounds 5
> and 6 had each been a **substantive** CHANGES-REQUIRED that transferred control back to me, and brief
> R7-H-1 and harness step 3 both require incrementing the counter on exactly that transition; they were
> rounds 4 and 5 of 5 and had not been counted. The stale-A rebases remain ruled not-a-round — that ruling
> stands and is unchanged; it simply never covered the substantive rounds that came after them.
>
> Understating this counter on the round where the handback bytes get cryptographically bound into the
> pin-tag annotation would have hidden the package's true position, and the bridge's fail-closed
> `fix_rounds <= max_fix_rounds` check would have kept passing on a false value. It is recorded here rather
> than quietly fixed because it was found against me, not by me.
>
> **Where that leaves the package now, stated plainly because it is the owner's to weigh:** `10 of 10` — at
> the ceiling again, and the ruling for what happens next already stands. Under the TERMINAL CONDITION, round
> 12 accepts even if it surfaces further findings of the **self-referential record class** (they ship as
> named notes in the register); any finding of a **different family** — code, guard, census, control, or
> scope-of-substance — parks the package permanently rather than buying another round.
...
| M3 | round 1 → CHANGES-REQUIRED (documentation-only) · round 2 → CHANGES-REQUIRED (record-honesty) · round 3 → CHANGES-REQUIRED with a **CRITICAL** (the CI job was a bash syntax error) · **round 4 → ACCEPT** at the pre-rebase tip · **stale-A re-gate** (dev moved before promotion; rebased onto `67d18746a`, ruled not-a-round) · round 5 → CHANGES-REQUIRED (post-rebase pointer sweep) · round 6 → CHANGES-REQUIRED (stale census anchor) · round 7 → CHANGES-REQUIRED (this gate record) · round 8 → CHANGES-REQUIRED (pre-rebase workflow-evidence records) — **HARD RIDER armed** · round 9 → CHANGES-REQUIRED (a REAL detector hole: create-by-save on `journal_entries`) → the rider fired → **`blocked_review`** → **owner RE-SCOPE**, fresh 2-round budget, Codex CLI authorized as a second reviewer · **two further stale-A rebases** onto `cea84740c` then the frozen `e3eea67f9`, both ruled not-a-round · round 10 → CHANGES-REQUIRED from BOTH reviewers (chained/aliased create-by-save, verb surface, namespace guard) · **fourth rebase** onto the manifest-raise commit `28c84edb3`, not-a-round · round 11 → CHANGES-REQUIRED (4 record findings) → **owner FINAL continuation + TERMINAL CONDITION** · **round 12 → ACCEPT** at `d65244fe2` under that condition → **the owner ceremony RAN and reached step 3**: repository variable set, annotated tag `ci-pin/enforcement-p1-r1` pushed at `d65244fe2`, and the pre-promotion `workflow_dispatch` executed — `backend-dpa-guard` **GREEN, its first live run**, but the RUN AS A WHOLE was **RED** on three unrelated invisible-red gates, so promotion was blocked at step 3 and never reached step 4a; the parent fixed the three on dev → **fifth rebase** onto `9eef1f077` under the red-run re-gate protocol, not-a-round · round 13 → CHANGES-REQUIRED (1 operative tag-class finding — the consumed `r1` ordinal — + 4 record-class notes) → **parent ruling under the owner's blanket delegation** · **sixth rebase** onto the max-10 manifest commit `82a032e04`, not-a-round · round 14 → PARENT-invoked bridge under the standing terminal condition | PARENT-invoked |
...
| **[MINOR]** M3's `commit:`/`updated:` were two commits behind the handover tip | FIXED — at round 3 they were re-pointed at that round's last CODE commit (pre-rebase `1552e215f`, replayed by the later rebases as `41c71cf37`; the field has been RE-DERIVED every round since and now names `c492765ca`), with the constraint stated inline: a YAML commit cannot contain its own SHA, so the authoritative tip is this header's Final SHA |
...
model instantiation. **So no re-seed and no re-pin:** seed `9cab548f8`, blob `1381983d4` and tag
`ci-pin/enforcement-p1-r1` all stand. *(The tag half of that sentence is historical: the ordinal was
consumed by the owner ceremony after the round-12 ACCEPT and reallocated to `r2` at round 13 — §6.1. The
seed and blob halves still stand, unchanged across all six rebases.)*

**Post-fix verification:** guard `OK (6 tests)` · ratchet `OK (2 tests, 113 assertions)` ·
PHPStan level 8 `[OK] No errors` (the CONFIGURED run — `phpstan.neon` scopes `paths:` to `app/`; the
`make()`-fixture advisories that §7.6 qualifies are out-of-lane and numbered 1 at this round) ·
Pint `pass` · census re-derived independently at
...
**Final-gate round 10 disposition (parent bridge: 1 Important + 3 Minors · Codex CLI second reviewer:
5 High + 1 Medium) — the largest fold in the package:**

| Finding | Source | Disposition |
|---|---|---|
| **[Important / Codex #1]** the round-9 create-by-save fix recognised only a receiver bound to a *simple* variable. Three shapes slipped past it: `JournalEntry::make([...])->save()` (chained, never bound), `JournalEntry::make([...])->fill([...])->save()` (chained through `fill`), and `$a = new JournalEntry; $b = $a; $b->save()` (aliased) | bridge + Codex | **FIXED** by hoisting the freshness test into one shared predicate, `isFreshModelExpr()`, used by every receiver path instead of re-implementing the check per shape: an expression is fresh when it is `new <ContractModel>` or `<ContractModel>::make(...)`, resolved through the same name resolver the rest of the scanner uses. All three shapes pinned as fixtures (`a_chained_make_then_save`, `d_chained_make_fill_save`, `e_aliased_fresh_then_save`) — **RED first**, evidence below |
| **[High / Codex #2]** the write-verb surface was incomplete: seven Eloquent/Builder verbs were not in `WRITE_METHODS`, so a write reached through any of them was invisible — not `not_applicable`, *invisible* | Codex | **FIXED** — the seven verbs added, and four that had no pinned cell given exact fixtures, so the matrix proves each one is really scanned rather than merely listed. `updateOrFail`/`deleteOrFail`/`updateQuietly` were corrected to Model (not Builder) receivers and `forceDeleteQuietly`/`restoreQuietly` given a `SoftDeletes` fixture model, after PHPStan proved the originally-drafted shapes impossible — corrected rather than suppressed |
| **[High / Codex #3]** a scanned file declaring two namespaces would have had its class names resolved against the wrong one | Codex | **FIXED** — new impossibility guard `no_scanned_file_declares_more_than_one_namespace()`; the assumption is now enforced instead of assumed |
| **[Minor ×3 + Medium]** counts and stale `m0_evidence`: the guard test count recorded `4` while the tip ran `6`; the matrix cell count was stale; five `m0_evidence` fields still attested a pre-rebase base | bridge + Codex | **FIXED** — every count re-derived live (`6` tests, **133** cells) and the five fields re-derived at the then-current base. The count sweep gained these as regenerated claims so the class closes mechanically |
| **[Disclosure]** two new blind spots the fold exposed rather than closed | both | **DISCLOSED** as blind spots **I** (argument-position relation creates and clone provenance) and **J** (pairing reachability is not flow-sensitive across closures), each verified **zero live instances** in `app/`, and both given an owner and a burn-down path in `docs/handoff/TICKET-dpa-scanner-depth-burndown-2026-08-20.md` |

**RED-first evidence for the round-10 fold, reproduced live at this tip** — the pre-fold scanner
(`git show ef8267cd7:…/DocumentPerActionWriteScanner.php`, i.e. Phase 4.3.17, the round-9 fix) run against
the CURRENT fixture file, then the scanner at HEAD run against the same file:

```
=== BEFORE (scanner at ef8267cd7, pre round-10 fold) ===
a_chained_make_then_save     line 205  not_applicable save
d_chained_make_fill_save     line 216  not_applicable save
e_aliased_fresh_then_save    line 230  not_applicable save
saveLifecycleOnly            line 129  not_applicable save
saveErasesLinkage            line 140  violation      save

=== AFTER (scanner at HEAD) ===
a_chained_make_then_save     line 205  violation      save
d_chained_make_fill_save     line 216  violation      save
e_aliased_fresh_then_save    line 230  violation      save
saveLifecycleOnly            line 129  not_applicable save
saveErasesLinkage            line 140  violation      save
```

The last two rows are the point of the probe as much as the first three: the two **negative controls** are
byte-identical before and after, so the fix is a targeted reclassification of fresh-receiver saves and not a
blanket "saves are now violations" sweep that would have shown up as a census move.

**The live census did not move — verified at this tip, not assumed:** `34 violations / 67 linked /
15 not_applicable (116 sites)`, violation key set **byte-identical** to the checked-in baseline in both
directions. So, again, **no re-seed and no re-pin**.

**Final-gate round 11 disposition (1 Important + 3 Minors) — all of the self-referential record class:**

| Finding | Disposition |
|---|---|
| **[Important]** `m3_evidence.scope_proof` certified `15 files, all inside … 0 allowlist violations` over a **17**-file tree whose 17th path, the burn-down ticket, sat outside every glob the field named. §7.4 disclosed the widening honestly; the machine-readable record — the artifact that lands verbatim in closing commit C and that P3-M0 reads as authoritative — did exactly what §7.4 promised it would not | **FIXED AT THE ROOT.** The owner ruled the out-of-allowlist path a **PARENT instruction error** and directed the file be moved inside the allowlist rather than the allowlist be widened. `git mv` to `docs/handoff/TICKET-dpa-scanner-depth-burndown-2026-08-20.md` (Phase 4.3.23), blind spot I re-pointed, and `scope_proof` re-derived at the true tip: **17 files, 0 allowlist violations**, with the check run as a `grep -v` **filter** whose output is zero lines — an assertion that cannot be satisfied by restating the globs. No allowlist widening survives anywhere in the record |
| **[Minor]** `m3_evidence.whole_package_rerun` still carried the unqualified PHPStan clause that round 10 had corrected in §7.6 — two words from a clause the same fold edited | **FIXED** — qualified with live numbers: 4 `larastan.noModelMake` advisories at `FixtureJournalEntryWrites.php:190,205,216,227`, `phpstan.neon` scopes `paths:` to `app/` so no configured lane analyses them, and the configured run is a **true** exit 0 captured under `set -o pipefail`. The round-9 historical mention above is qualified in place too |
| **[Minor]** `M3.commit` pointed at a commit two code rounds stale — the third instance of one class in one block (rounds 3 and 7 fired on it before) | **FIXED, and the class closed by rule rather than by substitution:** the field comment now carries the **re-derivation rule** (last commit in `base..HEAD` touching `apps/api/tests/Architecture/**`, re-derived every round, never carried) alongside the structural reason it can never be the handover tip |
| **[Minor]** the round-10 disposition and its RED-first evidence existed only as YAML prose | **FIXED** — this section, in the shape rounds 1–9 use, with the probe output above reproduced live rather than pasted from memory |

**Final-gate round 13 disposition (1 operative finding, tag class · 4 notes, record class):**

| Finding | Disposition |
|---|---|
| **[OPERATIVE — tag class]** `dpa_baseline_pin_tag` still named `ci-pin/enforcement-p1-r1`, and its inline justification still asserted the M2-era derivation (*no such tag exists locally or on origin, so n = 1*) — false at this tip. The owner ceremony under the round-12 ACCEPT had **consumed** that ordinal: variable set, annotated tag pushed at `d65244fe2`, dispatch RED. The next ceremony would have attempted a name that already exists at a different commit | **FIXED at the root, in a dedicated metadata commit** (Phase 4.3.27): reallocated to **`ci-pin/enforcement-p1-r2`** with the allocation re-derived live (`git tag -l` / `git ls-remote` → `r1` at `d65244fe2`; highest ordinal 1 ⇒ n = 2), the justification rewritten to record the consumption, and `r1` left **exactly as it stands** — never deleted, never moved, never reused; it remains a true record of the round-12 acceptance. `m2_evidence.pin_tag_allocation` keeps the M2-era derivation as historical beside the new one; `m3_evidence.owner_prerequisites` names the new ordinal and states that the repository-variable half is **unchanged**. No workflow edit was needed — the CI step reads the name from the YAML, verified by executing that step's own bytes (§7.3 layer 2 now prints `r2`). Full account: §6.1, §8.13 |
| **[Note 1]** the `max_fix_rounds` header comment still called `28c84edb3` *"this candidate's NEW base"* — stale by two rebases | **FIXED** — rewritten to name `82a032e04` as the current base and `28c84edb3` as an ancestor, with the mirror-value history (5 · 6 · 8 · 9 · 10) completed so the next reader does not have to reconstruct it |
| **[Note 2]** the gate-history narrative implied the owner ceremony had not been run, when it had reached **step 3** | **FIXED** — the M3 row now states it plainly: round 12 ACCEPT at `d65244fe2` → variable set, `r1` pushed, dispatch executed with `backend-dpa-guard` **green** but the run **red** overall on three unrelated gates → promotion blocked at step 3, never reaching step 4a → red-run re-gate. §7.2 and §8.12 already carried the substance; the narrative row did not |
| **[Note 3]** deliverable 5 reads `--testsuite=Architecture`; the job runs the two classes **by path**. Disclosed since M3 round 1, never adjudicated | **PARENT-RATIFIED** (recorded in `m3_evidence.suite_scoping` with its reasoning): by-path is the **tighter** contract — exact selection, a nonzero-selected-count assertion under `set -o pipefail` so deleting or renaming a class fails the job rather than silently selecting nothing, and the 14 excluded Architecture classes counted and named in §7.1 decision 1. The deviation is a strengthening, and it is now adjudicated rather than perpetually re-disclosed |
| **[Note 4]** burn-down item 4 — the CI regex would still match if the guard class lost a test | **STAYS OPEN**, ticketed in `docs/handoff/TICKET-dpa-scanner-depth-burndown-2026-08-20.md` item 4. Ruled out of scope for this round; recorded as open rather than silently closed |

**What round 13 did NOT touch:** the scanner (not one line of it — this round's only code-adjacent change is
the pin-tag *name* the CI step reads out of the YAML), the fixture matrix (133/133), the census
(**34 / 67 / 15**, key set byte-identical), the seed commit's content, the protected blob `1381983d4`, and
the repository-variable value. The sixth rebase folded into this round is not-a-round; see §8.13.

**What round 11 did NOT touch:** the scanner's classification behaviour (the one line of code in range is the
blind-spot-I path comment), the fixture matrix (133/133), the census (34/67/15, key set byte-identical), the
seed commit, the protected blob `1381983d4`, and the pre-allocated tag name. The fourth rebase folded into
this round is not-a-round per the standing ruling; see §8.11.
