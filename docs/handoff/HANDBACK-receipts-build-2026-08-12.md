...
- **S-7/S-11 index-options half — DONE.** `ReceiptFilterOptionsController` authorizes the exact read permission, intersects membership scope, includes inactive terminals and v4 authoring state, and derives the latest in-scope cashier snapshot with `ROW_NUMBER() OVER (PARTITION BY cashier_id …)` in SQL rather than materializing all receipts in PHP. `ReceiptFilterOptionsTest` locks the database-reduction query and fail-closed scope.
...
- **A-1/GATE-3/GATE-5/OP-23 — DONE.** The six exact POS child identities and composite compliance identity are in `usePermissions.ts`; every child remains keyed to its route contract and the compliance leaf sits immediately above Settings. `ReceiptPermissionParity.test.ts` locks the source mapping, while the 44-test Sidebar suite now behaviorally renders exact accountant/cashier href sets and proves zero-child groups disappear. OP-23 residual: the two exact-gated fraud pages still have no decided IA home; OQ-10 owns that decision.
...
- **BT-8/9/11/12/16/18 — DONE.** `AccountantReceiptPermissionsTest` now runs the real seeder into the test database and checks the persisted accountant role, including explicit refusal of `pos.process_returns`; the other focused classes are `ReceiptFilterOptionsTest`, `ReceiptAuthorizationTest`, `ReceiptIndexLocationScopeTest`, `PosReceiptsIndexMigrationStructureTest`, and `ReceiptFilterDateBoundaryTest`.
