...
- **Re-verified 2026-07-27 post-multi-location merge** (multiloc §1–§4 landed on origin/dev `43fa87d0d` 2026-07-22 — the original in-flight coordination fence is retired; base off current origin/dev and the overlap is already integrated). Remaining in-flight branch: treasury Phase 5 (`feat/treasury-phase5`, rebase/merge pending) — treasury-only surface, no overlap with the baseline files; standard rebase-and-rescan rule applies if anything collides at merge time.
- ⚠️ The multiloc merge changed two facts vs the 2026-07-22 review (both folded in below): the route-manifest drift is now **4 routes**, and the baseline swapped one entry (`RebalancingView.tsx` in, one `ProductStockLevels.tsx` entry out — still 42 total; see the ticket's 2026-07-22 Addendum).
...
`scripts/factory/check-manifest-drift.sh` currently FAILS preflight on **4** stale routes: `/expenses/analytics`, `/expenses/recurring`, `/inventory/placement` (added 2026-07-13) + `/inventory/stock-by-location` (multiloc merge, 2026-07-22). Run `node scripts/factory/gen-route-manifest.mjs`, verify the diff is exactly those 4 additions (all permission-gated: `expenses.view`, `expense-recurrences.view`, `inventory.view` ×2; POS manifest clean), commit both manifests. If dev has grown NEW drift beyond these 4 by the time you start, verify each new route is permission-gated and include it — list any additions in the Gate 1 report. This MUST land first — later waves run preflight and would red-gate on it.
...
Documents detail pages/line editors (~14), inventory/stock/movement views (~7), stock transfers + goods receipt (~4). **Special case — `RebalancingView.tsx` (`move.quantity`, added to the baseline at the multiloc merge; see ticket Addendum 2026-07-22):** the stock-matrix `RebalanceRow` API does not emit unit `decimal_places`, so the fix REQUIRES the backend endpoint to include unit metadata first, then the standard import-swap to canonical `lib/decimal` `formatQuantity` + `getQuantityDecimals`. Group commits by payload source so backend `quantity_decimals` resource additions amortize — but expect SEVERAL resource touches, not one: the documents cluster spans 6 document types with distinct detail endpoints. Each display site: canonical `formatQuantity` + `getQuantityDecimals(<line/product>)`; add `quantity_decimals` to the serving resource where absent (backend change + PHPUnit by path). Delete each baseline entry with its fix.
