...
Session detail payloads (`show`/`index`/`dashboard`, §3) and the `counter-view` endpoint (the blind counter view mobile polls, line 268) both expose `block_sales`, `ambiguity_window_minutes`, and `includes_zero_stock` — these were added in commit f14a50165. Mobile should read these directly from the session detail response for blocking banner / advisory display (see §3 guidance on zone advisory banner). `scope_filters` (raw, including `zone_ids`/`location_id`) IS also included in the full `show`/`index`/`dashboard` payload, but **not** in `counter-view`.
...
- Data source: the counting-session responses mobile fetches (`show`/`counter-view`, both landing in the session header ~3.1) expose `block_sales` and `ambiguity_window_minutes` fields (commit f14a50165). Mobile should read these directly for banner display. Since zone-scoped counts (the only kind mobile creates) can never have `block_sales: true` anyway (§2.2, rejected by validation), a **zone advisory banner** showing `ambiguity_window_minutes` is the only banner mobile realistically needs to render for its own sessions. A **blocking banner** for a location-wide/full-inventory block (`block_sales: true`) created on web is a POS-device concern (`TerminalResource`, §2.6), not something the mobile counting app needs to render — mobile can display the advisory window instead as guidance for which line edges are in the ambiguity window (see §4 test 3).
