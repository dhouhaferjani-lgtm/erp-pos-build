...
| B-13 | **POS manager-gate leak (POS-screens lane)**: `hasManagerAccess` takes the MAX of the PIN operator's role and the signed-in DEVICE user's role — on the ordinary single-account deployment (owner-logged-in terminal), a cashier PIN clears "manager-only" routes. The lane contained the blind-count consequence (fail-closed concealment on /shift), but `/reports` still shows per-tender takings behind the leaky gate (cashier could approximate expected cash). Ruling wanted: fix the role composition (PIN operator role only), conceal /reports takings under blind mode, or accept documented. |
