...
- **r4** (this commit): Pint drift on the edited test file fixed; the year bound superseded by the STRUCTURAL closure — the per-row marker insert now runs in a savepoint with a `QueryException` catch that counts-and-skips (measured residual it closes: ±16:00…±23:59 offset displacements, 22009), with a `'+20:00'` fixture and `unparseable_invoiced_at` now expecting 7; the migration docblock states the enumeration-vs-closure honesty note; converter docblock cites the real HTTP test; remaining stale citations symbol-ized; ci.yml rationale extended for the migration test; R4-6 comment honesty. PHPStan note for the promoter: explicit-file analysis of the migration reports 4 pre-existing-class findings (1 chokepoint-rule + 3 missingType) — the pre-edit file reports identically and `phpstan.neon` scopes `app/` only, so no CI lane sees them; recorded, not introduced.
