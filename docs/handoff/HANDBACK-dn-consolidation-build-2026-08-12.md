...
### Terminal fix rounds 3 and 4 (parent-executed, 2026-08-20) — promoter-facing record (R4-9)
- **r3** (`362105e16`): `SalesOrderHeaderLockException` + controller rethrow arm, red-proven over HTTP both directions; `safeInvoicedAt` year bound + year-zero fixtures; marker-migration test onto the PG CI lane; DTO/docblock truth corrections; scoped counts; i18n pin.
- **r4** (this commit): Pint drift on the edited test file fixed; the year bound superseded by the STRUCTURAL closure — the per-row marker insert now runs in a savepoint with a `QueryException` catch that counts-and-skips (measured residual it closes: ±16:00…±23:59 offset displacements, 22009), with a `'+20:00'` fixture and `unparseable_invoiced_at` now expecting 7; the migration docblock states the enumeration-vs-closure honesty note; converter docblock cites the real HTTP test; remaining stale citations symbol-ized; ci.yml rationale extended for the migration test; R4-6 comment honesty. PHPStan note for the promoter: explicit-file analysis of the migration reports 4 pre-existing-class findings (1 chokepoint-rule + 2 missingType + 1 nullsafe.neverNull — corrected per M5-terminal-r5) — the pre-edit file reports identically and `phpstan.neon` scopes `app/` only, so no CI lane sees them; recorded, not introduced.
- **r5** (`cb6fb5f82` register): ACCEPT — structural closure red-proven both directions by the reviewer (guard removed → 22009 abort; savepoint removed → 25P02); catch-breadth judged acceptable-and-recorded (six non-invoiced_at failure classes walked, all unreachable; SQLSTATE-narrowing offered as optional); R5-6 recorded: per-row savepoints consume one subtransaction XID per DN (>64 in one tenant txn → pg_subtrans fallback; optional per-chunk savepoint refinement).
