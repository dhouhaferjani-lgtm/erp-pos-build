...
### Task E1 — Demo-tenant browser E2E

- Status: complete (2026-07-12). The local DB-per-tenant stack ran from the isolated Phase 3 worktree with API `:8010`, Vite `:5173`, and the multi-queue worker. Tenant migrations and the roles/permissions reseed were applied before the run.
- Evidence: 13 gitignored screenshots under `docs/sessions/treasury-phase3-e2e/`, numbered in execution order from repository pre-state through the non-owner generic-dashboard widget.
- Drawer → bank: owner `owner@pharmabio.tn` transferred `250.000 TND` from `CASH-01` to `BANK-01`. The success toast rendered; balances moved from `245.595`/`25,326.964` to `-4.405`/`25,576.964`; both movement legs share transfer group `907e8356-d741-4d5d-af30-52726801c86e`; posted `JE-2026-000096` shows Dr `512 Banques` / Cr `53 Caisse`, `250.00` each.
- Drawer → safe same-GL exemption: transferred `50.000 TND` from `CASH-02` to `SAFE-01`. Balances became `150.000` and `5,252.654`; the source movement has transfer group `28c87fe0-b7d8-424b-b27b-e5127ca939c7` and an em-dash journal-entry cell, confirming movements-only behavior.
- Cash-movements report: the cross-GL transfer's `+250.000` and `-250.000` legs rendered with the shared source reference. The complete TND totals row rendered, and combined `BANK-01` + `In` filters narrowed the result set and recomputed totals to `861.140 in / 0.000 out / 861.140 net`.
- Cash-position refresh: repository total remained invariant while the seven-day flow totals increased by both transfer amounts, from `57,368.213 in / 1,393.000 out` to `57,668.213 in / 1,693.000 out`. The widget rendered `55,975.213 TND` available cash with updated type subtotals on owner `/reports` and manager `manager@pharmabio.tn` generic `/dashboard`.
...
- Opus adversarial review: `docs/handoff/gate-reviews-phase3/GATE-4-rc1.md` — `VERDICT: APPROVE`; no BLOCKER/HIGH/MEDIUM/LOW findings, no Fable escalation, and three INFO-only notes. The branch is approved as release-candidate-ready pending the owner-side merge.
