...
6. **Tests.** Re-run EVERY per-directory suite the implementor claims, BY PATH with the DEFAULT vitest pool (never --singleFork — it produces false mass failures via cross-file pollution; if a run hangs >3 min, kill it and `pkill -f 'node (vitest'`, report which file). Never accept reported counts — two false green claims have already been caught (gates 5 and 7). Verify no vitest workers remain afterward.
7. **Metric-evasion audit (mandatory since gate 7).** For every metric that improved, audit the MECHANISM: grep the diff for new indirection that defeats detectors — local alias tables re-exporting `tokens.*` under other names, suppression comments containing detector keywords (`react-hook-form`, rule names), renamed-but-equivalent literals (arbitrary-value classes), or anything that changes what a regex sees without changing what the user sees. A zero achieved by evasion = REJECT (two incidents already: gate-3 arbitrary-rem headers, gate-7 formTokenClasses aliasing).
