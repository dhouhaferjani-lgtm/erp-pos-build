...
`SAFE_DROP` and `CASH_CORRECTION` are declared `PROJECTED` (`FiscalEventCoveragePolicy.php:56-57`) with **live projector arms** (`ZSessionLifecycleProjection.php:32-33`), and the device side is **fully built out but never invoked**: both types sit in the authoring type union (`zSessionAuthoring.ts:91-92`), the payload registry's type list *and* its `IMPLEMENTED_EVENT_TYPES` set (`FiscalEventPayloadRegistry.ts:73-74`, `:101-102`), the engine allow-set (`FiscalEventEngine.ts:233`, `:241-242`) and the engine's payload-**validation** branches (`:893`, `:902`, and the `movement_type` enum at `:3734`). What is missing is a **production caller** — no code path authors either type. *(Corrected at entry gate round 2: the earlier wording said they exist "only" in the type union and the allow-set, which understated the surface. The load-bearing claim — no production caller — is unchanged.)* Today a safe drop travels as a plain `CASH_OUT` (`apps/pos/src/api/cashDrawerApi.ts:177` → `zSessionAuthoring.ts:645`). Safe drop is a standard NF525 cash-control operation and is currently **unauthorable end-to-end**.
