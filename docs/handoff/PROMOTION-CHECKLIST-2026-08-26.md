...
- [ ] **F-W2-14 (PR #210 + fix round 1, 2026-09-07) — `RolesAndPermissionsSeeder` MUST re-run on every already-provisioned tenant DB.** Two NEW permissions ship: `supplier-invoices.manage` (supplier-invoice create/re-match/post + ingestion commit + supplier-invoice attachments) and `payments.pay-supplier` (the AP branch of `POST /payments`). Under database-per-tenant they do not exist in an existing tenant DB until the seeder runs there.
  ...
  - **Behaviour change to announce (gate r2 finding 3):** a till taking a document-less payment from a partner typed `supplier` (e.g. cash back from a supplier) now needs `payments.pay-supplier`. Either grant it, or type the partner `both` — a `both` partner without a supplier-invoice allocation is still an inbound receipt a cashier may take (pinned by `SupplierPaymentPermissionTest::test_cashier_may_take_an_inbound_receipt_from_a_both_typed_partner`).
  ...
  - **Gate r3 finding N2 — the nav hiding above is not just controls.** `nav.supplierInvoices` gates the whole "Supplier invoices" SIDEBAR entry on the server-authoritative `supplier-invoices.manage` alone (no legacy-alias fallback, unlike the sibling `nav.purchaseOrders`/`nav.purchaseQuoteRequests` keys). Until `tenants:seed RolesAndPermissionsSeeder` + `permission:cache-reset` run on a tenant, `supplier-invoices.manage` does not exist there yet, so **even manager and admin — who could see this entry before this PR via the `purchases.view` alias — see no "Supplier invoices" link at all**, not merely a hidden button. The `/purchases/supplier-invoices` route still works by typed URL (`documents.view`), so the page is undiscoverable, not broken, and it fails closed — but do not describe this to an operator as "the button is hidden"; the whole nav entry is gone until the seeder step above runs.
  - **Gate r3 finding N3 — a second undocumented behaviour change from this PR: cashiers can no longer settle a supplier CREDIT NOTE.** `DocumentType::isSupplierSettlement()` (`apps/api/app/Modules/Document/Domain/Enums/DocumentType.php`) now returns true for `SupplierCreditNote` as well as `SupplierInvoice`/`PurchaseOrder`/`PurchaseQuoteRequest`, because a supplier credit note is still a supplier counterparty. Consequence: a cashier allocating a payment to a supplier credit note — money coming IN at the till — now needs `payments.pay-supplier` and gets 403, the same over-deny class as the document-less-payment-to-a-`supplier`-typed-partner case already announced above. This is intended (secure default, same as the rest of F-W2-14) and grantable the same way: grant `payments.pay-supplier` to the role or user, per Settings → Roles.
