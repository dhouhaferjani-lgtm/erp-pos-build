...
The restricted gate creates a disposable API user, uses API-set/verified PIN identity with a temporary exact `contacts.update` role, removes the cashier bootstrap role, proves `contacts.update` present and `partners.update`/`partners.view` absent, then asserts every role/user cleanup response. No seeded user's roles are altered. A seeded cashier was explicitly rejected after live `/auth/me` proved it has `partners.view`.
...
- Focused frontend regression command covering route gates/table, PartnerPicker, PartnerForm, the pure Nature heuristic, BUG-006, VehicleForm, and the session-H helper: **7 files / 89 tests passed**. Existing `act(...)` diagnostics and unmatched-route messages remain warning-only in older tests.
