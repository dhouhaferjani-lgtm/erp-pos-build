...
| Field | Value |
|---|---|
| Base SHA | `28c84edb3105624b917a44efa88254cedd617d41` — **fourth rebase, 2026-08-20**, onto the parent's manifest-raise commit, which the parent declares the LAST possible parent-side dev commit by construction (prior bases `e3eea67f9`, `cea84740c`, `67d18746a`; original `41fb478c2`) |
...
| Final SHA | `d65244fe27d3c0c24f045b820f64f9541e563e01` — the handover tip after final-gate round 11 (four record findings folded; the fourth rebase is not-a-round) (the handback itself is untracked by contract) |
...
| 2 | `git merge-base --is-ancestor 1e8c0fa03 28c84edb3` | exit 0 — **re-derived at the current base** (round 11, fourth rebase). M0 was originally evaluated at `41fb478c2`, which is an ancestor of `28c84edb3`, so the predicate held then and holds now; the row evidences the base it names |
...
| 7 | `commit_series`, `ratchet_trust_model_ack`, `control_manifest` non-null; `docs/handoff/enforcement-control-manifest.yaml` present at base with `sha256 = c57416b8d0385e839b1c801bbf1e2f26f865e3f5b546c5c942593f8fae99ddcb` (re-derived at THIS base: `git show 28c84edb3:docs/handoff/enforcement-control-manifest.yaml | shasum -a 256`), matching the `control_manifest` pin **and** the dispatch receipt. *(The base now IS the manifest-raise commit, so all THREE owner-side raises — 5→6, 6→8, 8→9 — are contained in it and the base value, the pin and the receipt are one number; the historical-vs-live split this row carried at rounds 9–10 no longer exists.)* | satisfied |
...
pass (commit `09e2c328f`) so the seed was generated by a scanner whose documentation matches its behaviour.
...
|---|---|---|
| pre | `09e2c328f` | M1 round-5 P3 closures + the bootstrap-only baseline WRITER (kept out of the seed so the seed commit contains exactly one file) |
| **commit 1 — SEED** | `7dc0e5ca126ef558a4da00cfa021bf3147958b7f` | `apps/api/tests/Architecture/baselines/document-per-action-baseline.json` — 34 keys, sorted, unique, line-number-free |
| **commit 2 — PINS + CHECKER** | `1781f15b8` | the YAML mirror pins, the ratchet test, and the scan parse-cache |
...
```
dpa_baseline_seed_commit    = 7dc0e5ca126ef558a4da00cfa021bf3147958b7f
...
> **Line anchors in this section were produced at `1781f15b8`** (the pins+checker commit). The subsequent
...
```
$ seed_blob=$(git rev-parse 7dc0e5ca1:apps/api/tests/Architecture/baselines/document-per-action-baseline.json)
...
**Re-derived VERBATIM at `d65244fe27d3c0c24f045b820f64f9541e563e01` against the NEW base `28c84edb3` (the manifest-raise commit — see §8.11)**, after this round's last tracked commit. Every number below is substituted from live output; none is carried.

```
$ git rev-parse HEAD
d65244fe27d3c0c24f045b820f64f9541e563e01

$ git diff --stat 28c84edb3105624b917a44efa88254cedd617d41..HEAD
 ...
 docs/handoff/progress/enforcement-p1.progress.yaml |  172 +-
 17 files changed, 5478 insertions(+), 26 deletions(-)
$ path allowlist check — run as a FILTER, not an assertion
$ git diff --name-only 28c84edb3105624b917a44efa88254cedd617d41..HEAD | grep -v -e '^apps/api/tests/Architecture/' -e '^\.github/workflows/ci\.yml$' -e '^docs/handoff/' | wc -l
       0
(total files: 17)
...
$ git merge-base --is-ancestor 28c84edb3105624b917a44efa88254cedd617d41 HEAD; echo $?
0
```
...
| 3 — §6.3's line anchors were pre-fix-commit with no commit pin | §6.3 now states the pastes were produced at the pins commit `1781f15b8` and to check assertion COUNTS, not `:NNN` anchors |
...
ratchet and census legs were re-executed at the exact handover tip `d65244fe2`; the PHPStan, Pint and
fail-closed legs ran one commit earlier, on a tree whose ONLY difference is the progress-YAML record
commit that this handback accompanies — no scanned or analysed path differs between them. Said plainly
...
--- guard ---     OK (6 tests, 8 assertions)      9.0 s, 447 MB
--- ratchet ---   OK (2 tests, 113 assertions)    9.9 s, 447 MB
...
- **19 belong to other packages' fixtures** in the same directory (`WebhookFixtures/…`), pre-existing and
  outside this package's range.
...
|---|---|---|---|
| `base_sha` | `28c84edb3105624b917a44efa88254cedd617d41` | receipt `base_sha` — same | ✅ |
...
directions** (0 added, 0 removed). Therefore **no seed revision and no re-pin**: seed commit `7dc0e5ca1` and
...
the same (P3-M0 included), would have broken. Re-pointed to `7dc0e5ca126ef558a4da00cfa021bf3147958b7f`, the
...
stand. The two-commit topology survives the rebase intact and adjacent: seed `7dc0e5ca1`, then pins
`1781f15b8`.
...
`1781f15b8` (was `2b7dc05a6`), the M1/M2 `commit:`/`updated:` pointers, and every M0 predicate — ancestry
...
| **[MINOR]** M3's `commit:`/`updated:` were two commits behind the handover tip | FIXED — at round 3 they were re-pointed at that round's last CODE commit (pre-rebase `1552e215f`, replayed by the later rebases as `2d4dfb87c`; the field has been RE-DERIVED every round since and now names `e259405f5`), with the constraint stated inline: a YAML commit cannot contain its own SHA, so the authoritative tip is this header's Final SHA |
...
model instantiation. **So no re-seed and no re-pin:** seed `7dc0e5ca1`, blob `1381983d4` and tag
...
(`git show 38a9595ef:…/DocumentPerActionWriteScanner.php`, i.e. Phase 4.3.17, the round-9 fix) run against
the CURRENT fixture file, then the scanner at HEAD run against the same file:

```
=== BEFORE (scanner at 38a9595ef, pre round-10 fold) ===
