...
| Field | Value |
|---|---|
| Base SHA | `9eef1f07731ed47193e743b9f5437163f37dec06` — **fifth rebase, 2026-08-20**, onto the dispatch-red reconciliation tip under the RED-RUN RE-GATE protocol (see §8.12); the fourth was the parent's manifest-raise commit `28c84edb3` (prior bases `e3eea67f9`, `cea84740c`, `67d18746a`; original `41fb478c2`) |
...
| Final SHA | `c5408d568eea711b1003e0e11bb340a73cddc6f2` — the handover tip after final-gate round 11 (four record findings folded) plus the red-run re-gate rebase; **both rebases are not-a-round**, so `fix_rounds` stays `9 of 9` (the handback itself is untracked by contract) |
...
| 2 | `git merge-base --is-ancestor 1e8c0fa03 9eef1f077` | exit 0 — **re-derived at the current base** (fifth, red-run rebase). M0 was originally evaluated at `41fb478c2`, which is an ancestor of `9eef1f077`, so the predicate held then and holds now; the row evidences the base it names |
...
| 7 | `commit_series`, `ratchet_trust_model_ack`, `control_manifest` non-null; `docs/handoff/enforcement-control-manifest.yaml` present at base with `sha256 = c57416b8d0385e839b1c801bbf1e2f26f865e3f5b546c5c942593f8fae99ddcb` (re-derived at THIS base: `git show 9eef1f077:docs/handoff/enforcement-control-manifest.yaml | shasum -a 256` — the fifth-rebase base does not touch the manifest, so the digest and both mirrors are unchanged), matching the `control_manifest` pin **and** the dispatch receipt. *(The base contains the manifest-raise commit `28c84edb3`, so all THREE owner-side raises — 5→6, 6→8, 8→9 — are contained in it and the base value, the pin and the receipt are one number; the historical-vs-live split this row carried at rounds 9–10 no longer exists.)* | satisfied |
...
pass (commit `4dfbb000f`) so the seed was generated by a scanner whose documentation matches its behaviour.
...
|---|---|---|
| pre | `4dfbb000f` | M1 round-5 P3 closures + the bootstrap-only baseline WRITER (kept out of the seed so the seed commit contains exactly one file) |
| **commit 1 — SEED** | `c1bded36ad05bee4b38f13a4e381a715128417da` | `apps/api/tests/Architecture/baselines/document-per-action-baseline.json` — 34 keys, sorted, unique, line-number-free |
| **commit 2 — PINS + CHECKER** | `78e1f08ac` | the YAML mirror pins, the ratchet test, and the scan parse-cache |
...
```
dpa_baseline_seed_commit    = c1bded36ad05bee4b38f13a4e381a715128417da
...
> **Line anchors in this section were produced at `78e1f08ac`** (the pins+checker commit). The subsequent
...
```
$ seed_blob=$(git rev-parse c1bded36a:apps/api/tests/Architecture/baselines/document-per-action-baseline.json)
...
**Re-derived VERBATIM at `c5408d568eea711b1003e0e11bb340a73cddc6f2` against the NEW base `9eef1f077` (the dispatch-red reconciliation tip — see §8.12; the fourth-rebase base `28c84edb3` is §8.11)**, after this round's last tracked commit. Every number below is substituted from live output; none is carried.

```
$ git rev-parse HEAD
c5408d568eea711b1003e0e11bb340a73cddc6f2

$ git diff --stat 9eef1f07731ed47193e743b9f5437163f37dec06..HEAD
 ...
 docs/handoff/progress/enforcement-p1.progress.yaml |  174 +-
 17 files changed, 5480 insertions(+), 26 deletions(-)
$ path allowlist check — run as a FILTER, not an assertion
$ git diff --name-only 9eef1f07731ed47193e743b9f5437163f37dec06..HEAD | grep -v -e '^apps/api/tests/Architecture/' -e '^\.github/workflows/ci\.yml$' -e '^docs/handoff/' | wc -l
       0
(total files: 17)
...
$ git merge-base --is-ancestor 9eef1f07731ed47193e743b9f5437163f37dec06 HEAD; echo $?
0
```
...
| 3 — §6.3's line anchors were pre-fix-commit with no commit pin | §6.3 now states the pastes were produced at the pins commit `78e1f08ac` and to check assertion COUNTS, not `:NNN` anchors |
...
ratchet and census legs, the PHPStan, Pint and fail-closed legs, and the four sweeps were ALL re-executed
at the exact handover tip `c5408d568` after the red-run rebase — nothing in this section is carried from
the pre-rebase run. Said plainly
rather than rounded up, because "re-run at the tip" is exactly the kind of claim this package has had
corrected against it twice.
...
--- guard ---     OK (6 tests, 8 assertions)      9.8 s, 447 MB
--- ratchet ---   OK (2 tests, 113 assertions)   10.5 s, 447 MB
...
|---|---|---|---|
| `base_sha` | `9eef1f07731ed47193e743b9f5437163f37dec06` | receipt `base_sha` — same (the parent re-pinned the receipt for the red-run re-gate; verified against the live file, not assumed) | ✅ |
...
| `control_manifest.sha256` | `c57416b8d0385e839b1c801bbf1e2f26f865e3f5b546c5c942593f8fae99ddcb` | receipt `manifest_sha256`, and `shasum -a 256` of the live file — same | ✅ |
| top-level `max_fix_rounds` | `9` | manifest `p1.max_fix_rounds: "9"` (and receipt `max_fix_rounds: 9`) | ✅ |
| `milestones[M3].fix_rounds` | `9` | ≤ `max_fix_rounds` | ✅ |
...
directions** (0 added, 0 removed). Therefore **no seed revision and no re-pin**: seed commit `c1bded36a` and
...
the same (P3-M0 included), would have broken. Re-pointed to `c1bded36ad05bee4b38f13a4e381a715128417da`, the
...
stand. The two-commit topology survives the rebase intact and adjacent: seed `c1bded36a`, then pins
`78e1f08ac`.
...
repository variable does not move**; the pre-allocated tag name `ci-pin/enforcement-p1-r1`; and the census
...
**Re-derived, never carried:** `base_sha`, the replayed seed `c1bded36a` (was `7dc0e5ca1`) and pins
`78e1f08ac` (was `1781f15b8`), the M1/M2/M3 `commit:`/`updated:` pointers, and every M0 predicate — both
...
`ci-pin/enforcement-p1-r1`, **and this time the control manifest digest too**: the fifth-rebase base does not
...
> ### ✅ `fix_rounds: 9` of `max_fix_rounds: 9` — AUTHORIZED AND MIRRORED; the bridge field-checks clean
>
> The counter has been raised three times, each time by the OWNER and never by me, and each raise is
> recorded verbatim in the progress YAML beside the M3 row:
>
...
> also the FIRST that sits **at** this candidate's base rather than outside its range, because the parent's
> manifest commit `28c84edb3` became the base — see §8.11. Simulated against the live receipt and manifest,
> every field-check predicate passes, including `fix_rounds 9 <= max 9` (§7.6).
>
> **On the terminal condition, stated so the record is not read as self-serving:** it is an owner
> instrument, not an executor licence. It does not excuse a defect of any other family, and the four items
> of round 11 were the entire authorized scope of this round — nothing else was touched beyond the
> not-a-round rebase re-pin the parent folded in.
>
...
>
> **Where that leaves the package now, stated plainly because it is the owner's to weigh:** `9 of 9` — at
> the ceiling again, and the owner has already ruled what happens next. Under the TERMINAL CONDITION, round
...
| M3 | round 1 → CHANGES-REQUIRED (documentation-only) · round 2 → CHANGES-REQUIRED (record-honesty) · round 3 → CHANGES-REQUIRED with a **CRITICAL** (the CI job was a bash syntax error) · **round 4 → ACCEPT** at the pre-rebase tip · **stale-A re-gate** (dev moved before promotion; rebased onto `67d18746a`, ruled not-a-round) · round 5 → CHANGES-REQUIRED (post-rebase pointer sweep) · round 6 → CHANGES-REQUIRED (stale census anchor) · round 7 → CHANGES-REQUIRED (this gate record) · round 8 → CHANGES-REQUIRED (pre-rebase workflow-evidence records) — **HARD RIDER armed** · round 9 → CHANGES-REQUIRED (a REAL detector hole: create-by-save on `journal_entries`) → the rider fired → **`blocked_review`** → **owner RE-SCOPE**, fresh 2-round budget, Codex CLI authorized as a second reviewer · **two further stale-A rebases** onto `cea84740c` then the frozen `e3eea67f9`, both ruled not-a-round · round 10 → CHANGES-REQUIRED from BOTH reviewers (chained/aliased create-by-save, verb surface, namespace guard) · **fourth rebase** onto the manifest-raise commit `28c84edb3`, not-a-round · round 11 → CHANGES-REQUIRED (4 record findings) → **owner FINAL continuation + TERMINAL CONDITION** · **owner dispatch on that tip: `backend-dpa-guard` GREEN (first live run)**, three unrelated invisible-red gates fixed on dev → **fifth rebase** onto `9eef1f077` under the red-run re-gate protocol, not-a-round · round 12 → PARENT-invoked bridge under the terminal condition | PARENT-invoked |
...
| **[MINOR]** M3's `commit:`/`updated:` were two commits behind the handover tip | FIXED — at round 3 they were re-pointed at that round's last CODE commit (pre-rebase `1552e215f`, replayed by the later rebases as `467898aa9`; the field has been RE-DERIVED every round since and now names `7c2229d19`), with the constraint stated inline: a YAML commit cannot contain its own SHA, so the authoritative tip is this header's Final SHA |
...
model instantiation. **So no re-seed and no re-pin:** seed `c1bded36a`, blob `1381983d4` and tag
...
(`git show 3fc411ed7:…/DocumentPerActionWriteScanner.php`, i.e. Phase 4.3.17, the round-9 fix) run against
the CURRENT fixture file, then the scanner at HEAD run against the same file:

```
=== BEFORE (scanner at 3fc411ed7, pre round-10 fold) ===
