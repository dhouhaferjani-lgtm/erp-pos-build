...
| Field | Value |
|---|---|
| Base SHA | `41fb478c21ae010568036aa66ffaef726ec63f09` — *"enforcement-p1: parent pre-dispatch pins (dpa_3c_merge/reviewed SHAs with verified ancestry, commit_series Phase 4.m.s, trust ack, control_manifest) — 3D landed at e4dcf0936 opens the P1 slot"* |
...
| Final SHA | `66c695a2b571fccacede3f4bb9ce9042b3b04d13` — the handover tip A (the handback itself is untracked by contract) |
...
| `apps/api/tests/Architecture/DocumentPerActionFixtures/**` | Six fixture classes, one method per matrix cell. |
...
**114 pinned cells** in `fixtureMatrix()`, enforced by four gates: the classification matrix, the
positive-coverage gate, the negative-control gate, and the rule-surface agreement gate. Cells added after
the first draft of this table, by gate round: round 2 — nullable-linkage ×4, unreadable MUTATE ×2,
nullable-call erasure ×1, non-nullable-property boundary ×1, relation-mediated ×2, model-internal ×3,
variant-regrain ×1, stub-bypass ×1; round 3 — alias shapes ×3, `updateOrInsert` ×3; round 4 — mass-assignment
erasure ×4 (`fill`, unreadable `forceFill`, `setAttribute`, lifecycle-only control), two-arg merge order ×1,
destructured null ×1, top-level/route-closure ×3.
...
| **V10** — POS scrap return | `ReceiptReturnService.php:1330/1334` → **linked**; `ReturnScrapWriteOffService.php:165` → **violation #29** | Remediated for the write path; the guard additionally surfaces the post-hoc `is_historical` mutation of the returned movement, which is new (append-only rule) |
...
**Violations the DPA register never covered — the COMPLETE list** (17 of the 34; the rest map to register
items or S0 residues in the table above):
...
| 16 | `ResetOpeningBalanceService.php:137` | same shape on the reversal path |
...
| 33 | `RepositoryTransferService.php:105` | **live `journal_entries` ROW DELETE — see §3.8** |
| 20–28 | `WeightedAverageCostService.php` ×9 | the largest cluster: reference params still `?string`, so every movement create and its paired level writes are unlinked |
...
|---|---|---|
| pre | `1b1f99e00` | M1 round-5 P3 closures + the bootstrap-only baseline WRITER (kept out of the seed so the seed commit contains exactly one file) |
| **commit 1 — SEED** | `ff5642f87998e458059694f256d4033636809605` | `apps/api/tests/Architecture/baselines/document-per-action-baseline.json` — 34 keys, sorted, unique, line-number-free |
| **commit 2 — PINS + CHECKER** | `9946b10b9` | the YAML mirror pins, the ratchet test, and the scan parse-cache |
...
> **Line anchors in this section were produced at `9946b10b9`** (the pins+checker commit). The subsequent
...
```
$ seed_blob=$(git rev-parse ff5642f87:apps/api/tests/Architecture/baselines/document-per-action-baseline.json)
...
1198:    needs: [backend-lint, backend-analyse, backend-architecture, backend-dpa-guard, backend-test, …]
```
...
**Layer 1 — YAML validity.** `actionlint` is **NOT INSTALLED** on this machine (`which actionlint` → not
found), recorded explicitly as the contract requires; fell back to a strict YAML parse. This layer is
**insufficient** for the Actions job/event graph by contract — it proves syntax, never that a job triggers.
...
# the pin-tag extraction the job performs, run against the real YAML:
extracted PIN_TAG='ci-pin/enforcement-p1-r1'
  extraction: MATCHES the pre-allocated tag name
```
...
```
$ git diff --stat 41fb478c2..HEAD
 .github/workflows/ci.yml                                        |   92 +-
 apps/api/tests/Architecture/DocumentPerActionBaselineRatchetTest.php |  257 +
 apps/api/tests/Architecture/DocumentPerActionFixtures/*.php     | 1428 +
 apps/api/tests/Architecture/DocumentPerActionWriteGuardTest.php |  389 +
 apps/api/tests/Architecture/Support/DocumentPerActionWriteScanner.php | 2461 +
 apps/api/tests/Architecture/Support/write-document-per-action-baseline.php | 53 +
 apps/api/tests/Architecture/baselines/document-per-action-baseline.json | 36 +
 docs/handoff/progress/enforcement-p1.progress.yaml              |   63 +-
 15 files changed, 4761 insertions(+), 18 deletions(-)

$ path allowlist check
  every path is inside apps/api/tests/Architecture/** | .github/workflows/ci.yml | docs/handoff/**
  — 0 violations

$ control-file preflight (brief §5 step 1(iii))
  none touched: PASS
  (checked: adversarial-review.sh, adversarial-review-final.sh, the brief, SELF-REVIEW-HARNESS.md,
   enforcement-control-manifest.yaml, .claude/agents/*-reviewer.md, any *control-manifest* surrogate)

$ production code touched?
  none: PASS — guard-only, DO-NOT-TOUCH respected (no baselined violator's code was "fixed while here")

$ workflow permissions grant check (gate-r6 R6-H-5)
  no permissions grant added: PASS (no `contents: write` or equivalent in the workflow diff)
```
...
| 3 — §6.3 line anchors were pre-fix-commit with no commit pin | §6.3 now states the pastes were produced at `9946b10b9` and to check counts, not anchors |
...
Nothing asserts that `DocumentPerActionBaselineRatchetTest.php` exists; deleting it (or it plus the baseline)
leaves the Architecture suite green with no ratchet. Same residual class the brief's §6 F-8 discloses for
`ci.yml`, with the same mitigations (scope allowlist, milestone gates reviewing every diff, the owner's
pre-promotion dispatch run). Disclosed in the ratchet docblock too.
