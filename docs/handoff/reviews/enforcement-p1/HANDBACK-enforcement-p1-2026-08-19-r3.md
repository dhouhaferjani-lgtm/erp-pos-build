...
| Final SHA | `059f9a662922837bf7dda5feb893aae4a8dee454` — the handover tip after final-gate fix round 2 (the handback itself is untracked by contract) |
...
pass (commit `1b1f99e00`) so the seed was generated by a scanner whose documentation matches its behaviour.
...
**Re-derived VERBATIM at `059f9a662922837bf7dda5feb893aae4a8dee454` — the exact tip this handover ships**
(final gate round 2, finding 1). The previous version of this block attested the pre-fix-round tree: it was
written before the round-1 fix commits and never re-derived when the diff changed underneath it, which is
precisely the failure a scope proof exists to prevent. It is now produced AFTER the last tracked commit of
this round, so it certifies its own tip rather than an earlier one.

```
$ git rev-parse HEAD
059f9a662922837bf7dda5feb893aae4a8dee454

$ git diff --stat 41fb478c21ae010568036aa66ffaef726ec63f09..HEAD
 ...
 .../Support/DocumentPerActionWriteScanner.php      | 2461 ++++++++++++++++++++
 .../Support/write-document-per-action-baseline.php |   53 +
 .../baselines/document-per-action-baseline.json    |   36 +
 docs/handoff/progress/enforcement-p1.progress.yaml |   90 +-
 15 files changed, 4786 insertions(+), 24 deletions(-)

$ path allowlist check
0 violations across 15 files
  (every path inside apps/api/tests/Architecture/** | .github/workflows/ci.yml | docs/handoff/**)

$ control-file preflight (brief §5 step 1(iii))
none touched: PASS
  checked: adversarial-review.sh · adversarial-review-final.sh · the brief ·
  SELF-REVIEW-HARNESS.md · enforcement-control-manifest.yaml · .claude/agents/*-reviewer.md ·
  any *control-manifest* surrogate

$ production code
none: PASS
  (checked apps/api/app/** · apps/web/** · apps/pos/** · apps/api/database/** — guard-only,
   DO-NOT-TOUCH respected: no baselined violator's code was "fixed while here")

$ workflow permissions grant check (gate-r6 R6-H-5)
no permissions grant added: PASS
  (no `permissions:` block and no `contents: write` in the workflow diff)
```

Two stat lines move between the round-1 handover and this one, and both are this fix round's own work:
`DocumentPerActionWriteGuardTest.php` 389 → 393 (the dead-variable removal and the narrowed docblock) and
`enforcement-p1.progress.yaml` 63 → 90 (the two fix-round records). Totals 4761/18 → 4786/24. Nothing else
changed, and nothing left the allowlist.
...
| M3 | final gate round 1 → CHANGES-REQUIRED (documentation-only), fix round 1 applied; round 2 → CHANGES-REQUIRED (record-honesty only), fix round 2 applied; re-handed over | PARENT-invoked |
