...
| T-c mirror mismatch | `VerifyEventChainCommandTest::seedReceiptMirrorTamper()` at `apps/api/tests/Feature/Fiscal/VerifyEventChainCommandTest.php:584`; contract test at `:473` | receipt references the intended internally valid fiscal event and mirrors its canonical bytes; its receipt-chain `previous_hash` matches the prior receipt; only its valid 64-hex `fiscal_hash` disagrees with the event's `current_hash` |
...
Citation sweep result: **resolved = 32; unresolved = 0**.
