...
```text
N_extracted=266 N_mapped=266 relocated=24 unresolved=0 output=docs/handoff/reviews/wave3-3c-3d/M0-citation-inventory.csv
csv_rows=266 extensionless=23 relocated=24 file_scope=0 comment_anchors=36
```

The extractor consumes the dispatch and the authoritative plan from its beginning through the end of §4. It extracts `File.php:line`, extensionless `Class:line`, backticked lowercase-method, and timestamp-prefixed migration forms, resolves each path, maps from the plan reference SHA where possible, and fails non-zero if the path, actual anchor line, real enclosing/documented symbol, or classified semantic assertion is absent. It distinguishes statement, comment, and document anchors: a citation whose reference construct is a rationale block stays on that block and names the method it documents, while a statement citation records the exact statement line found rather than a nearby blank, brace, or comment. Source annotations `(comment)` and `(docblock)` are consumed as required-kind constraints. A reference/current semantic-token coherence check fails closed with `semantic_drift_requires_relocation` when a mapped address contains a different construct. Twenty-four rows have `relocated` status, representing 17 fully pinned manual relocation keys with repeated citations counted per row; each pin validates target symbol and semantic text so a shifted hard-coded line becomes unresolved rather than silently passing. The command itself emits both reproducible metric lines above.

The complete row set is in `M0-citation-inventory.csv`; `scripts/tests/wave3-citation-inventory-test.php` pins the live corpus, all 17 manual relocation keys and target semantics, the comment-subject regression at `ReturnScrapWriteOffService.php:217-226`, docblock-to-symbol coherence, source-line classification, and positive/negative drift sensitivity. M2 is required to read this register and rerun the command before implementation; stale literals remaining in the authority prose are not implementation addresses.

Timestamp prefixes resolve only when unique. The ambiguous `2026_07_12_100000` prefix is explicitly mapped to `2026_07_12_100000_unique_journal_entries_source_treasury_transfer.php`, because plan §0.7 names the treasury-transfer partial unique at that row; the same-timestamp payment-instrument migration does not define the cited journal-entry index.
...
Round-3 fix-commit revert/replay: reverting `8f7d053a6` at `ac89b8c88` made the covering probe exit 1 with `FAIL: V10 inv-I1 comment citation no longer identifies its source block`; reapplying at `850b62b46` restored the `217-226` comment anchor and green corpus.

Round-4 response: the five deleted `DocumentPostingService` constructs now relocate to `DeliveryComplianceGate`'s physical predicate and unified linkage evaluation; D-19 twin B relocates to `DeliveryNoteFromDocumentFactory.php:111-118`. A fail-closed reference/current coherence check prevents a deleted construct from passing merely because another statement exists nearby, with a built-in positive/negative self-test. Comment-dominated ranges map to their comment block even when the first line is blank, so both V10 citation forms resolve to `217-226`. Closing-token citations prefer the preceding construct, putting `GeneralLedgerService.php:4444` on `source_id` line 4708. The same check exposed five legitimate integrated semantic successors, which are explicitly relocated and pinned rather than weakening the threshold.

The progress `commit` field records the latest milestone implementation/reapply commit. The immediately following metadata-only evidence commit necessarily makes `HEAD` one commit newer; this is deliberate and avoids a self-referential SHA that cannot be committed truthfully.

Round-4 fix-commit revert/replay: reverting `e87ca4345` at `b48e88974` made the covering probe exit 1 with `FAIL: deleted delivery predicate was accepted at an unrelated mapped statement`; reapplying at `06ebab27d` restored the cross-file successor and drift gate.

Round-5 response: extraction now includes all ten previously invisible plan citations, raising the corpus from 256 to 266. Timestamp prefixes have a unique-prefix resolver plus the documented treasury ambiguity rule, and the lowercase `validateDeliveryCompliance:605` citation joins the deleted-cluster relocation. The T18 `(comment)` annotation moves `ParapharmacySeeder.php:971` to its actual `PostCOGSOnInvoice` comment at `977-979`. Every relocation is now checked against an expected target symbol and semantic-text needle; stale absolute lines therefore fail with `relocation_semantic_pin_mismatch` during the required pre-M2 rerun.

Round-5 fix-commit revert/replay: reverting `8958d5c85` at `e45248fdf` reduced the corpus to 256 and made the covering probe exit 1 with `FAIL: timestamp-prefixed procurement index citation was not extracted`; reapplying at `f0a74d332` restored all 266 rows.
