...
**Frozen legacy fallback treatment:** purposes are deliberately absent from direct legacy-seeder output. No fingerprint re-pin is proposed. The unattended tenant migration runs the purpose-first backfill for every existing company before the rerouted destructive-loss writer is used. Newly certified country-default templates must now include `InventoryShrinkageExpense`, and every shipped v2 template includes both approved purposes. A company created later through the separately ruled frozen-seeder fallback can still omit the purposes; that compatibility path retains the explicit warning/no-entry guard until the idempotent backfill is run. Tests pin the automatic migration, the warning/no-entry fallback, and the v2-template posting-ready path. The fallback is a transitional owner-approved exception, not evidence that the variance was booked.
