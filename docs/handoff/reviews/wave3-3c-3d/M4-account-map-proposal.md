...
**Frozen legacy fallback treatment:** purposes are deliberately absent from direct legacy-seeder output. No fingerprint re-pin is proposed. The unattended tenant migration runs the purpose-first backfill for every existing company before the rerouted destructive-loss writer is used. Normal registration and second-company creation install the same definitions atomically through `ChartOfAccountsService` immediately after the frozen seeder. Newly certified country-default templates must include `InventoryShrinkageExpense`, and every shipped v2 template includes both approved purposes. Direct/raw seeder use outside the service remains the separately ruled compatibility case and retains the explicit warning/no-entry guard until the idempotent backfill is run. Tests pin the automatic migration, both creation paths, the warning/no-entry compatibility path, and the v2-template posting-ready path.
