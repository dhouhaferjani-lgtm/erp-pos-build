...
- The three legacy chart seeder files and their fingerprints are unchanged. Legacy-v1 templates remain
  separately publishable because both variance purposes remain SOFT at the template publication gate.
- The frozen fallback is deliberate absence: a legacy chart missing shrinkage logs a warning and creates
  no destructive-loss entry. The v2 template and the tenant backfill make the chart posting-ready.
