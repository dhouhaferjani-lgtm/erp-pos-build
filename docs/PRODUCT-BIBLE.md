...
| **Income** | ~85% (new) | Income recording BE + FE shipped (mirror of Expense; GL Dr cash/bank / Cr class-7 + repository inflow). Error-state UX fixed 2026-07-05; reported "Chargement…" symptom unreproduced (TD-013, narrowed). | low (para launch: nice-to-have) |
...
| TD-012 | **`/reports/finance-summary` — RESOLVED 2026-07-05.** Endpoint implemented (`Accounting/.../ReportsController::financeSummary`, gate `reports.view`): balance-sheet totals + P&L MTD/YTD + AR/AP summed from GL purpose-tagged accounts (`CustomerReceivable`/`SupplierPayable`) off the same flat balance-sheet result — NOT the Aged* open-document proxies (treasury-reviewer caught the PO-as-AP trap; negative fixtures pin it). | Resolved | Closed |
| TD-013 | **`/income` list "Chargement…" — PARTIALLY resolved 2026-07-05, symptom unreproduced.** Happy-path wiring verified correct (mirrors Expense; no double-unwrap, tenantScopedKey OK). `IncomeListPage` previously rendered the misleading "no income" empty state on 403/500 (no error branch) — a tested `QueryError` + retry branch added. The literal stuck-loading symptom is NOT producible by an error on TanStack v5 (`retry:1` → `isLoading=false`) — still owes a runtime check on the environment where it was seen. | Open (narrowed: error UX fixed; symptom unreproduced) | Low |
