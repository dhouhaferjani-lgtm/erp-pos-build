...
Task D4: complete (cash-movements hook preserves raw {data,meta} with tenant-scoped key; current-month/filter/mixed-currency totals/manual table/pagination/safe refs page; reports-gated lazy route + Accounting nav; en/fr/ar finance keys; RED verified; finance+route+Sidebar GREEN 27 files/172 tests; typecheck/lint/audits clean; pinned Doctor has 0 new diagnostics, with 93 display solely from four pre-existing host-file warnings outside changed lines)
