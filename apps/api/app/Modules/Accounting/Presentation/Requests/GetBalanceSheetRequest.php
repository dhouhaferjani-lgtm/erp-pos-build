 ...
 *
 * Form request validator for Balance Sheet (Statement of Financial Position) report queries.
 *
 * Validates query parameters for retrieving a balance sheet report:
 ...
 * - as_of_date OR fiscal_period_id: At least one can be provided, but not required
 * - fiscal_period_id: Must be a valid UUID, must exist in fiscal_periods table, must belong to current company
 ...
 * ```php
 * // Controller method
 * public function balanceSheet(GetBalanceSheetRequest $request) {
 ...
 * GET /api/v1/reports/balance-sheet?as_of_date=2025-12-31&include_zero_balances=true
 * GET /api/v1/reports/balance-sheet?include_hierarchy=false
 * ```
 */
class GetBalanceSheetRequest extends FormRequest
{
    ...
    {
        return [
            // Snapshot date for the balance sheet
            ...
            // If provided, the period's end_date will be used as the snapshot date
