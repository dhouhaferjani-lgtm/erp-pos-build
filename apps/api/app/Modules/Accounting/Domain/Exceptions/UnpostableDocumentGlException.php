 ...
 *
 * **Three unbalanced-entry types, and picking the wrong one silently does nothing.**
 * enforcement-P3 M1 added a third; the distinction is load-bearing for anyone
 * writing a `catch`:
 *
 *   | type | thrown by | parent |
 *   |---|---|---|
 *   | `UnpostableDocumentGlException` (this) | `DocumentPostingService::post()` pre-flight, BEFORE the seal | `\DomainException` → 422 `BUSINESS_ERROR` |
 *   | {@see UnbalancedJournalEntryException} | `AccountingService::assertLegsBalance()` — post-seal, document-sourced, its ONLY source | `\RuntimeException` → unmapped 500 |
 *   | {@see UnbalancedJournalEntryPostException} | the GL posting chokepoint, `GeneralLedgerService::sealAndPersistEntry()` — every `postEntry`/`postEntryNow` path | `\InvalidArgumentException` → unmapped 500 |
 *
 * The latter two are deliberately SIBLINGS under different parents, not one shared
 * type: M1 tried sharing and it broke a live contract in whichever direction the
 * parent was chosen (under `\InvalidArgumentException` the post-seal refusal fell
 * out of `CreditNoteController::post()`'s `catch (\RuntimeException)`; under
 * `\RuntimeException` the chokepoint's refusal newly hit `catch (\RuntimeException)`
 * clauses rendering **422**). The chokepoint's type keeps the `\InvalidArgumentException`
 * hierarchy its bare throw already had, so naming it changed no catch site.
 *
 * **Consequence for narrowing work** (e.g. the per-site narrowing R-10 asks for in
 * `docs/handoff/reviews/enforcement-p3/M1-census.md`): to intercept a CHOKEPOINT
 * balance refusal you must target `UnbalancedJournalEntryPostException` or the
 * `\InvalidArgumentException` family. Narrowing a `catch (\RuntimeException)` clause
 * will compile, read correctly, and match nothing.
 *
 * Both parents are guarded by
