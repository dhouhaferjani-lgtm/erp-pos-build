...
final readonly class AgedPayablesService
{
    ...
    private function getOutstandingInvoices(string $companyId, Carbon $asOfDate, int $scale, array $locationIds = []): Collection
    {
        ...
        $openingItems = $this->historicalSupplierOpenItems($companyId, $asOfDate, $scale, $locationIds);

        return $postedPurchaseOrders
            ->concat($autoReceivedPurchaseOrders)
            ->concat($openingItems)
            ->values();
    }

    /**
     * W4-4 — AP OPENING items, the debt the tenant carried in on day one.
     *
     * This report is purchase-ORDER driven: its two other arms read posted POs and
     * the uninvoiced accrual on auto-generated received POs. An opening supplier
     * item has no purchase order behind it — it was raised in the previous system —
     * so before this arm the 500.000 owed at cutover appeared in `GL 401` and in the
     * partner sub-ledger but in NO aged report, and the operator had no dated view
     * of what to pay first.
     *
     * Deliberately narrowed to `is_historical` documents. Non-historical supplier
     * invoices are ALSO absent from this report (they are represented by their PO),
     * and widening the type filter to every `SupplierInvoice` would double-count
     * them against the PO arms. That gap is real but pre-existing and out of this
     * lane — recorded as a residual in the W4-3 handback.
     *
     * `whereOutstanding()` reads the `balance_due` cache when it is warm;
     * `ArApOpeningService` seeds it with the OPEN amount at post, and
     * `Document::outstandingBalance()` treats a non-null cache as authoritative, so
     * a migrated partially-settled item ages at what is still owed, not at its
     * original total.
     *
     * @param  list<string>  $locationIds
     * @return Collection<int, Document>
     */
    private function historicalSupplierOpenItems(string $companyId, Carbon $asOfDate, int $scale, array $locationIds = []): Collection
    {
