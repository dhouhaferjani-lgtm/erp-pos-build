...
final class SalesReportService
{
    ...
    public function paymentMethodBreakdown(DateRangeData $range, array $companyIds, array $locationIds): array
    {
        ...
        // per-receipt change_due in a subquery BEFORE summing. The legacy POS
        // cash-count query faces the identical row-fan-out hazard and resolves
        // it the same way: MAX(change_due) grouped by receipt first, summed
        // second. Subtracting `pos_receipts.change_due`
