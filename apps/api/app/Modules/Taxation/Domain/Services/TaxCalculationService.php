    ...
    public function calculateDocumentTaxes(Document $document): TaxCalculationResult
    {
        ...
        foreach ($linesByRate as $rate => $linesWithRate) {
            ...
            if ($matchingConfig) {
                ...
                foreach ($linesWithRate as $line) {
                    $lineSubtotal = bcmul((string) $line->quantity, (string) $line->unit_price, $this->scale() + 1);
                    $lineTax = bcmul($lineSubtotal, $rateFraction, $this->scale() + 1);
                    $taxAccumulator = bcadd($taxAccumulator, $lineTax, $this->scale() + 1);
                }
                $taxAmount = CurrencyScale::bcformat($taxAccumulator, $this->scale());

                $lineItemsTaxTotal = bcadd($lineItemsTaxTotal, $taxAmount, $this->scale());
                ...
                $runningTaxTotal = bcadd($runningTaxTotal, $taxAmount, $this->scale());
            }
        }
        ...
        foreach ($applicableTaxes as $taxConfig) {
            if ($taxConfig->applies_to === TaxApplicationLevel::DocumentTotal) {
                ...
                $documentTaxTotal = bcadd($documentTaxTotal, $taxAmount, $this->scale());
        ...
        $totalTax = bcadd($lineItemsTaxTotal, $documentTaxTotal, $this->scale());
    ...
    private function calculateSubtotal(Document $document): string
    {
        ...
        foreach ($document->lines as $line) {
            ...
            $lineTotal = CurrencyScale::bcformat(
                bcmul((string) $line->quantity, (string) $line->unit_price, $this->scale() + 1),
                $this->scale(),
            );
        ...
        if ($document->discount_amount) {
            $subtotal = bcsub($subtotal, (string) $document->discount_amount, $this->scale());
        }
