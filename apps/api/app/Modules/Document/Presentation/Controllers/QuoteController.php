...
/**
 * Controller for Quote document operations.
 *
 * This controller handles all quote-specific endpoints:
 * - List quotes (with pagination and filters)
 * - Create new quotes
 * - View single quote
 * - Update draft quotes
 * - Delete draft quotes
 * - Confirm quotes (Draft -> Confirmed)
 */
class QuoteController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $query = $this->baseQuery()->ofType(DocumentType::Quote);
     ...
     */
    public function show(Request $request, string $quote): JsonResponse
    {
     ...
     */
    public function update(UpdateDocumentRequest $request, string $quote): JsonResponse
    {
        /** @var User $user */
        $user = $request->user();
     ...
     *
     * Only draft quotes can be deleted. Confirmed quotes cannot be deleted.
     *
     * DELETE /api/v1/quotes/{quote}
     */
    public function destroy(Request $request, string $quote): Response|JsonResponse
    {
        /** @var User $user */
        $user = $request->user();
    ...
    /**
     * Confirm a quote (Draft -> Confirmed).
     *
     * Uses pessimistic locking inside the transaction to prevent race conditions
     * when two requests try to confirm the same quote simultaneously.
     *
     * POST /api/v1/quotes/{quote}/confirm
     */
    public function confirm(Request $request, string $quote): JsonResponse
    {
        /** @var User $user */
        $user = $request->user();
        ...
        try {
            $documentModel = DB::transaction(function () use ($companyId, $quote): Document {
