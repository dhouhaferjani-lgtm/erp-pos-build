...
use App\Modules\Catalog\Presentation\Controllers\SignedMediaController;
...
// Signed-URL media serving — no bearer token required; HMAC signature is the gate.
// Short-lived URLs (60-min TTL) generated by MediaUrlResolver for uploaded assets;
// allows <img src="…"> in the bearer-token SPA without a CORS-authorized download proxy.
// throttle:signed-media — 120 req/min per IP (secondary defence against enumeration loops).
// `signed:relative` validates the HMAC over path + query only (no host), so the route works
// behind the nginx proxy / in Docker where the request host (api) differs from APP_URL.
// MediaUrlResolver mints RELATIVE signed URLs (absolute: false) to match.
Route::prefix('api/v1')->middleware(['api', 'signed:relative', 'throttle:signed-media'])->group(function () {
    Route::get('media/{tenant}/{attachment}/serve', [SignedMediaController::class, 'serve'])
        ->name('media.serve');
});
