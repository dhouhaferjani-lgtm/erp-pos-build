...
final class StoreReceiptRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        return [
            ...
            'lines.*.product_id' => ['nullable', 'uuid', 'exists:products,id', 'required_without:lines.*.composite_item_id'],
            'lines.*.composite_item_id' => ['nullable', 'uuid', 'exists:composite_items,id', 'required_without:lines.*.product_id'],
            'lines.*.quantity' => ['required', 'numeric', 'gt:0'],
            'lines.*.unit_price' => ['required', 'numeric', 'gte:0'],
            'lines.*.modifiers' => ['nullable', 'array'],
            'lines.*.modifiers.*.modifier_id' => ['required', 'uuid', 'exists:modifiers,id'],
