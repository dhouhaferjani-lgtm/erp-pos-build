...
/**
 * Slice D batch 1 — put the enum value domains of `payments` in the DATABASE.
 *
 * `payments` is the treasury spine. `status` decides whether cash is counted,
 * `payment_type` decides which side of the ledger the row lands on, and `origin`
 * is the POS/back-office attribution the Z-report reconciliation joins on.
 *
 * `origin` is NULLABLE (rows written before the origin lane carry NULL and are
 * read as `PaymentOrigin::UnknownLegacy` by the application, NOT rewritten), so
 * its CHECK carries an explicit `IS NULL` branch per the C-37(iii) rule.
 *
 * NOTE for the reviewer: `PaymentType` has SEVEN cases, not the nine the lane
 * brief's scope table asserted. The value list below is derived from
 * `PaymentType::cases()` and is therefore correct by construction; the brief's
 * count was wrong, not the enum.
 *
 ...
 *     FROM payments
 *    WHERE status IS NULL OR status NOT IN ('pending', 'completed', 'failed', 'reversed')
 *    GROUP BY status;
 *
 *   SELECT COALESCE(payment_type, '<NULL>') AS value, COUNT(*)
 *     FROM payments
 *    WHERE payment_type IS NULL OR payment_type NOT IN ('document_payment', 'advance', 'refund', 'credit_application', 'supplier_payment', 'pos', 'reversal')
 *    GROUP BY payment_type;
 *
 *   SELECT COALESCE(origin, '<NULL>') AS value, COUNT(*)
 *     FROM payments
 *    WHERE origin IS NOT NULL AND origin NOT IN ('pos', 'web_admin', 'mobile', 'api', 'unknown_legacy', 'back_office')
 *    GROUP BY origin;
 *
...
return new class extends Migration
{
    ...
    private const COLUMNS = [
        'status' => [PaymentStatus::class, false],
        'payment_type' => [PaymentType::class, false],
        'origin' => [PaymentOrigin::class, true],
    ];
