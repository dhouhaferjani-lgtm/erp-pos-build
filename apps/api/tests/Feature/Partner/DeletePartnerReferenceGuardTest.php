...
class DeletePartnerReferenceGuardTest extends TestCase
{
    ...
    private function referenceRow(string $table, string $partnerId): array
    {
        ...
        return match ($table) {
            ...
            // The sealed fiscal receipt family. `customer_id` is a VARCHAR,
            // not a uuid FK — see PosPartnerReferenceSource for why that is
            // counted anyway and what the device-local-uuid case means.
            'pos_deposit_receipts' => [
                'tenant_id' => $tenantId,
                'company_id' => $companyId,
                'customer_id' => $partnerId,
                'fiscal_event_id' => $uuid(),
                'deposit_receipt_uuid' => $uuid(),
                'customer_name' => 'Referenced Partner',
                'amount' => '25.000',
                'currency_code' => 'EUR',
                'payload_snapshot' => json_encode(['kind' => 'DEPOSIT_RECEIPT']),
            ],
            'pos_account_charge_receipts' => [
                'tenant_id' => $tenantId,
                'company_id' => $companyId,
                'customer_id' => $partnerId,
                'fiscal_event_id' => $uuid(),
                'account_charge_uuid' => $uuid(),
                'customer_name' => 'Referenced Partner',
                'amount_charged' => '25.000',
                'currency_code' => 'EUR',
                'payload_snapshot' => json_encode(['kind' => 'ACCOUNT_CHARGE']),
            ],
            'pos_account_payment_receipts' => [
                'tenant_id' => $tenantId,
                'company_id' => $companyId,
                'customer_id' => $partnerId,
                'fiscal_event_id' => $uuid(),
                'account_payment_uuid' => $uuid(),
                'customer_name' => 'Referenced Partner',
                'amount' => '25.000',
                'currency_code' => 'EUR',
                'payload_snapshot' => json_encode(['kind' => 'ACCOUNT_PAYMENT']),
            ],
            'journal_lines' => [
