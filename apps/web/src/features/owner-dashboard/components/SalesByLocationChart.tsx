...
export function SalesByLocationChart({ data, isLoading = false, isError = false }: SalesByLocationChartProps) {
  ...
  const option: EChartsOption = {
    color: [chartColors.primary, chartColors.success, chartColors.warning, chartColors.cyan, chartColors.violet],
    ...
    series: locations.map((location) => ({
      name: location,
      type: 'bar' as const,
      data: periods.map((period) => Number(data.find((row) => row.period === period && row.location_name === location)?.gross_sales ?? 0)),
    })),
  }
