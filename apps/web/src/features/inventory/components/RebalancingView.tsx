...
export function RebalancingView() {
  ...
  return <section className="space-y-3" aria-label={t('stockByLocation.rebalance.title')}><h2 className={`text-lg font-semibold ${textColors.primary}`}>{t('stockByLocation.rebalance.title')}</h2><div className="space-y-2">{moves.map((move) => <div className={`${tokens.card.base} flex items-center justify-between p-3`} key={`${move.row.product_id}-${move.from.location_id}-${move.to.location_id}`}><span className={textColors.primary}>{move.row.name}</span><span className={textColors.tertiary}>{locationNames.get(move.from.location_id) ?? move.from.location_id} → {locationNames.get(move.to.location_id) ?? move.to.location_id} · {formatQuantity(move.quantity, getQuantityDecimals(move.row))}</span></div>)}</div></section>
}
