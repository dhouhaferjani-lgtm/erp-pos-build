...
export function SalesWithholdingTrackingPage() {
  ...
  const [filters, setFilters] = useState({
    certificate_received: undefined as boolean | undefined,
    date_from: '',
    date_to: '',
  })

  // TODO: Implement API hook when backend endpoint is ready
  // const { data, isLoading } = useSalesWithholdingTracking(filters)
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full divide-y divide-gray-200">
              <thead className="bg-gray-50">
                <tr>
                  <th className="px-4 py-3 text-start text-xs font-medium text-gray-500 uppercase tracking-wider">
                    {t('salesWithholding.invoiceNumber')}
                  </th>
                  <th className="px-4 py-3 text-start text-xs font-medium text-gray-500 uppercase tracking-wider">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {/* Example row structure - will be populated with real data */}
                {/* {trackingRecords.map((record) => (
                  <tr key={record.id} className="hover:bg-gray-50">
                    <td className="px-4 py-3 whitespace-nowrap">
                      <Link
                        to={`/sales/invoices/${record.document_id}`}
                    ...
                    <td className="px-4 py-3 whitespace-nowrap text-end">
                      <div className="text-sm font-mono text-gray-900">
                        {parseFloat(record.invoice_amount).toFixed(3)} TND
                      </div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-end">
                      <div className="text-sm font-mono font-semibold text-red-600">
                        {parseFloat(record.withholding_amount).toFixed(3)} TND
                      </div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-end">
                      <div className="text-sm font-mono text-gray-900">
                        {parseFloat(record.expected_receivable).toFixed(3)} TND
                      </div>
                    </td>
                    <td className="px-4 py-3 whitespace-nowrap text-center">
                      {record.certificate_received ? (
