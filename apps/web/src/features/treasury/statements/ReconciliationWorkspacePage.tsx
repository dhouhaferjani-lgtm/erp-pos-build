...
export function ReconciliationWorkspacePage() {
  ...
  const movementsQuery = useQuery({
    queryKey: tenantScopedKey(['repository-movements', statement?.payment_repository_id ?? 'none', { search: movementSearch }]),
    queryFn: () => searchRepositoryMovements(statement?.payment_repository_id ?? '', movementSearch),
  ...
  async function refreshWorkspace() {
    await Promise.all([
      ...
      queryClient.invalidateQueries({ queryKey: ['bank-statement-line-suggestions'] }),
      queryClient.invalidateQueries({ queryKey: ['repository-movements'] }),
    ])
  }
  ...
  const resolved = statement.lines.filter((line) => line.match_status === 'matched' || line.match_status === 'ignored').length
  const remainingTotal = statement.lines.reduce((total, line) => total.plus(remainingForLine(line)), new Big(0)).toFixed(3)
  const ignoredTotal = statement.lines.reduce((total, line) => line.match_status !== 'ignored' ? total : line.direction === 'in' ? total.plus(line.amount) : total.minus(line.amount), new Big(0)).toFixed(3)
  ...
  return (
    <div className="space-y-6">
      ...
      <StatementCompletionDialog key={String(completeOpen)} isOpen={completeOpen} ignoredTotal={ignoredTotal} currency={statement.currency} pending={completeMutation.isPending} onClose={() => { setCompleteOpen(false) }} onConfirm={(acknowledged) => { completeMutation.mutate(acknowledged) }} />
    </div>
  )
}
