...
export function ManualMatchSearch({ movements, currency, lineRemaining, search, amount, disabled = false, onSearch, onAmountChange, onAllocate }: ManualMatchSearchProps) {
  ...
  const maxAmount = (lineCapacity.lte(movementCapacity) ? lineCapacity : movementCapacity).toFixed(3)
  ...
  return (
    <section className="space-y-3" aria-label={t('statements.workspace.manual.title')}>
      ...
      <label className={tokens.label.base}>{t('statements.workspace.manual.movement')}<Select value={effectiveSelectedId} disabled={movements.length === 0} onChange={(event) => { setSelectedId(event.target.value) }}><option value="">{t('statements.workspace.manual.none')}</option>{movements.map((movement) => <option key={movement.id} value={movement.id}>{movement.source_type} · {movement.occurred_at.slice(0, 10)} · {movement.source_id}</option>)}</Select></label>
      ...
      <label className={tokens.label.base}>{t('statements.workspace.manual.amount')}<MoneyInput aria-label={t('statements.workspace.manual.amount')} value={amount} onChange={onAmountChange} currency={currency} min="0.001" max={maxAmount} /></label>
