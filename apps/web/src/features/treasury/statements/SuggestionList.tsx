  ...
  return (
    <section className="space-y-3" aria-label={t('statements.workspace.suggestions.title')}>
      ...
      {suggestions.map((suggestion) => (
        <article key={`${suggestion.tier}-${suggestion.action_type ?? suggestion.movement_ids.join('-')}`} className={cn('rounded-lg border p-3', semanticColorTokens.border.subtle, semanticColorTokens.surface.pageAlpha)}>
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-1">
              <div className="flex items-center gap-2"><StatusBadge tone={suggestion.tier === 1 ? 'success' : suggestion.tier === 2 ? 'info' : 'pending'}>{t('statements.workspace.suggestions.tier', { tier: suggestion.tier })}</StatusBadge>{suggestion.reference_matched ? <StatusBadge tone="success">{t('statements.workspace.suggestions.reference')}</StatusBadge> : null}</div>
              <p className={cn('text-sm', textColors.secondary)}>{suggestion.reason}</p>
