...
export function LinePanel({ line, currency, suggestions, movements, pending = false, mutable = true, onSearch, onExecute, onAllocate, onUnallocate, onIgnore, onUnignore, onCreate }: LinePanelProps) {
  ...
  return (
    <aside className="space-y-6">
      ...
      {line.match_status === 'ignored' ? <div className={cn(tokens.alert.base, tokens.alert.warning)}><div><strong>{t(`statements.workspace.ignore.reasons.${line.ignore_reason ?? 'other'}`)}</strong><p>{line.ignore_text}</p></div>{mutable ? <Button size="sm" variant="secondary" disabled={pending} onClick={onUnignore}><RotateCcw className="me-2 h-4 w-4" />{t('statements.workspace.ignore.unignore')}</Button> : null}</div> : <>
        {mutable ? <>
