          ...
          {showProfileForm ? (
            <div className={cn('space-y-4 rounded-lg border p-4', semanticColorTokens.border.subtle, semanticColorTokens.surface.pageAlpha)}>
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <label className={tokens.label.base}>{t('statements.profile.parser')}<Select value={parserKey} onChange={(event) => { const value = event.target.value; if (isParserKey(value)) setParserKey(value) }}><option value="csv">CSV</option><option value="xlsx">XLSX</option></Select></label>
                <label className={tokens.label.base}>{t('statements.profile.direction')}<Select value={directionConvention} onChange={(event) => { const value = event.target.value; if (isDirectionConvention(value)) setDirectionConvention(value) }}><option value="signed_amount">{t('statements.profile.signed')}</option><option value="debit_credit_columns">{t('statements.profile.debitCredit')}</option></Select></label>
                <label className={tokens.label.base}>{t('statements.profile.decimal')}<Select value={decimalFormat} onChange={(event) => { const value = event.target.value; if (isDecimalFormat(value)) setDecimalFormat(value) }}><option value="comma_decimal">1 234,56</option><option value="dot_decimal">1,234.56</option><option value="comma">1234,56</option><option value="dot">1234.56</option></Select></label>
