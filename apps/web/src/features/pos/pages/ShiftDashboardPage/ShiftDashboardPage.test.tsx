...
describe('ShiftDashboardPage', () => {
  ...
  it('calls onOpenShift when opening balance is submitted', async () => {
    ...
    await waitFor(() => {
      expect(onOpenShift).toHaveBeenCalledWith('500.000')
    })
  })
  ...
  it('displays shift duration when shift is open', () => {
    ...
    // Should show time elapsed (format may vary) - appears in header and shift info
    const durationElements = getAllByText(/hours?|minutes?/i)
  ...
  it('calls onCloseShift with actual cash count when closing shift', async () => {
    ...
    fireEvent.change(input, { target: { value: '1245.000' } })

    const confirmButton = getByRole('button', { name: /confirm/i })
    fireEvent.click(confirmButton)

    await waitFor(() => {
      expect(onCloseShift).toHaveBeenCalledWith('1245.000')
    })
  })
  ...
  it('calculates and displays cash variance when closing shift', async () => {
    ...
    // Expected: 1250.000, Actual: 1245.000, Variance: -5.000
    await waitFor(() => {
  ...
  it('calls onCashDeposit when deposit button clicked', async () => {
    ...
    fireEvent.change(amountInput, { target: { value: '100.000' } })
