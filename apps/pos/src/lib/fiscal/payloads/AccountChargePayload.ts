...
export interface AccountChargeCreditDecision {
  ...
  limit_remaining_after: string | null;
  rule_code: string;
}
...
export interface AccountChargePayload {
  ...
  invoice_classification: 'B2C_ACCOUNT_CHARGE' | 'B2B_FACTURE_DRAFT_SOURCE';
...
export function goldenAccountChargePayload(): AccountChargePayload {
  return {
    ...
    credit_decision: {
      ...
      limit_remaining_after: '81.000',
      rule_code: 'approved_within_limit',
    },
    currency_code: 'TND',
    currency_scale: 3,
    customer: {
    ...
    invoice_classification: 'B2C_ACCOUNT_CHARGE',
    line_items: [
      {
        gtin: null,
