...
export function Modal({ isOpen, onClose, title, children, footer, size = 'md', closable = true }: ModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      {/* Backdrop */}
      <div
        className="ez-fade-in absolute inset-0 bg-black/50"
      ...
      >
        ...
        <div className="flex shrink-0 items-center justify-between border-b border-border-subtle px-6 py-4">
          ...
          <button
            ...
            className="flex h-12 w-12 items-center justify-center rounded-full text-ink-faint hover:bg-surface-sunken hover:text-ink disabled:cursor-not-allowed disabled:opacity-30 disabled:hover:bg-transparent disabled:hover:text-ink-faint"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
