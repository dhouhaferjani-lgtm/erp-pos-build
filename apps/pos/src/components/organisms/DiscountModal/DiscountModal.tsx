...
export interface DiscountModalProps {
  isOpen: boolean;
  onClose: () => void;
  onApplyTransactionDiscount: (data: DiscountApplyPayload) => void;
  canDiscount: boolean;
...
export function DiscountModal({
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      targetReferenceIdRef.current = crypto.randomUUID();
    }
  }, [isOpen]);
  ...
  const handleManagerPinSubmit = useCallback(async () => {
    ...
    try {
      ...
      const manager = await verifyScopedManagerPin({
        ...
        targetEventType: 'DISCOUNT_LIMIT_OVERRIDE',
        targetReferenceId: targetReferenceIdRef.current,
        reason: reasonText || 'Discount limit override',
      });
      const approvalEvidence = await authorPosOverride({
        ...
        target: {
          ...
          reason: reasonText,
          target_reference_id: targetReferenceIdRef.current,
        },
  ...
  return (
    ...
    >
      ...
      <div className="flex shrink-0 items-center justify-between border-b border-border-subtle bg-surface-raised px-4 py-3">
        ...
        <span id="discount-modal-title" className="text-lg font-bold text-ink">
        ...
        <div className="flex flex-[3] flex-col">
          {needsApproval ? (
            <>
              ...
              <div className="mb-3 rounded-tile bg-action-subtle border border-action p-3 text-center text-sm text-action">
                {discountType === 'percentage'
            ...
            <>
              ...
              <div className="mb-4">
                <label className="mb-1 block text-sm font-medium text-ink-muted">
                  {t('discount.reason')}
                  {requiresReason && <span className="text-danger"> *</span>}
                </label>
                <input
                  type="text"
                  value={reason}
