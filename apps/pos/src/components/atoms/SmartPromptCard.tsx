...
export function SmartPromptCard({ recommendation, onAdd }: SmartPromptCardProps) {
  ...
  return (
    <div className="smart-prompt-card relative flex items-center gap-3 rounded-tile border border-indigo-500/10 bg-indigo-500/5 p-2">
      <div className="min-w-0 flex-1">
        <div className="truncate text-sm font-medium">{recommendation.product_name}</div>
        <div className="flex items-center gap-1.5">
          <span className="rounded-sm bg-indigo-500/15 px-1.5 py-0.5 text-[10px] text-indigo-400">
      ...
      {showInfo && (
        <div className="absolute right-0 top-full z-10 mt-1 w-64 rounded-tile border border-indigo-500/20 bg-gray-800 p-3 text-xs shadow-lg">
